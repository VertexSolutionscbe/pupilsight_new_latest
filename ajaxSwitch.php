...
if (isset($_POST['type'])) {
    ...
    switch ($type) {
                    ...
                    if ($amtPaid < $transcation_amount) {
                        ...
                        if (!empty($valuesip)) {
                            ...
                            foreach ($valuesip as $itmid) {
                                ...
                                $chkcolsql = 'SELECT total_amount, SUM(total_amount_collection) as tot_amt_col FROM fn_fees_student_collection WHERE pupilsightPersonID = ' . $pupilsightPersonID . ' AND invoice_no = "' . $invoice_no . '" AND fn_fee_invoice_item_id = "' . $itid . '" AND total_amount >= 0 AND is_active = "1" ';
        ...
        case "loadInvoicesCollections":
            ...
            // $oldinvoices = 'SELECT fn_fee_invoice.*,fn_fee_invoice.id as invoiceid, fn_fee_invoice_student_assign.invoice_no as stu_invoice_no, fn_fee_invoice_student_assign.id as invid, g.is_fine_editable, g.fine_type, g.rule_type, GROUP_CONCAT(DISTINCT asg.route_id) as routes, GROUP_CONCAT(DISTINCT asg.transport_type) as routetype FROM fn_fee_invoice LEFT JOIN pupilsightStudentEnrolment ON fn_fee_invoice.pupilsightSchoolYearID=pupilsightStudentEnrolment.pupilsightSchoolYearID LEFT JOIN pupilsightPerson ON pupilsightStudentEnrolment.pupilsightPersonID=pupilsightPerson.pupilsightPersonID RIGHT JOIN fn_fee_invoice_student_assign ON pupilsightPerson.pupilsightPersonID=fn_fee_invoice_student_assign.pupilsightPersonID AND fn_fee_invoice.id = fn_fee_invoice_student_assign.fn_fee_invoice_id LEFT JOIN fn_fees_fine_rule AS g ON fn_fee_invoice.fn_fees_fine_rule_id = g.id LEFT JOIN trans_route_assign AS asg ON pupilsightPerson.pupilsightPersonID = asg.pupilsightPersonID WHERE fn_fee_invoice.pupilsightSchoolYearID != "' . $pupilsightSchoolYearID . '" AND pupilsightPerson.pupilsightPersonID = "' . $stuId . '" AND fn_fee_invoice_student_assign.invoice_status != "Fully Paid"  AND fn_fee_invoice_student_assign.status = 1  GROUP BY fn_fee_invoice.id ORDER BY fn_fee_invoice_student_assign.id ASC';

            $oldinvoices = 'SELECT fn_fee_invoice.*,fn_fee_invoice.id as invoiceid, fn_fee_invoice_student_assign.invoice_no as stu_invoice_no, fn_fee_invoice_student_assign.id as invid, g.is_fine_editable, g.fine_type, g.rule_type, GROUP_CONCAT(DISTINCT asg.route_id) as routes, GROUP_CONCAT(DISTINCT asg.transport_type) as routetype FROM fn_fee_invoice 
            RIGHT JOIN fn_fee_invoice_student_assign ON fn_fee_invoice.id = fn_fee_invoice_student_assign.fn_fee_invoice_id LEFT JOIN fn_fees_fine_rule AS g ON fn_fee_invoice.fn_fees_fine_rule_id = g.id LEFT JOIN trans_route_assign AS asg ON fn_fee_invoice_student_assign.pupilsightPersonID = asg.pupilsightPersonID WHERE fn_fee_invoice.pupilsightSchoolYearID != "' . $pupilsightSchoolYearID . '" AND fn_fee_invoice_student_assign.pupilsightPersonID = "' . $stuId . '" AND fn_fee_invoice_student_assign.invoice_status != "Fully Paid"  AND fn_fee_invoice_student_assign.status = 1  GROUP BY fn_fee_invoice.id ORDER BY fn_fee_invoice_student_assign.id ASC';
            $oldresultinv = $connection2->query($oldinvoices);
            $oldinvdata = $oldresultinv->fetchAll();
            ...
            if (!empty($oldinvdata)) {
                $invdata = array_merge($oldinvdata, $invdata);
            }
            // echo '<pre>';
            // print_r($invdata);
            // echo '</pre>';
            ...
            foreach ($invdata as $k => $d) {
                $academicID = $d['pupilsightSchoolYearID'];
                ...
                $sqlchkInv1 = 'SELECT count(id) as kount FROM fn_fee_invoice_student_assign WHERE invoice_no = "' . $invno . '" AND invoice_status = "Fully Paid" AND status = "1" ';
                $resultchkInv1 = $connection2->query($sqlchkInv1);
                $invChk1 = $resultchkInv1->fetch();

                if (!empty($invChk1) && $invChk1['kount'] >= 1) {
                ...
                $sqlyr = 'SELECT name FROM pupilsightSchoolYear WHERE pupilsightSchoolYearID = ' . $academicID . ' ';
                $resultyr = $connection2->query($sqlyr);
                $datayr = $resultyr->fetch();
                $invdata[$k]['academic_year'] = $datayr['name'];
...
?>
    ...
    <td>" . $row['sectionname'] . "</td>";
            ...
            case 'studentMarks_excel_new':
                ...
                $data = $result->fetchAll();
                // foreach ($data as $k => $dt) {
                //     $sqlm = 'SELECT a.*, b.subject_display_name as subject_name, c.skill_display_name,m.max_marks FROM examinationMarksEntrybySubject AS a LEFT JOIN subjectToClassCurriculum AS b ON a.pupilsightDepartmentID = b.pupilsightDepartmentID LEFT JOIN subjectSkillMapping AS c ON a.skill_id = c.skill_id
                //         LEFT JOIN examinationSubjectToTest AS m ON a.test_id = m.test_id AND a.pupilsightDepartmentID = m.pupilsightDepartmentID
                //         WHERE a.test_id = ' . $testId . ' AND a.pupilsightPersonID = ' . $dt['pupilsightPersonID'] . ' AND b.pupilsightProgramID = ' . $program . ' AND b.pupilsightYearGroupID =' . $cls . '  GROUP by a.pupilsightDepartmentID,c.skill_id';
                //     $resultm = $connection2->query($sqlm);
                //     $datam = $resultm->fetchAll();
                //     if (!empty($datam)) {
                //         $data[$k]['marks'] = $datam;
                //     }
                // }
                // $sql1 = 'SELECT a.*, b.subject_display_name as subject_name, c.skill_display_name,m.max_marks FROM examinationMarksEntrybySubject AS a LEFT JOIN subjectToClassCurriculum AS b ON a.pupilsightDepartmentID = b.pupilsightDepartmentID LEFT JOIN subjectSkillMapping AS c ON a.skill_id = c.skill_id
                // LEFT JOIN examinationSubjectToTest AS m ON a.test_id = m.test_id AND a.pupilsightDepartmentID = m.pupilsightDepartmentID
                // WHERE a.test_id = ' . $testId . '  AND b.pupilsightProgramID = ' . $program . ' AND b.pupilsightYearGroupID =' . $cls . ' GROUP by a.pupilsightDepartmentID,c.skill_id';


                $sql1 = 'SELECT a.*, b.subject_display_name as subject_name, b.subject_type, c.skill_display_name FROM examinationSubjectToTest AS a LEFT JOIN subjectToClassCurriculum AS b ON a.pupilsightDepartmentID = b.pupilsightDepartmentID LEFT JOIN subjectSkillMapping AS c ON a.skill_id = c.skill_id
                    WHERE a.test_id = ' . $testId . ' AND a.is_tested = "1" AND b.pupilsightProgramID = ' . $program . ' AND b.pupilsightYearGroupID =' . $cls . ' GROUP by a.pupilsightDepartmentID,c.skill_id';
                $resu_h = $connection2->query($sql1);
                $datam_h = $resu_h->fetchAll();
                    ...
                    <table id="excelexport">
                        ...
                        foreach ($data as $row) {
                            ...
                            foreach ($datam_h as $m) {
                                ...
                                //     $sqlelc = 'SELECT id FROM assign_elective_subjects_tostudents WHERE pupilsightPersonID = '.$row['pupilsightPersonID'].' AND pupilsightDepartmentID = '.$pupilsightDepartmentID.' ';
                                ...
                                $sql = 'SELECT * FROM examinationMarksEntrybySubject WHERE test_id = ' . $testId . ' AND pupilsightPersonID = ' . $row['pupilsightPersonID'] . ' AND pupilsightDepartmentID = ' . $pupilsightDepartmentID . ' ';
                                if (!empty($skill_id)) {
                                    $sql .= " AND skill_id = " . $skill_id . " ";
                                }
                                //echo $sql.'<br>';
                                $resu_h = $connection2->query($sql);
                                $dataRes = $resu_h->fetch();

                                if (!empty($dataRes)) {
                                    $sql = 'SELECT grade_name FROM  examinationGradeSystemConfiguration WHERE id="' . $dataRes['gradeId'] . '"';
                                    $result = $connection2->query($sql);
                                    $gradeName = $result->fetch();
                                    $grade_name = '';
                                    ...
                                    // $marks = str_replace(".00", "", $m['marks_obtained']);
                                    $marks = $dataRes['marks_obtained'];
                                    // if (!empty($marks) ) {
                                    //     if ($dataRes['marks_abex']) {
                                    //         $marks = $dataRes['marks_abex'];
                                    //     }
                                    // }

                                    if (!empty($dataRes['marks_abex'])) {
                                        $marks = $dataRes['marks_abex'];
                                    }
            ...
            case 'studentMarks_excel_report':
                $program = $_POST['program'];
                $cls = $_POST['cls'];
                $section = $_POST['section'];
                $testId = implode(',', $_POST['testId']);
                ...
                $resu_h = $connection2->query($sql1);
                $datam_h = $resu_h->fetchAll();
                $cols = count($datam_h) + 5;

                $row1 = $data[0];

                //get Exam Names
                $sqe = "SELECT group_concat(name separator ', ') as name FROM examinationTest where id in(" . $testId . ")";
                $re = $connection2->query($sqe);
                $dtes = $re->fetch();
                $testName = $dtes["name"];
                $header = $row1['classname'] . " " . $row1['sectionname'];
                if ($testName) {
                    $header .= ", " . $testName;
                }
                ...
                ?>
                    <table id="excelexport">
                        <tr>
                            <?php
                            echo "<th colspan='" . $cols . "' style='text-align:center;'>" . strtoupper($header) . "</th>";
                            ?>
                        </tr>
                        <tr>
                            <th>Srno</th>
                            ...
                            foreach ($datam_h as $m) {
                                $hdCol = ""; //header column
                                if ($m['skill_display_name']) {
                                    $hdCol = $m['subject_name'] . "-" . $m['skill_display_name'] . "/" . ceil($m['max_marks']);
                                } else {
                                    $hdCol = $m['subject_name'] . "/" . ceil($m['max_marks']);
                                }
                                echo "<th>$hdCol</th>";
                                if (is_numeric($m['max_marks'])) {
                                    $totalMaxMarks += $m['max_marks'];
                                }
                            }
                            echo "<th>Total Marks</th><th>%</th>";
                            $totalMaxMarks = ceil($totalMaxMarks);
                            ?>
                        </tr>
                        ...
                        foreach ($data as $row) {
                            ...
                            $srno++;
                            try {
                                $sqelc = "SELECT group_concat(s.name separator ', ') as name FROM assign_elective_subjects_tostudents as e 
                            left join pupilsightDepartment as s on e.pupilsightDepartmentID=s.pupilsightDepartmentID
                            where 
                            e.pupilsightPersonID='" . $row['pupilsightPersonID'] . "' and 
                            e.pupilsightYearGroupID='" . $cls . "' and 
                            e.pupilsightProgramID='" . $program . "' ";
                                $re_le = $connection2->query($sqelc);
                                $da_le = $re_le->fetch();
                                echo "<td>" . $da_le["name"]  . "</td>";
                            } catch (Exception $ex) {
                                print_r($ex);
                            }
                            ...
                            echo "<td>" . $total . "</td>";
                            if ($total > 0) {
                                $perMarks = round((($total * 100) / $totalMaxMarks), 2);
                            }
                            echo "<td>" . $perMarks . "</td>";
