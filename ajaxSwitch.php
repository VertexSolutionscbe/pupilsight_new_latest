...
if (isset($_POST["type"])) {
    ...
    switch ($type) {
            ...
            if ($remarks == "" or $trans_id == "") {
                ...
                try {
                    foreach ($trans_id as $ts) {
                        ...
                        $sqlpt =
                            "SELECT transaction_id FROM fn_fees_collection WHERE id = " .
                            $ts .
                            " ";
                        ...
                        $data = [
                            "remarks" => $remarks,
                            "fn_fees_collection_id" => $ts,
                            "canceled_by" => $cuid,
                            "cdt" => $cdt,
                        ];
                        $sql =
                            "INSERT INTO fn_fees_cancel_collection SET remarks=:remarks, fn_fees_collection_id=:fn_fees_collection_id, canceled_by=:canceled_by, cdt=:cdt";
                        $result = $connection2->prepare($sql);
                        $result->execute($data);

                        $datau = ["transaction_status" => "2", "id" => $ts];
                        $sqlu =
                            "UPDATE fn_fees_collection SET transaction_status=:transaction_status WHERE id=:id";
                        $resultu = $connection2->prepare($sqlu);
                        $resultu->execute($datau);

                        $sqlinv =
                            "SELECT * FROM fn_fees_student_collection WHERE transaction_id = " .
                            $transId .
                            " ";
                        ...
                        if (!empty($valueinvData)) {
                            foreach ($valueinvData as $valueinv) {
                                $dataui = [
                                    ...
                                    "invoice_no" => $valueinv["invoice_no"],
                                ];
                                $sqlui =
                                    "UPDATE fn_fee_invoice_student_assign SET invoice_status=:invoice_status WHERE invoice_no=:invoice_no";
                                $resultui = $connection2->prepare($sqlui);
                                $resultui->execute($dataui);

                                $datai = [
                                    "pupilsightPersonID" =>
                                        $valueinv["pupilsightPersonID"],
                                    "transaction_id" =>
                                        $valueinv["transaction_id"],
                                    "fn_fees_invoice_id" =>
                                        $valueinv["fn_fees_invoice_id"],
                                    "fn_fee_invoice_item_id" =>
                                        $valueinv["fn_fee_invoice_item_id"],
                                    "invoice_no" => $valueinv["invoice_no"],
                                    "total_amount" => $valueinv["total_amount"],
                                    "discount" => $valueinv["discount"],
                                    "total_amount_collection" =>
                                        $valueinv["total_amount_collection"],
                                    "status" => $valueinv["status"],
                                ];
                                $sqli =
                                    "INSERT INTO fn_fees_student_cancel_collection SET pupilsightPersonID=:pupilsightPersonID, transaction_id=:transaction_id, fn_fees_invoice_id=:fn_fees_invoice_id, fn_fee_invoice_item_id=:fn_fee_invoice_item_id, invoice_no=:invoice_no, total_amount=:total_amount, discount=:discount,  total_amount_collection=:total_amount_collection, status=:status";
        ...
        case "collectionForm_request":
            ...
            if (!empty($_POST["reference_date"])) {
                ...
                $reference_date = date(
                    "Y-m-d",
                    strtotime(implode("-", array_reverse($fd)))
                );
            } else {
                $reference_date = "";
            }
            ...
            if (!empty($bank_id)) {
                $sqlbn =
                    "SELECT name FROM fn_masters WHERE id = " . $bank_id . " ";
            ...
            if (!empty($_POST["payment_date"])) {
                ...
                $payment_date = date(
                    "Y-m-d",
                    strtotime(implode("-", array_reverse($pd)))
                );
            } else {
                $payment_date = "";
            }
            ...
            if (!empty($_POST["dd_cheque_date"])) {
                ...
                $instrument_date = date(
                    "Y-m-d",
                    strtotime(implode("-", array_reverse($insd)))
                );
            } else {
                $instrument_date = "";
            }
            ...
            $sqlrt =
                "SELECT a.ac_no, b.path, b.column_start_by FROM fn_fees_head AS a LEFT JOIN fn_fees_receipt_template_master AS b ON a.receipt_template = b.id WHERE a.id = " .
                $fn_fees_head_id .
                " ";
            ...
            $total_amount_without_fine_discount =
                $_POST["total_amount_without_fine_discount"];
                ...
                try {
                    if (!empty($_POST["receipt_number"])) {
                        ...
                        if (!empty($fn_fees_receipt_series_id)) {
                            $sqlrec =
                                'SELECT id, formatval FROM fn_fee_series WHERE id = "' .
                            ...
                            foreach ($invformat as $inv) {
                                if ($inv == "{AB}") {
                                    $datafort = [
                                        "fn_fee_series_id" => $fn_fees_receipt_series_id,
                                        "order_wise" => $orderwise,
                                        "type" => "numberwise",
                                    ];
                                    $sqlfort =
                                        "SELECT id, no_of_digit, last_no FROM fn_fee_series_number_format WHERE fn_fee_series_id=:fn_fee_series_id AND order_wise=:order_wise AND type=:type";
                                    $resultfort = $connection2->prepare(
                                        $sqlfort
                                    );
                                    ...
                                    $sqlfort1 =
                                        "UPDATE fn_fee_series_number_format SET last_no=:last_no WHERE fn_fee_series_id=:fn_fee_series_id AND type=:type AND order_wise=:order_wise";
                                    $resultfort1 = $connection2->prepare(
                                        $sqlfort1
                                    );
                    ...
                    $data = [
                        "fn_fees_invoice_id" => $fn_fees_invoice_id,
                        "pupilsightPersonID" => $pupilsightPersonID,
                        "pupilsightSchoolYearID" => $pupilsightSchoolYearID,
                        "fn_fees_counter_id" => $counterid,
                        "receipt_number" => $receipt_number,
                        "is_custom" => $is_custom,
                        "payment_mode_id" => $payment_mode_id,
                        "bank_id" => $bank_id,
                        "dd_cheque_no" => $dd_cheque_no,
                        "dd_cheque_date" => $dd_cheque_date,
                        "dd_cheque_amount" => $dd_cheque_amount,
                        "payment_status" => $payment_status,
                        "payment_date" => $payment_date,
                        "fn_fees_head_id" => $fn_fees_head_id,
                        "fn_fees_receipt_series_id" => $fn_fees_receipt_series_id,
                        "transcation_amount" => $transcation_amount,
                        "total_amount_without_fine_discount" => $total_amount_without_fine_discount,
                    ...
                    $sqlu =
                        "UPDATE fn_fees_collection SET transaction_id=:transaction_id WHERE id=:id";
                    $resultu = $connection2->prepare($sqlu);
                    $resultu->execute($datau);
                    ...
                    if ($amtPaid < $transcation_amount) {
                        $isql =
                            "SELECT a.id, a.fn_fee_invoice_id, a.total_amount,b.invoice_no, a.fn_fee_item_id FROM fn_fee_invoice_item AS a LEFT JOIN fn_fee_invoice_student_assign AS b ON a.fn_fee_invoice_id = b.fn_fee_invoice_id WHERE a.id IN (" .
                            $invoice_item_id .
                        ...
                        if (!empty($valuesip)) {
                            ...
                            foreach ($valuesip as $itmid) {
                                ...
                                if ($itemamount < $chkamount) {
                                    ...
                                    $status = "1";
                                    $sqdis =
                                        "SELECT * FROM fn_fee_item_level_discount WHERE pupilsightPersonID = " .
                                        $pupilsightPersonID .
                                        " AND item_id =  " .
                                        $itid .
                                        " ";
                                ...
                                $datai = [
                                    "pupilsightPersonID" => $pupilsightPersonID,
                                    "transaction_id" => $transactionId,
                                    "fn_fees_invoice_id" => $fn_fee_invoice_id,
                                    "fn_fee_invoice_item_id" => $itid,
                                    "invoice_no" => $invoice_no,
                                    "total_amount" => $itemamount,
                                    "total_amount_collection" => $paidamount,
                                    "status" => $status,
                                ];
                                //print_r($datai);
                                $sqli =
                                    "INSERT INTO fn_fees_student_collection SET pupilsightPersonID=:pupilsightPersonID, transaction_id=:transaction_id, fn_fees_invoice_id=:fn_fees_invoice_id, fn_fee_invoice_item_id=:fn_fee_invoice_item_id, invoice_no=:invoice_no, total_amount=:total_amount, total_amount_collection=:total_amount_collection, status=:status";
                                $resulti = $connection2->prepare($sqli);
                                $resulti->execute($datai);

                                $desql =
                                    "SELECT id FROM fn_fees_deposit_account WHERE fn_fee_item_id = " .
                                    $fn_fee_item_id .
                                    ' AND overpayment_account != "1" ';
                                ...
                                if (!empty($depData)) {
                                    ...
                                    $sqld =
                                        "INSERT INTO fn_fees_collection_deposit SET deposit_account_id=:deposit_account_id, pupilsightPersonID=:pupilsightPersonID, pupilsightSchoolYearID=:pupilsightSchoolYearID, amount=:amount, transaction_id=:transaction_id, status=:status, cdt=:cdt";
                        ...
                        if (!empty($invoice_item_id)) {
                            ...
                            foreach ($itemId as $itid) {
                                ...
                                $sqlf =
                                    "SELECT a.fn_fee_invoice_id,a.total_amount,b.invoice_no, a.fn_fee_item_id FROM fn_fee_invoice_item AS a LEFT JOIN fn_fee_invoice_student_assign AS b ON a.fn_fee_invoice_id = b.fn_fee_invoice_id WHERE a.id=:id AND b.pupilsightPersonID=:pupilsightPersonID";
                                $resultf = $connection2->prepare($sqlf);
                                $resultf->execute($dataf);
                                $values = $resultf->fetch();
                                $fn_fee_invoice_id =
                                    $values["fn_fee_invoice_id"];
                                $invoice_no = $values["invoice_no"];
                                $itemamount = $values["total_amount"];
                                $fn_fee_item_id = $values["fn_fee_item_id"];

                                $chkpayitem =
                                    "SELECT a.id FROM fn_fees_student_collection AS a LEFT JOIN fn_fees_collection AS b ON a.transaction_id = b.transaction_id WHERE a.fn_fees_invoice_id = " .
                                    $fn_fee_invoice_id .
                                    " AND a.fn_fee_invoice_item_id = " .
                                    $itid .
                                ...
                                if (!empty($valuecp)) {
                                    $datai = [
                                        "partial_transaction_id" => $transactionId,
                                        "total_amount_collection" => $itemamount,
                                        "status" => "1",
                                        "id" => $valuecp["id"],
                                    ];
                                    $sqli =
                                        "UPDATE fn_fees_student_collection SET partial_transaction_id=:partial_transaction_id, total_amount_collection=:total_amount_collection, status=:status WHERE id=:id";
                                    $resulti = $connection2->prepare($sqli);
                                    $resulti->execute($datai);
                                } else {
                                    $sqdis =
                    ...
                    if (!empty($deposit)) {
                        $desql =
                            'SELECT id FROM fn_fees_deposit_account WHERE overpayment_account = "1" ';
                    ...
                    if (!empty($deposit_fee_item_account_id)) {
                        $datad = [
                            "deposit_account_id" => $deposit_fee_item_account_id,
                            "pupilsightPersonID" => $pupilsightPersonID,
                            "pupilsightSchoolYearID" => $pupilsightSchoolYearID,
                    ...
                    if ($checkmode == "multiple") {
                        ...
                        for ($i = 0; $i < $l; $i++) {
                            $datam = [
                                "transaction_id" => $t_id,
                                "payment_mode_id" => $pmode[$i],
                                "bank_id" => $bank_id[$i],
                                "amount" => $amount[$i],
                                "reference_no" => $mrefno[$i],
                                "instrument_date" => $minstruDate[$i],
                            ];
                            $sqlm =
                                "INSERT INTO fn_multi_payment_mode SET transaction_id=:transaction_id, payment_mode_id=:payment_mode_id, bank_id=:bank_id,amount=:amount,reference_no=:reference_no,instrument_date=:instrument_date";
                            ...
                            if (!empty($bank_id[$i])) {
                                $sqlbn =
                                    "SELECT name FROM fn_masters WHERE id = " .
                                    $bank_id[$i] .
                                    " ";
                                $resultbn = $connection2->query($sqlbn);
                                $bankNameData = $resultbn->fetch();
                        ...
                        $pmId = implode(",", $pmode);
                        $sqlpt =
                            "SELECT GROUP_CONCAT(name) AS modeName FROM fn_masters WHERE id IN (" .
                            $pmId .
                            ") ";
                        $resultpt = $connection2->query($sqlpt);
                        $valuept = $resultpt->fetch();
                        $paymentModeName =
                            "Multiple (" . $valuept["modeName"] . ")";
                    } else {
                        $sqlpt =
                            "SELECT name FROM fn_masters WHERE id = " .
                            $payment_mode_id .
                    ...
                    $chkcussql =
                        'SELECT field_name FROM custom_field WHERE field_name = "correspondence_address" ';
                    ...
                    $sqlstu =
                        "SELECT a.officialName , a.admission_no, a.roll_no, sc.name as academic_year, p.name as progname, b.name as class, c.name as section " .
                        $fieldName .
                        " FROM pupilsightPerson AS a LEFT JOIN pupilsightStudentEnrolment AS d ON a.pupilsightPersonID = d.pupilsightPersonID LEFT JOIN pupilsightSchoolYear AS sc ON d.pupilsightSchoolYearID = sc.pupilsightSchoolYearID LEFT JOIN pupilsightProgram AS p ON d.pupilsightProgramID = p.pupilsightProgramID LEFT JOIN pupilsightYearGroup AS b ON d.pupilsightYearGroupID = b.pupilsightYearGroupID LEFT JOIN pupilsightRollGroup AS c ON d.pupilsightRollGroupID = c.pupilsightRollGroupID WHERE a.pupilsightPersonID = " .
                        $pupilsightPersonID .
                        " ";
                    $resultstu = $connection2->query($sqlstu);
                    $valuestu = $resultstu->fetch();
                    ...
                    $sqlfat =
                        "SELECT b.officialName , b.phone1, b.email FROM pupilsightFamilyRelationship AS a LEFT JOIN pupilsightPerson AS b ON a.pupilsightPersonID1 = b.pupilsightPersonID WHERE a.pupilsightPersonID2 = " .
                        $pupilsightPersonID .
                        " AND a.relationship = 'Father' ";
                    ...
                    $due_date = "";
                    if (
                        !empty($valueinv["due_date"]) &&
                        $valueinv["due_date"] != "1970-01-01"
                    ) {
                        $due_date = date(
                            "d/m/Y",
                            strtotime($valueinv["due_date"])
                        );
                    }

                    $class_section =
                        $valuestu["class"] . " " . $valuestu["section"];
                    $payment_receipt_date = date(
                        "d-m-Y",
                        strtotime($payment_date)
                    );
                    if (!empty($instrument_date)) {
                        $instrument_receipt_date = date(
                            "d-m-Y",
                            strtotime($instrument_date)
                        );
                    } else {
                    ...
                    if (!empty($invoice_id)) {
                        ...
                        $sqlconInv =
                            'SELECT GROUP_CONCAT(title SEPARATOR " - ") AS invtitle FROM fn_fee_invoice WHERE id IN (' .
                            $idsInv .
                            ")";
                        ...
                        foreach ($invid as $iid) {
                            $datau = [
                                "invoice_status" => $invoice_status,
                                "fn_fees_invoice_id" => $iid,
                                "pupilsightPersonID" => $pupilsightPersonID,
                            ];
                            $sqlu =
                                "UPDATE fn_fees_collection SET invoice_status=:invoice_status WHERE fn_fees_invoice_id=:fn_fees_invoice_id AND pupilsightPersonID=:pupilsightPersonID";
                            $resultu = $connection2->prepare($sqlu);
                            $resultu->execute($datau);
                            ...
                            $dataiu = [
                                "invoice_status" => $invoice_status,
                                "pupilsightPersonID" => $pupilsightPersonID,
                                "fn_fee_invoice_id" => $iid,
                            ];
                            $sqliu =
                                "UPDATE fn_fee_invoice_student_assign SET invoice_status=:invoice_status WHERE pupilsightPersonID=:pupilsightPersonID AND fn_fee_invoice_id=:fn_fee_invoice_id";
                            $resultiu = $connection2->prepare($sqliu);
                            $resultiu->execute($dataiu);

                            $chksql =
                                "SELECT fn_fee_structure_id, display_fee_item, title as invoice_title, is_concat_invoice FROM fn_fee_invoice WHERE id = " .
                                $iid .
                                " ";
                            ...
                            if ($is_concat_invoice == "1") {
                                ...
                                if ($valuech["display_fee_item"] == "2") {
                                    $sqcs =
                                        "select SUM(fi.total_amount) AS tamnt, SUM(fi.amount) AS amnt, SUM(fi.tax) AS ttax from fn_fee_invoice_item as fi, fn_fee_items as items where fi.fn_fee_item_id = items.id and fi.fn_fee_invoice_id =  " .
                                        $iid .
                                        " ";
                                    $resultfi = $connection2->query($sqcs);
                                    $valuefi = $resultfi->fetchAll();
                                    if (!empty($valuefi)) {
                                        ...
                                        foreach ($valuefi as $vfi) {
                                            $sqcol =
                                                "SELECT SUM(total_amount) AS tamntCol , SUM(discount) AS disCol, SUM(total_amount_collection) AS ttamtCol FROM fn_fees_student_collection WHERE fn_fees_invoice_id = " .
                                                $iid .
                                                " AND ( transaction_id = " .
                                                $transactionId .
                                                " OR partial_transaction_id = " .
                                                $transactionId .
                                                " )  ";
                                            $resultcol = $connection2->query(
                                                $sqcol
                                            );
                                            ...
                                            $sqitid =
                                                "SELECT GROUP_CONCAT(id) AS itmIds FROM fn_fee_invoice_item WHERE fn_fee_invoice_id = " .
                                                $iid .
                                                " ";
                                            $resultitid = $connection2->query(
                                                $sqitid
                                            );
                                            ...
                                            $sqdis =
                                                "SELECT SUM(discount) AS dis FROM fn_fee_item_level_discount WHERE pupilsightPersonID = " .
                                                $pupilsightPersonID .
                                                " AND item_id IN (" .
                                                $itmIDS .
                                                ") ";
                                            $resultdis = $connection2->query(
                                                $sqdis
                                            );
                                            $valuedis = $resultdis->fetch();
                                            $disItemAmt = 0;
                                            if (!empty($valuedis)) {
                                                ...
                                                $newItemAmtCol =
                                                    $itemAmtCol + $disItemAmt;
                                                $itemAmtPen =
                                                    $itemAmt - $newItemAmtCol;
                                            } else {
                                                $disItemAmt = 0;
                                                $itemAmtPen =
                                                    $itemAmt - $itemAmtCol;
                                            ...
                                            $dts_receipt_feeitem[] = [
                                                "serial.all" => $cnt,
                                                "particulars.all" => htmlspecialchars(
                                                    trim(
                                                        $valuech[
                                                            "invoice_title"
                                                        ]
                                    ...
                                    $sqcs =
                                        "select fi.total_amount, fi.amount, fi.tax, fi.id, items.name from fn_fee_invoice_item as fi, fn_fee_items as items where fi.fn_fee_item_id = items.id and fi.fn_fee_invoice_id =  " .
                                        $iid .
                                        " and fi.id in(" .
                                        $invoice_item_id .
                                        ")  ";
                                    ...
                                    if (!empty($valuefi)) {
                                        ...
                                        foreach ($valuefi as $vfi) {
                                            $sqcol =
                                                "SELECT * FROM fn_fees_student_collection WHERE fn_fees_invoice_id = " .
                                                $iid .
                                                " AND fn_fee_invoice_item_id =  " .
                                                $vfi["id"] .
                                                " AND ( transaction_id = " .
                                                $transactionId .
                                                " OR partial_transaction_id = " .
                    ...
                    if (!empty($concatInvId)) {
                        ...
                        $sqcs =
                            "select SUM(fi.total_amount) AS tamnt, SUM(fi.amount) AS amnt, SUM(fi.tax) AS ttax from fn_fee_invoice_item as fi, fn_fee_items as items where fi.fn_fee_item_id = items.id and fi.fn_fee_invoice_id IN  (" .
                            $invconids .
                            ") ";
                        $resultfi = $connection2->query($sqcs);
                    ...
                    $dts_receipt = [
                        ...
                        "instrument_no" => $instrument_no,
                        "transcation_amount" => number_format(
                            $amount_paying,
                            2,
                            ".",
                            ""
                        ...
                        "inv_total" => number_format(
                            $totalamtWitoutTaxDis,
                            2,
                            ".",
                            ""
                        ),
                        "concat_invoice_title" => htmlspecialchars(
                            $concatInvoiceTitle
                        ),
                        "total_amount_discount" => number_format(
                            $discount,
                            2,
                            ".",
                            ""
                        ),
                        "total_amount_pending" => number_format(
                            $totalPending,
                            2,
                            ".",
                            ""
                    ...
                    if (
                        !empty($dts_receipt) &&
                        !empty($dts_receipt_feeitem) &&
                        !empty($receiptTemplate)
                    ) {
                        $callback =
                            $_SESSION[$guid]["absoluteURL"] .
                            "/thirdparty/phpword/receiptNew.php";
        ...
        case "loadInvoicesCollections":
            ...
            foreach ($invdata as $k => $d) {
                $sqlsd =
                    "SELECT b.name FROM fn_fee_invoice_item AS a LEFT JOIN fn_fee_items AS b ON a.fn_fee_item_id = b.id WHERE a.fn_fee_invoice_id = " .
                    $d["invoiceid"] .
                    ' AND b.name = "Staff Discount"  ';
                ...
                $sqlamt =
                    "SELECT SUM(fn_fee_invoice_item.total_amount) as totalamount, SUM(fn_fee_invoice_item.discount) as disamount FROM fn_fee_invoice_item WHERE fn_fee_invoice_id = " .
                    $d["invoiceid"] .
                    " ";
                $resultamt = $connection2->query($sqlamt);
                $dataamt = $resultamt->fetch();
                $sql_dis =
                    "SELECT discount FROM fn_invoice_level_discount WHERE pupilsightPersonID = " .
                    $stuId .
                    "  AND invoice_id='" .
                    $d["invoiceid"] .
                    "' ";
                ...
                $sp_item_sql =
                    "SELECT SUM(discount.discount) as sp_discount
                FROM fn_fee_invoice_item as fee_item
                LEFT JOIN fn_fee_item_level_discount as discount
                ON fee_item.id = discount.item_id WHERE fee_item.fn_fee_invoice_id= " .
                    $d["invoiceid"] .
                    " AND pupilsightPersonID = " .
                    $stuId .
                    "  ";
                ...
                if (
                    !empty($d["transport_schedule_id"]) &&
                    $d["transport_schedule_id"] != ""
                ) {
                    ...
                    if (!empty($routes)) {
                        ...
                        foreach ($routes as $rt) {
                            if (!empty($rt)) {
                                $sqlsc =
                                    "SELECT * FROM trans_route_price WHERE schedule_id = " .
                                    $d["transport_schedule_id"] .
                                    " AND route_id = " .
                                    $rt .
                                    " ";
                ...
                if (
                    !empty($special_dis["discount"]) ||
                    !empty($sp_item_dis["sp_discount"])
                ) {
                    $invdata[$k]["finalamount_with_des"] =
                        $totalamount -
                        $special_dis["discount"] -
                        $sp_item_dis["sp_discount"];
                    $totalamount =
                        $totalamount -
                        $special_dis["discount"] -
                        $sp_item_dis["sp_discount"];
                    $dis_item_inv =
                        $special_dis["discount"] + $sp_item_dis["sp_discount"];
                } else {
                ...
                if (!empty($d["fn_fees_discount_id"])) {
                    $std_query =
                        "SELECT fee_category_id FROM `pupilsightPerson` WHERE `pupilsightPersonID` = " .
                        $stuId .
                        " ";
                    ...
                    $dissql =
                        "SELECT * FROM fn_fee_discount_item WHERE fn_fees_discount_id = " .
                        $d["fn_fees_discount_id"] .
                        " AND name = " .
                        $fee_category_id .
                        " ";
                    ...
                    if (!empty($disamtdata)) {
                        if ($disamtdata["item_type"] == "Fixed") {
                            $totalamount =
                                $totalamount - $disamtdata["amount_in_number"];
                            $invdata[$k]["finalamount"] = $totalamount;
                        } else {
                            $totalamount =
                                ($totalamount / 100) *
                                $disamtdata["amount_in_percent"];
                            $invdata[$k]["finalamount"] = $totalamount;
                        }
                    }
                }
                ...
                if (!empty($fineId) && $curdate > $duedate) {
                    $sqlschday =
                        "SELECT GROUP_CONCAT(pupilsightDaysOfWeekID) as daysid, GROUP_CONCAT(name) as weekend FROM pupilsightDaysOfWeek WHERE schoolDay = 'N' ";
                    ...
                    if ($finetype == "1" && $ruletype == "1") {
                        $sqlf =
                            'SELECT * FROM fn_fees_rule_type WHERE fn_fees_fine_rule_id = "' .
                            $fineId .
                            '" AND fine_type = "' .
                            $finetype .
                            '" AND rule_type = "' .
                        ...
                        if ($d["due_date"] != "1970-01-01") {
                            ...
                            if (!empty($nday)) {
                                $amtper =
                                    $finedata["amount_in_percent"] * $nday;
                            } else {
                                $amtper = $finedata["amount_in_percent"];
                            }
                            $type = "percent";
                ...
                $sqla =
                    'SELECT GROUP_CONCAT(a.fn_fee_invoice_item_id) AS invitemid, b.invoice_status, b.transaction_id FROM fn_fees_student_collection AS a LEFT JOIN fn_fees_collection AS b ON  a.transaction_id = b.transaction_id WHERE a.invoice_no = "' .
                    $invno .
                    '" AND b.transaction_status IN (1,3) ';
                ...
                $sqlchkInv =
                    'SELECT count(b.id) as kount FROM fn_fees_student_collection AS a LEFT JOIN fn_fees_collection AS b ON  a.transaction_id = b.transaction_id WHERE a.invoice_no = "' .
                    $invno .
                    '" AND b.invoice_status = "Fully Paid" AND b.transaction_status IN (1,3) ';
                ...
                if (!empty($invChk) && $invChk["kount"] >= 1) {
                    ...
                    if (!empty($inv["invitemid"])) {
                        ...
                        if (!empty($d["transport_schedule_id"])) {
                            ...
                            $sqlp =
                                "SELECT SUM(total_amount_collection) as paidtotalamount FROM fn_fees_student_collection WHERE pupilsightPersonID = " .
                                $stuId .
                                " AND transaction_id = " .
                                $stTransId .
                                " AND fn_fee_invoice_item_id IN (" .
                                $itemids .
                                ") ";
            ...
            if (!empty($invdata)) {
                foreach ($invdata as $ind) {
                    ...
                    $totAmt_with_dis = number_format(
                        $ind["finalamount_with_des"],
                        2,
                        ".",
                        ""
                    );
                    $totAmtdisAmt = number_format(
                        $ind["totamtdisamount"],
                        2,
                        ".",
                        ""
                    ...
                    $sqlp =
                        'SELECT id FROM fn_fee_waive_off WHERE invoice_no = "' .
                        $ind["stu_invoice_no"] .
                        '" ';
                    $resultp = $connection2->query($sqlp);
                    ...
                    if ($ind["chkpayment"] == "Paid") {
                        ...
                        echo '<tr><td><input type="checkbox" class=" invoice' .
                            $ind["id"] .
                            '" name="invoiceid[]" data-h="' .
                            $ind["fn_fees_head_id"] .
                            '" data-se="' .
                            $ind["rec_fn_fee_series_id"] .
                            '" id="allfeeItemid" data-stu="' .
                            $stuId .
                            '" data-fper="' .
                            $ind["amtper"] .
                            '" data-ftype="' .
                            ...
                            $ind["invid"] .
                            '" data-ife="' .
                            $ind["is_fine_editable"] .
                            '" value="0" checked disabled ></td><td>' .
                            $ind["stu_invoice_no"] .
                            "</td><td>" .
                            $ind["title"] .
                            "</td><td>" .
        ...
        case "update_template_for_receipt":
            if (!empty($_FILES["file_upload"]["name"])) {
                ...
                $r_name =
                    "thirdparty/phpword/templates/updated_at/" .
                    date("Y_m_d") .
                    "_" .
                    $old_file .
                    ".docx";
        ...
        case "get_dicount_type_change":
            ...
            if (!empty($d_type)) {
                if ($d_type == "1") {
                    ...
                    $invoices =
                        ...
                        '" AND pupilsightPerson.pupilsightPersonID = "' .
                        $stuId .
                        '" AND  fn_fee_invoice.id IN(' .
                        $ids .
                        ") GROUP BY fn_fee_invoice.id";
                    $resultinv = $connection2->query($invoices);
                    $invdata = $resultinv->fetchAll();
                    $totalamount = 0;
                    foreach ($invdata as $k => $d) {
                        $sqlamt =
                            "SELECT SUM(fn_fee_invoice_item.amount) as tot_amount, SUM(fn_fee_invoice_item.total_amount) as totalamount, SUM(fn_fee_invoice_item.discount) as disamount FROM fn_fee_invoice_item WHERE fn_fee_invoice_id = " .
                            $d["invoiceid"] .
                            " ";
                        $resultamt = $connection2->query($sqlamt);
                        $dataamt = $resultamt->fetch();
                        ...
                        $invno = $d["stu_invoice_no"];
                        $sqla =
                            'SELECT GROUP_CONCAT(a.fn_fee_invoice_item_id) AS invitemid, b.transaction_id FROM fn_fees_student_collection AS a LEFT JOIN fn_fees_collection AS b ON  a.transaction_id = b.transaction_id WHERE a.invoice_no = "' .
                            $invno .
                            '" AND b.transaction_status = "1" ';
                        $resulta = $connection2->query($sqla);
                        $inv = $resulta->fetch();
                    ...
                    $sqlstaff =
                        "SELECT b.officialName , b.pupilsightPersonID FROM pupilsightStaff AS a LEFT JOIN pupilsightPerson AS b ON a.pupilsightPersonID = b.pupilsightPersonID";
                    ...
                    <table class="table" cellspacing="0" style="width: 100%;">
                        ...
                        <td  width="5%">' .
                                        $pending .
                                        '</td>
                        <td  width="5%"><input type="text" name="discount_a[]" value="' .
                                        $special_dis["discount"] .
                                        '" readonly class="form-control inid_' .
                                        $ind["invoiceid"] .
                                        '" ></td>
                        <td  width="1%"><input type="checkbox"  class="chkinvoice_discount invoice' .
                                        $ind["invoiceid"] .
                        ...
                        <td class="waiveClass" style="display:none" width="10%">
                            <select name="assigned_by[]" class="form-control assn_' .
                                        $ind["invoiceid"] .
                                        '">
                        ...
                        <td class="waiveClass" style="display:none" width="5%"><input type="date" name="date[]" value=""  class="form-control dte_' .
                                        $ind["invoiceid"] .
                                        '" ></td>
                        <td class="waiveClass" style="display:none" width="5%"><input type="text" name="remark[]" value=""  class="form-control rem_' .
                                        $ind["invoiceid"] .
                                        '" ><input type="hidden" name="invoice_no[]" value="' .
                                        $ind["stu_invoice_no"] .
                                        '"  class="form-control invno_' .
                                        $ind["invoiceid"] .
                                        '" ></td>
                        </tr>';
                                }
                ...
                <?php
                    ...
                    <table class="table" cellspacing="0" style="width: 100%;">
                        ...
                        <tbody>
                            ...
                            $fee_category_id = $std_data["fee_category_id"];
                            $sqli =
                                "SELECT e.pupilsightPersonID,a.*,a.id as itemid, b.*, b.id as ifid, b.name as feeitemname, c.id AS invoiceid, c.transport_schedule_id, d.format, e.invoice_no as stu_invoice_no, f.item_type, f.name, f.min_invoice, f.max_invoice, f.amount_in_percent, f.amount_in_number, GROUP_CONCAT(DISTINCT asg.route_id) as routes, GROUP_CONCAT(DISTINCT asg.transport_type) as routetype  FROM fn_fee_invoice_item AS a LEFT JOIN fn_fee_items AS b ON a.fn_fee_item_id = b.id LEFT JOIN fn_fee_invoice AS c ON a.fn_fee_invoice_id = c.id LEFT JOIN fn_fee_series AS d ON c.inv_fn_fee_series_id = d.id LEFT JOIN fn_fee_invoice_student_assign AS e ON c.id = e.fn_fee_invoice_id  LEFT JOIN fn_fee_discount_item as f ON c.fn_fees_discount_id = f.fn_fees_discount_id LEFT JOIN trans_route_assign AS asg ON e.pupilsightPersonID = asg.pupilsightPersonID WHERE a.fn_fee_invoice_id IN (" .
                                $ids .
                                ") AND e.pupilsightPersonID = " .
                                $stuId .
                                " GROUP BY a.id";
                            ...
                            foreach ($feeItem as $fI) {
                                ...
                                $discount = 0;
                                $sql_dis =
                                    "SELECT SUM(discount) AS discount FROM fn_fee_item_level_discount WHERE pupilsightPersonID = " .
                                    $stuId .
                                    "  AND item_id='" .
                                    $fI["itemid"] .
                                    "' ";
                                ...
                                $sqlchk =
                                    "SELECT COUNT(a.id) as kount FROM fn_fees_student_collection AS a LEFT JOIN fn_fees_collection AS b ON a.transaction_id = b.transaction_id WHERE a.fn_fee_invoice_item_id = " .
                                    $fI["itemid"] .
                                    " AND a.pupilsightPersonID = " .
                                    $stuId .
                                    ' AND b.transaction_status = "1" ';
                                ...
                                if ($fee_category_id == $fI["name"]) {
                                    if ($fI["item_type"] == "Fixed") {
                                        ...
                                        $discount =
                                            $fI["amount_in_percent"] . "%";
                                        $discountamt =
                                            ($fI["amount_in_percent"] / 100) *
                                            $totalamount;
                                    }
                                }
            ...
            <td width="10%">
               <input type="number" class="form-control itid_' .
                                    $fI["itemid"] .
                                    '" value="' .
                                    $special_dis["discount"] .
                                    '" readonly>
            </td>
             <td width="10%"><label class="leading-normal" for="feeItemid"></label> <input type="checkbox" class="' .
                                    $cls .
                                    '" id="feeItemid" data-id="' .
                                    $fI["itemid"] .
                                    '" value="' .
                                    $fI["itemid"] .
                                    '" ' .
        ...
        case "invoice_level_dataStore":
            ...
            if (!empty($stid) && !empty($invids)) {
                ...
                for ($i = 0; $i < $count; $i++) {
                    $sqlpt =
                        "SELECT id FROM fn_invoice_level_discount WHERE pupilsightPersonID = " .
                        $stid .
                        "  AND invoice_id='" .
                        $invids[$i] .
                        "' ";
                    ...
                    if (empty($valuept["id"])) {
                        $datau = [
                            "pupilsightPersonID" => $stid,
                            "invoice_id" => $invids[$i],
                            "discount" => $discountVal[$i],
                        ];
                        $sql =
                            "INSERT INTO fn_invoice_level_discount SET pupilsightPersonID=:pupilsightPersonID, invoice_id=:invoice_id, discount=:discount";
                        $result = $connection2->prepare($sql);
                        ...
                        $sqlu =
                            "UPDATE fn_invoice_level_discount SET pupilsightPersonID=:pupilsightPersonID,discount=:discount WHERE invoice_id=:invoice_id";
                        $resultu = $connection2->prepare($sqlu);
                        $resultu->execute($datau);
                    }
                }
        ...
        case "fee_item_level_dataStore":
            ...
            if (!empty($stid) && !empty($items)) {
                ...
                for ($i = 0; $i < $count; $i++) {
                    $sqlpt =
                        "SELECT id FROM fn_fee_item_level_discount WHERE pupilsightPersonID = " .
                        $stid .
                        "  AND item_id='" .
                        $items[$i] .
                        "' ";
                    ...
                    if (empty($valuept["id"])) {
                        ...
                        $sql =
                            "INSERT INTO fn_fee_item_level_discount SET pupilsightPersonID=:pupilsightPersonID, item_id=:item_id, discount=:discount";
                        $result = $connection2->prepare($sql);
                        $result->execute($datau);
                    } else {
                        $datau = [
                        ...
                        $sqlu =
                            "UPDATE fn_fee_item_level_discount SET pupilsightPersonID=:pupilsightPersonID,discount=:discount WHERE item_id=:item_id";
                        $resultu = $connection2->prepare($sqlu);
                        $resultu->execute($datau);
                    }
                }
        ...
        case "loadYearByTests":
            $pupilsightSchoolYearID = $_POST["pupilsightSchoolYearID"];
            $test_sql =
                'SELECT  examinationTestMaster.id,examinationTestMaster.name FROM `examinationTestMaster` LEFT JOIN `pupilsightSchoolYear` ON `examinationTestMaster`.`pupilsightSchoolYearID`=`pupilsightSchoolYear`.`pupilsightSchoolYearID` WHERE `examinationTestMaster`.`pupilsightSchoolYearID` = "' .
                $pupilsightSchoolYearID .
                '" ORDER BY examinationTestMaster.name ASC';
        ...
        case "loadTestByProgram":
            ...
            $test_sql =
                'SELECT p.pupilsightProgramID,p.name
            ...
            WHERE examTAC.test_master_id = "' .
                $test_id .
                '" GROUP BY examTAC.pupilsightProgramID';
            $test_res = $connection2->query($test_sql);
            $tests = $test_res->fetchAll();
            $options = '<option value="">Select program</option>';
            foreach ($tests as $val) {
        ...
        case "loadClassesByTest":
            ...
            $test_sql =
                'SELECT g.pupilsightYearGroupID,g.name
            FROM pupilsightYearGroup as g 
            LEFT JOIN examinationTestAssignClass as examTAC
            ON g.pupilsightYearGroupID = examTAC.pupilsightYearGroupID 
            WHERE examTAC.test_master_id ="' .
                $test_id .
                '" AND examTAC.pupilsightProgramID="' .
                $pupilsightProgramID .
                '"  GROUP BY examTAC.pupilsightYearGroupID ORDER bY g.name ASC';
            $test_res = $connection2->query($test_sql);
            $tests = $test_res->fetchAll();
        ...
        case "getSectionM":
            ...
            $sql =
                'SELECT a.*, b.name FROM pupilsightProgramClassSectionMapping AS a LEFT JOIN pupilsightRollGroup AS b ON a.pupilsightRollGroupID = b.pupilsightRollGroupID WHERE a.pupilsightProgramID = "' .
                $pid .
                '" AND a.pupilsightYearGroupID IN(' .
                $cid .
        ...
        case "getSubjectbasedonclassByM":
            $pupilsightYearGroupID = implode(
                ",",
                $_POST["pupilsightYearGroupID"]
            );
            $sq =
                "select pupilsightDepartmentID, subject_display_name, di_mode from subjectToClassCurriculum where pupilsightYearGroupID IN(" .
                $pupilsightYearGroupID .
                ")  GROUP BY subject_display_name order by subject_display_name asc";
        ...
        case "load_remarks_descriptive_indicator_config":
            ...
            $prg = $_POST["program"];
            $sqlr =
                "SELECT * FROM subject_skill_descriptive_indicator_config WHERE pupilsightDepartmentID = " .
                $sub_id .
                " AND pupilsightYearGroupID = " .
                $cls .
                " AND pupilsightProgramID = " .
                $prg .
                " ";
            ...
            foreach ($rowdatar as $row) {
                echo '<tr>
        <td>
        <div class="dte mb-1"></div><div class="  txtfield mb-1"><div class="flex-1 relative"><input type="checkbox" class="selectGrdCheck" name="Selectone_byOne[]" id="' .
                    $row["id"] .
                    '" value="' .
                    $row["id"] .
                    '" ></div></div></td>
        ...
        <div class="input-group stylish-input-group">
            <div class="dte mb-1"></div><div class="  txtfield mb-1"><div class="flex-1 relative"> <input type="text" name="remarksname" class="remarks_id" value="' .
                    $row["remark_description"] .
                    '" style="    width: 500px;" ></div></div>
        </div></td></tr>';
            }
            break;
         ...
         <td>
            ...
            <td>
                <div class="input-group stylish-input-group w-full">
                    <div class="dte mb-1"></div><div class="  txtfield mb-1"><div class="flex-1 relative">
                    <input type="text" name="remarksname" class="remarks_id" value="' .
                    $row["remark_description"] .
                    '" style="width: 500px;" >
                    </div></div>
                </div>
        ...
        case "load_test_groups":
            ...
            if (!empty($_POST["pupilsightRollGroupID"])) {
                $pupilsightRollGroupID = implode(
                    ",",
                    $_POST["pupilsightRollGroupID"]
                );
                $pupilsightProgramID = $_POST["pupilsightProgramID"];
                $pupilsightSchoolYearID =
                ...
                $sql =
                    'SELECT  examinationTest.* FROM `examinationTest` LEFT JOIN `examinationTestAssignClass` ON `examinationTest`.`id`=`examinationTestAssignClass`.`test_id` LEFT JOIN `pupilsightSchoolYear` ON `examinationTest`.`pupilsightSchoolYearID`=`pupilsightSchoolYear`.`pupilsightSchoolYearID` WHERE `examinationTest`.`pupilsightSchoolYearID` = "' .
                    $pupilsightSchoolYearID .
                    '" AND `examinationTestAssignClass`.`pupilsightProgramID` = "' .
                    $pupilsightProgramID .
                    '" AND `examinationTestAssignClass`.`pupilsightYearGroupID` = "' .
                    $pupilsightYearGroupID .
                    '" AND `examinationTestAssignClass`.`pupilsightRollGroupID` IN (' .
                    $pupilsightRollGroupID .
                    ")  ORDER BY `examinationTest`.`id` DESC";
                $result = $connection2->query($sql);
                ...
                if (!empty($test)) {
                    foreach ($test as $cl) {
                        $data .=
                            '<tr><td>
                <input class="slt_test" type ="checkbox" name="testID[]" value="' .
                            $cl["id"] .
                            '">' .
                            " </td>
                <td>" .
        ...
        case "load_tests_subjects":
            ...
            if (!empty($_POST["testID"])) {
                ...
                $sqls =
                    "SELECT a.id,a.test_id,a.pupilsightDepartmentID,a.skill_id,i.subject_display_name as subname,j.name as skill FROM examinationSubjectToTest AS a LEFT JOIN subjectToClassCurriculum as i ON a.pupilsightDepartmentID =i.pupilsightDepartmentID LEFT JOIN ac_manage_skill as j ON a.skill_id = j.id WHERE a.test_id IN(" .
                    $testID .
                    ') AND a.is_tested = "1"  GROUP BY a.id  ORDER BY i.pos ASC  ';
                ...
                if (!empty($rowdatas)) {
                    foreach ($rowdatas as $cl) {
                        $data .=
                            '<tr><td>
                            <input class="slt_test" type ="checkbox" name="subjectSkillId[]" value="' .
                            $cl["test_id"] .
                            "-" .
                            $cl["pupilsightDepartmentID"] .
                            "-" .
                            $cl["skill_id"] .
                            '">' .
                            " </td>
        ...
        case "studentMarks_excel":
            ...
            $testId = implode(",", $_POST["testId"]);
            $sql =
                "SELECT b.officialName, b.pupilsightPersonID, d.name as classname, e.name as sectionname FROM `examinationMarksEntrybySubject` AS a LEFT JOIN pupilsightPerson AS b ON a.pupilsightPersonID = b.pupilsightPersonID LEFT JOIN pupilsightStudentEnrolment AS c ON a.pupilsightPersonID = c.pupilsightPersonID LEFT JOIN pupilsightYearGroup AS d ON c.pupilsightYearGroupID = d.pupilsightYearGroupID LEFT JOIN pupilsightRollGroup AS e ON c.pupilsightRollGroupID = e.pupilsightRollGroupID WHERE   a.test_id = " .
                $testId .
                " GROUP BY a.pupilsightPersonID";
            $result = $connection2->query($sql);
            $data = $result->fetchAll();
            foreach ($data as $k => $dt) {
                $sqlm =
                    'SELECT a.*, b.name as subject_name, c.skill_display_name,m.max_marks FROM examinationMarksEntrybySubject AS a LEFT JOIN pupilsightDepartment AS b ON a.pupilsightDepartmentID = b.pupilsightDepartmentID LEFT JOIN subjectSkillMapping AS c ON a.skill_id = c.skill_id
                    LEFT JOIN examinationSubjectToTest AS m ON a.pupilsightDepartmentID = m.pupilsightDepartmentID
                    WHERE a.test_id = ' .
                    $testId .
                    " AND a.pupilsightPersonID = " .
                    $dt["pupilsightPersonID"] .
                    " GROUP by a.pupilsightDepartmentID,c.skill_id";
            ...
            ?>
                    <table id="excelexport">
                        <tr>
                            ...
                            <th>Class</th>
                            <th>Section</th>
                            <?php foreach ($datam_h as $m) { ?>
                                <th><?php echo $m["subject_name"] .
                                    "-" .
                                    $m["skill_display_name"] .
                                    "/" .
                                    ceil($m["max_marks"]); ?></th>
                            <?php } ?>
                        </tr>
    ...
    <td>" .
                            ...
                            foreach ($data as $val) {
                                ...
                                foreach ($marks as $m) {
                                    $sql =
                                        'SELECT grade_name FROM  examinationGradeSystemConfiguration WHERE id="' .
                                        $m["gradeId"] .
                                        '"';
                                    $result = $connection2->query($sql);
    ...
    <td>" .
        ...
        case "load_Student_data":
            ...
            $sql =
                'SELECT a.*, b.officialName FROM  pupilsightStudentEnrolment AS a LEFT JOIN pupilsightPerson AS b ON a.pupilsightPersonID = b.pupilsightPersonID WHERE a.pupilsightSchoolYearID = "' .
                $pupilsightSchoolYearID .
                '" AND a.pupilsightProgramID = "' .
                $pupilsightProgramID .
                '" AND a.pupilsightYearGroupID = "' .
                $pupilsightYearGroupID .
                '" AND a.pupilsightRollGroupID = "' .
                $cid .
                '" AND pupilsightRoleIDPrimary=003 GROUP BY b.pupilsightPersonID';
            $result = $connection2->query($sql);
            $sections = $result->fetchAll();
