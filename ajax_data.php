...
if ($type == 'updateApplicantData') {
    ...
    $sqlet = "SELECT b.* FROM campaign AS a LEFT JOIN pupilsightTemplate AS b ON a.email_template_id = b.pupilsightTemplateID WHERE a.id = " . $campaignid . " ";
    $resultet = $connection2->query($sqlet);
    $rowdataet = $resultet->fetch();

    if(!empty($rowdataet)){
        $emailSubjct_camp = $rowdataet['subject'];
        $emailquote = $rowdataet['description'];
    } else {
        $emailSubjct_camp = 'Application Status';
        $emailquote = 'Your Application Submitted Successfully';
    }

    $sqlst = "SELECT b.* FROM campaign AS a LEFT JOIN pupilsightTemplate AS b ON a.sms_template_id = b.pupilsightTemplateID WHERE a.id = " . $campaignid . " ";
    $resultst = $connection2->query($sqlst);
    $rowdatast = $resultst->fetch();

    if(!empty($rowdatast)){
        $smsquote = $rowdatast['description'];
    } else {
