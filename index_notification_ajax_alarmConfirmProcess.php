...
//Proceed!
if ($pupilsightAlarmID == '' or $pupilsightPersonID == '') {
    header("Location: {$URL}");
} else {
    //Check alarm
    try {
    ...
    if ($result->rowCount() == 1) {
        ...
        try {
            $dataConfirm = array('pupilsightAlarmID' => $pupilsightAlarmID, 'pupilsightPersonID' => $pupilsightPersonID);
        ...
        if ($resultConfirm->rowCount() == 0) {
            //Insert confirmation
            try {
                $dataConfirm = array('pupilsightAlarmID' => $pupilsightAlarmID, 'pupilsightPersonID' => $pupilsightPersonID, 'timestamp' => date('Y-m-d H:i:s'));
