...
class FileUploader
{
    ...
    /**
     * List of allowed file types from pupilsightFileExtension table
     * @var  array
     */
    ...
    public function getFileExtensions($type = '')
    {
        if (!isset($this->fileExtensions) || !empty($type)) {
            ...
            if (!empty($type)) {
                ...
                $sql = "SELECT LOWER(extension) FROM pupilsightFileExtension WHERE FIND_IN_SET(LOWER(type), :types) ORDER BY type, name";
            } else {
                $data = array();
                $sql = "SELECT LOWER(extension) FROM pupilsightFileExtension ORDER BY type, name";
            }
