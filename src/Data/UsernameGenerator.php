...
class UsernameGenerator
{
    ...
    /**
     * Generates a username based on the provided pupilsightRoleID, using the 'isDefault' format if none exists for that role.
     * @version  v15
     * @since    v15
     * @param    string|int  $pupilsightRoleID
     * @return   string  Unique username
     */
    public function generateByRole($pupilsightRoleID)
    {
        $usernameFormat = '';

        // Get the username format data by pupilsightRoleID
        $data = array('pupilsightRoleID' => $pupilsightRoleID);
        $sql = "SELECT pupilsightUsernameFormat.*, pupilsightRole.name as roleName FROM pupilsightUsernameFormat JOIN pupilsightRole ON (FIND_IN_SET(pupilsightRole.pupilsightRoleID, pupilsightRoleIDList)) WHERE pupilsightRoleID=:pupilsightRoleID OR isDefault='Y' ORDER BY FIND_IN_SET(:pupilsightRoleID, pupilsightRoleIDList) DESC LIMIT 1";
        ...
        if ($result->rowCount() > 0) {
            ...
            if ($row['isNumeric'] == 'Y') {
                ...
                $callback = function($number) use (&$pdo, &$row) {
                    $data = array('pupilsightUsernameFormatID' => $row['pupilsightUsernameFormatID'], 'numericValue' => $number);
                    $sql = "UPDATE pupilsightUsernameFormat SET numericValue=:numericValue WHERE pupilsightUsernameFormatID=:pupilsightUsernameFormatID";
    ...
    public function isUsernameUnique($username)
    {
        ...
        $sql = "SELECT pupilsightPersonID from pupilsightPerson WHERE username=:username OR username=LOWER(:username)";
        $result = $this->pdo->executeQuery($data, $sql);
