...
class ImportType
{
    ...
    public static function getBaseDir(Connection $pdo)
    {
        ...
        $absolutePath = 'C:/xampp/htdocs/pupilsight';
    ...
    protected function validateWithDatabase(Connection $pdo)
    {
        ...
        foreach ($this->table as $fieldName => $field) {
            ...
            if (isset($columns[$columnFieldName])) {
                ...
                echo '<div class="alert alert-danger">Invalid field '. $fieldName .'</div>';
            }
        }
    ...
    protected function loadAccessData(Connection $pdo)
    {
        if (empty($this->access['module']) || empty($this->access['action'])) {
            ...
            $this->details['category'] = 'Pupilsight';
            return;
        }

        try {
            $data = array('module' => $this->access['module'], 'action' => $this->access['action'] );
            $sql = "SELECT pupilsightAction.category, pupilsightAction.entryURL
                    FROM pupilsightAction
                    JOIN pupilsightModule ON (pupilsightAction.pupilsightModuleID=pupilsightModule.pupilsightModuleID)
                    WHERE pupilsightModule.name=:module
                    AND pupilsightAction.name=:action
                    ORDER BY pupilsightAction.precedence ASC
                    LIMIT 1";
    ...
    protected function loadRelationalData(Connection $pdo)
    {
        ...
        if ($this->useYearGroups) {
            try {
                $sql="SELECT pupilsightYearGroupID, nameShort FROM pupilsightYearGroup ORDER BY sequenceNumber";
            ...
            if ($resultYearGroups->rowCount() > 0) {
                while ($yearGroup = $resultYearGroups->fetch()) {
                    $this->yearGroups[ $yearGroup['nameShort'] ] = $yearGroup['pupilsightYearGroupID'];
                }
            }
        }
        ...
        if ($this->useLanguages) {
            try {
                $sql="SELECT name FROM pupilsightLanguage";
        ...
        if ($this->useCountries || $this->usePhoneCodes) {
            try {
                $sql="SELECT printable_name, iddCountryCode FROM pupilsightCountry";
        ...
        if ($this->useCustomFields) {
            try {
                $sql="SELECT pupilsightPersonFieldID, name, type, options, required FROM pupilsightPersonField where active = 'Y'";
            ...
            if ($resultCustomFields->rowCount() > 0) {
                ...
                foreach ($this->table as $fieldName => $field) {
                    ...
                    $this->setField($fieldName, 'customField', $this->customFields[ $customFieldName ]['pupilsightPersonFieldID']);
    ...
    public function filterFieldValue($fieldName, $value)
    {
        ...
        switch ($filter) {
            ...
            case 'language': // Translate a few languages to pupilsight-specific use
     ...
     *
     * @return  string  Random password, based on default Pupilsight function
     */
