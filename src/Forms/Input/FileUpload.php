        ...
        if (!empty($this->attachments)) {
            ...
            foreach ($this->attachments as $attachmentName => $attachmentPath) {
                ...
                if (!empty($attachmentPath)) {
                    ...
                    $output .=  "<a download class='inline-button' href='".$this->absoluteURL.'/'.$attachmentPath."'><i title='".__('Download')."' class='mdi mdi-file-download mdi-24px'></i></a>";

                    if ($this->canDelete) {
                        $attachmentNameEscaped = str_replace(['[', ']'], ['\\\\[', '\\\\]'], $attachmentName);
                        if (!empty($this->deleteAction)) {
                            $output .=  "<a class='inline-button' href='".$this->absoluteURL.'/'.$this->deleteAction."' onclick='return confirm(\"".__('Are you sure you want to delete this record?').' '.__('Unsaved changes will be lost.')."\")'><img title='".__('Delete')."' class='mdi mdi-trash-can-outline mdi-24px px-2'></i></a>";
                        } else {
                            $output .= "<div class='inline-button' onclick='if(confirm(\"".__('Are you sure you want to delete this record?').' '.__('Changes will be saved when you submit this form.')."\")) { $(\"#".$attachmentNameEscaped."\").val(\"\"); $(\"#".$idEscaped."\").show(); $(\"#".$idEscaped." + .max-upload\").show(); $(\"#".$idEscaped."\").prop(\"disabled\", false); $(this).parent().detach().remove(); };'><i title='".__('Delete')."'  class='mdi mdi-trash-can-outline mdi-24px px-2'/></i></div>";
                        }
                    }
                    $output .= '</div>';
                }
