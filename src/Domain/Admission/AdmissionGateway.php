...
class AdmissionGateway extends QueryableGateway
{
    ...
    public function getApp_status(QueryCriteria $criteria, $submissionId, $cuid)
    {
        ...
        $query = $this
            ...
            ->cols([
                'cs.id', 'cm.id as campaign_id', 'cm.form_id', 'cs.submission_id', 'cs.state', 'cs.state_id', 'cs.status', 'cm.name', 'ws.created_at', 'ws.id as subid', 'ws.pupilsightProgramID', 'ws.pupilsightYearGroupID', 'ws.pupilsightPersonID', 'we.field_name', 'we.sub_field_name', 'we.field_value', 'pupilsightPerson.email', 'pupilsightPerson.phone1', 'cm.is_fee_generate'
            ])
    ...
    public function getCampaignFormList(QueryCriteria $criteria, $formId)
    {
        ...
        if (!empty($form_id)) {
            ...
            $query = $this
                ...
                ->from('wp_fluentform_entry_details as fd')
                ->cols([
                    'fd.submission_id as submission_no', 'fd.submission_id', 'fd.status', "GROUP_CONCAT(case when fd.sub_field_name IS NULL OR fd.sub_field_name = '' then fd.field_name else fd.sub_field_name end) as field_name", "GROUP_CONCAT(field_value SEPARATOR '|$$|') as field_value"
                ]);
