 ...
 echo "\n\n<br>\n\n";
    ...
    public function getFeesItem(
        ...
        $query = $this->newQuery()
            ...
            ->cols([
                ...
                "fn_fee_items.name",
                "fn_fee_items.code",
                "fn_fee_items.pupilsightSchoolYearID",
                "fn_fee_items.fn_fee_item_type_id",
                "pupilsightSchoolYear.name AS acedemic_year",
                "fn_fee_item_type.name AS fee_item_type",
            ])
            ->leftJoin(
                "pupilsightSchoolYear",
                "fn_fee_items.pupilsightSchoolYearID=pupilsightSchoolYear.pupilsightSchoolYearID"
            )
            ->leftJoin(
                "fn_fee_item_type",
                "fn_fee_items.fn_fee_item_type_id=fn_fee_item_type.id"
            )
            ->where(
                'fn_fee_items.pupilsightSchoolYearID = "' .
                    $pupilsightSchoolYearID .
        ...
        if (!empty($search)) {
            $query
                ->where('fn_fee_items.name LIKE "%' . $search . '%" ')
    ...
    public function getFeesCounter(QueryCriteria $criteria)
    {
        ...
        $query = $this->newQuery()
            ...
            ->cols([
                "fn_fees_counter.id",
                "fn_fees_counter.name",
                "fn_fees_counter.code",
                "fn_fees_counter.status",
            ])
        ...
        if (!empty($data)) {
            foreach ($data as $k => $d) {
                ...
                $query2 = $this->newQuery()
                    ...
                    ->where(
                        'fn_fees_collection.payment_date = "' . $date . '" '
                    )
                    ->where(
                        "fn_fees_collection.fn_fees_counter_id = " .
                            $corunterid .
                            " "
                    );
                //echo $query2;
    ...
    public function getFeesCounterUsedBy(QueryCriteria $criteria, $id, $input)
    {
        ...
        $query = $this->newQuery()
            ...
            ->cols([
                ...
                "fn_fees_counter.code",
                "fn_fees_counter.status",
                "fn_fees_counter_map.active_date",
                "fn_fees_counter_map.start_time",
                "fn_fees_counter_map.end_time",
                "pupilsightPerson.officialName",
            ])
            ->leftJoin(
                "fn_fees_counter_map",
                "fn_fees_counter.id=fn_fees_counter_map.fn_fees_counter_id"
            )
            ->leftJoin(
                "pupilsightPerson",
                "fn_fees_counter_map.pupilsightPersonID=pupilsightPerson.pupilsightPersonID"
            )
        ...
        if (!empty($input["from_date"])) {
            ...
            $query->where(
                'fn_fees_counter_map.active_date >= "' . $fdate . '" '
            );
        }
        ...
        if (!empty($input["to_date"])) {
            ...
            $query->where(
                'fn_fees_counter_map.active_date <= "' . $tdate . '" '
            );
        }
        ...
        if (!empty($data)) {
            foreach ($data as $k => $d) {
                ...
                if (!empty($corunterid) && !empty($stime) && !empty($etime)) {
                    ...
                    $db = new DBQuery();
                    $newdata = $db->select(
                        'SELECT fn_fees_collection.amount_paying, fn_fees_collection.payment_mode_id, fn_masters.name as pname FROM `fn_fees_collection` LEFT JOIN fn_masters ON fn_fees_collection.payment_mode_id = fn_masters.id WHERE fn_fees_collection.cdt >= "' .
                            $cdts .
                            '" AND fn_fees_collection.cdt <= "' .
                            $cdte .
                            '" AND fn_fees_collection.fn_fees_counter_id = ' .
                            $corunterid .
                            " "
                    );
    ...
    public function getFeesCounterUsedTotal(
        QueryCriteria $criteria,
        $id,
        $input
    ) {
        ...
        $query = $this->newQuery()
            ->from("fn_fees_collection")
            ->cols(["SUM(fn_fees_collection.amount_paying) as amount_paying"])
        ...
        if (!empty($input["from_date"])) {
            ...
            $fdate = date("Y-m-d", strtotime(implode("-", array_reverse($fd))));
            $query->where(
                'fn_fees_collection.payment_date >= "' . $fdate . '" '
            );
        }
        ...
        if (!empty($input["to_date"])) {
            ...
            $tdate = date("Y-m-d", strtotime(implode("-", array_reverse($td))));
            $query->where(
                'fn_fees_collection.payment_date <= "' . $tdate . '" '
            );
        }
        //echo $query;
        return $query;
    ...
    public function getDepositAccount(QueryCriteria $criteria)
    {
        $query = $this->newQuery()
            ...
            ->cols([
                "fn_fees_deposit_account.id",
                "fn_fees_deposit_account.ac_name",
                "fn_fees_deposit_account.ac_code",
                "fn_fees_deposit_account.overpayment_account",
                "fn_fee_items.name as fee_item",
                'SUM(if(fn_fees_collection_deposit.status = "Credit",fn_fees_collection_deposit.amount,0) ) AS creditData',
            ])
            ->leftJoin(
                "fn_fee_items",
                "fn_fees_deposit_account.fn_fee_item_id=fn_fee_items.id"
            )
            ->leftJoin(
                "fn_fees_collection_deposit",
                "fn_fees_deposit_account.id=fn_fees_collection_deposit.deposit_account_id"
            )
        ...
        if (!empty($data)) {
            foreach ($data as $k => $d) {
                ...
                $query2 = $this->newQuery()
                    ...
                    ->where(
                        'fn_fees_collection_deposit.deposit_account_id = "' .
                            $deposit_account_id .
                            '" '
                    )
    ...
    public function getFeesHead(
        ...
        $query = $this->newQuery()
            ...
            ->cols([
                "fn_fees_head.id",
                "fn_fees_head.name",
                "fn_fees_head.account_code",
                "fn_fees_head.description",
                "fn_fees_head.bank_name",
                "fn_fees_head.ac_no",
                "COUNT(fn_fee_structure.id) as structurekount",
                "pupilsightSchoolYear.name AS acedemic_year",
            ])
            ->leftJoin(
                "fn_fee_structure",
                "fn_fees_head.id=fn_fee_structure.fn_fees_head_id"
            )
            ->leftJoin(
                "pupilsightSchoolYear",
                "fn_fees_head.pupilsightSchoolYearID=pupilsightSchoolYear.pupilsightSchoolYearID"
            )
            ->where(
                'fn_fees_head.pupilsightSchoolYearID = "' .
                    $pupilsightSchoolYearID .
                    '" '
            )
    ...
    public function getFeeFineRule(QueryCriteria $criteria)
    {
        $query = $this->newQuery()
            ...
            ->cols(["id", "name", "code", "description"]);

        return $this->runQuery($query, $criteria, true);
    }

    public function getFeeDiscountRule(
        QueryCriteria $criteria,
        $pupilsightSchoolYearID
    ) {
        $query = $this->newQuery()
            ...
            ->leftJoin(
                "pupilsightSchoolYear",
                "fn_fees_discount.pupilsightSchoolYearID=pupilsightSchoolYear.pupilsightSchoolYearID"
            )
            ->where(
                'fn_fees_discount.pupilsightSchoolYearID = "' .
                    $pupilsightSchoolYearID .
                    '" '
            );
    ...
    public function getFeeStructure(
        ...
        } elseif (isset($input["fn_fees_head_id"])) {
        ...
        if (!empty($input)) {
            $query = $this->newQuery()
                ...
                ->cols([
                    "fn_fee_structure.*",
                    "pupilsightSchoolYear.name AS acedemic_year",
                    "fn_fees_head.name AS account_head",
                ...
                ->leftJoin(
                    "fn_fees_head",
                    "fn_fee_structure.fn_fees_head_id=fn_fees_head.id"
                )
                ->where(
            ...
            if (!empty($input["name"])) {
                $query->where(
                    'fn_fee_structure.name LIKE "%' . $input["name"] . '%" '
                );
            }
            if (!empty($fn_fees_head_id)) {
                $query->where(
                    'fn_fee_structure.fn_fees_head_id = "' .
                        $fn_fees_head_id .
                        '" '
                );
            }
    ...
    public function getFeesStructureAssignItem(QueryCriteria $criteria)
    {
        ...
        $query = $this->newQuery()
            ...
            ->cols([
                "fn_fees_class_assign.*",
                "fn_fee_structure.name AS structure_name",
                "pupilsightProgram.name AS program_name",
                "pupilsightYearGroup.name AS class",
            ])
            ->leftJoin(
                "fn_fee_structure",
                "fn_fees_class_assign.fn_fee_structure_id=fn_fee_structure.id"
            )
            ->leftJoin(
                "pupilsightProgram",
                "fn_fees_class_assign.pupilsightProgramID=pupilsightProgram.pupilsightProgramID"
            )
            ->leftJoin(
                "pupilsightYearGroup",
                "fn_fees_class_assign.pupilsightYearGroupID=pupilsightYearGroup.pupilsightYearGroupID"
            )
            //->leftJoin('pupilsightRollGroup', 'fn_fees_class_assign.pupilsightRollGroupID=pupilsightRollGroup.pupilsightRollGroupID')
            ->where(
                "fn_fees_class_assign.fn_fee_structure_id = :fn_fee_structure_id"
            )
    ...
    public function getFeesStructureAssignStudent(
        QueryCriteria $criteria,
        $input
    ) {
        ...
        $query = $this->newQuery()
            ...
            ->cols([
                "fn_fees_student_assign.*",
                "pupilsightPerson.officialName AS student_name",
                "pupilsightPerson.pupilsightPersonID AS stuid",
                "pupilsightPerson.admission_no",
                "GROUP_CONCAT(DISTINCT fn_fee_structure.name SEPARATOR ', ') as structure_name",
                "pupilsightPerson.pupilsightPersonID AS studentid",
                "pupilsightYearGroup.name as classname",
                "pupilsightProgram.name as progname",
            ])
            ->leftJoin(
                "fn_fees_student_assign",
                "pupilsightPerson.pupilsightPersonID=fn_fees_student_assign.pupilsightPersonID"
            )
            ->leftJoin(
                "fn_fee_structure",
                "fn_fees_student_assign.fn_fee_structure_id=fn_fee_structure.id"
            )
            ->leftJoin(
                "pupilsightStudentEnrolment",
                "pupilsightPerson.pupilsightPersonID=pupilsightStudentEnrolment.pupilsightPersonID"
            )
            ->leftJoin(
                "pupilsightYearGroup",
                "pupilsightStudentEnrolment.pupilsightYearGroupID=pupilsightYearGroup.pupilsightYearGroupID"
            )
            ->leftJoin(
                "pupilsightProgram",
                "pupilsightStudentEnrolment.pupilsightProgramID=pupilsightProgram.pupilsightProgramID"
            )

            ->leftJoin(
                "pupilsightFamilyRelationship",
                "pupilsightPerson.pupilsightPersonID=pupilsightFamilyRelationship.pupilsightPersonID2"
            )
            ->leftJoin(
                "pupilsightPerson AS a",
                "pupilsightFamilyRelationship.pupilsightPersonID1=a.pupilsightPersonID"
            )

            ->leftJoin(
                "pupilsightFamilyRelationship AS m",
                "pupilsightPerson.pupilsightPersonID=m.pupilsightPersonID2"
            )
            ->leftJoin(
                "pupilsightPerson AS b",
                "m.pupilsightPersonID1=b.pupilsightPersonID"
            )

            ->where(
                'pupilsightPerson.pupilsightRoleIDAll = "' .
                    $pupilsightRoleIDAll .
                    '" '
            );
        if (!empty($input["pupilsightProgramID"])) {
            $query->where(
                'pupilsightStudentEnrolment.pupilsightProgramID = "' .
                    $input["pupilsightProgramID"] .
                    '" '
            );
        }
        if (!empty($input["pupilsightSchoolYearIDpost"])) {
            $query->where(
                'pupilsightStudentEnrolment.pupilsightSchoolYearID = "' .
                    $input["pupilsightSchoolYearIDpost"] .
                    '" '
            );
        }
        if (!empty($input["pupilsightYearGroupID"])) {
            $query->where(
                'pupilsightStudentEnrolment.pupilsightYearGroupID = "' .
                    $input["pupilsightYearGroupID"] .
                    '" '
            );
        }
        if (!empty($input["pupilsightRollGroupID"])) {
            $query->where(
                'pupilsightStudentEnrolment.pupilsightRollGroupID = "' .
                    $input["pupilsightRollGroupID"] .
                    '" '
            );
        }
        ...
        if (!empty($input["search"]) && empty($input["searchfield"])) {
            ...
            $query
                ->where(
                    'pupilsightPerson.officialName LIKE "%' . $search . '%" '
                )
                ->orwhere(
                    'pupilsightPerson.pupilsightPersonID = "' . $search . '" '
                )
    ...
    public function getStudentlist_quick_cashpayment(QueryCriteria $criteria)
    {
        ...
        $query = $this->newQuery()
            ...
            ->where(
                "pupilsightPerson.pupilsightRoleIDAll = :pupilsightRoleIDAll"
            )
    ...
    public function getInv1oice(
        ...
        $query = $this->newQuery()
            ...
            ->cols([
                "fn_fee_invoice.*",
                "fn_fee_series.series_name",
                "fn_fees_head.name AS account_head",
                "pupilsightProgram.name AS program",
            ])
            // ->leftJoin('pupilsightSchoolYear', 'fn_fee_structure.pupilsightSchoolYearID=pupilsightSchoolYear.pupilsightSchoolYearID')
            ->leftJoin(
                "fn_fee_series",
                "fn_fee_invoice.inv_fn_fee_series_id=fn_fee_series.id"
            )
            ->leftJoin(
                "fn_fee_invoice_class_assign",
                " fn_fee_invoice.id=fn_fee_invoice_class_assign.fn_fee_invoice_id"
            )
            ->leftJoin(
                "pupilsightProgram",
                " fn_fee_invoice_class_assign.pupilsightProgramID=pupilsightProgram.pupilsightProgramID"
            )

            ->leftJoin(
                "fn_fees_head",
                "fn_fee_invoice.fn_fees_head_id=fn_fees_head.id"
            )
            ->where(
                'fn_fee_invoice.pupilsightSchoolYearID  = "' .
                    $pupilsightSchoolYearID .
                    '" '
            )
    ...
    public function getInvoiceTotal(
        ...
        if (
            !empty($input["admission_no"]) ||
            !empty($input["pupilsightProgramID"]) ||
            !empty($input["pupilsightRollGroupID"]) ||
            !empty($input["student_name"]) ||
            !empty($input["due_date"]) ||
            !empty($input["invoice_status"]) ||
            !empty($input["invoice_title"]) ||
            !empty($input["invoice_no"]) ||
            !empty($input["invoice_date"])
        ) {
            $query = $this->newQuery()
                ->from("fn_fee_invoice")
                ->cols([
                    ...
                    "pupilsightProgram.name AS program",
                    "pupilsightYearGroup.name as class",
                    "pupilsightPerson.pupilsightPersonID as std_id",
                    "pupilsightPerson.officialName as std_name",
                    "pupilsightPerson.admission_no",
                    "pupilsightRollGroup.name as section",
                    "fn_fee_invoice_student_assign.invoice_no",
                    "fn_fee_invoice_student_assign.id as invid",
                    "fn_fee_invoice_student_assign.invoice_status as invstatus",
                    "fn_fee_invoice_student_assign.status as chkstatus",
                    "fn_fee_invoice_student_assign.id as insid",
                ])
                ->leftJoin(
                    "fn_fee_invoice_student_assign",
                    " fn_fee_invoice.id=fn_fee_invoice_student_assign.fn_fee_invoice_id"
                )
                ...
                ->leftJoin(
                    "pupilsightPerson",
                    " fn_fee_invoice_student_assign.pupilsightPersonID=pupilsightPerson.pupilsightPersonID"
                )

                ->leftJoin(
                    "pupilsightYearGroup",
                    " fn_fee_invoice_class_assign.pupilsightYearGroupID=pupilsightYearGroup.pupilsightYearGroupID"
                )

                ->leftJoin(
                    "pupilsightProgramClassSectionMapping",
                    " fn_fee_invoice_class_assign.pupilsightYearGroupID=pupilsightProgramClassSectionMapping.pupilsightYearGroupID"
                )
                ->leftJoin(
                    "pupilsightRollGroup",
                ...
                ->leftJoin(
                    "pupilsightStudentEnrolment",
                    "fn_fee_invoice_student_assign.pupilsightPersonID=pupilsightStudentEnrolment.pupilsightPersonID"
                )
                ->leftJoin(
                    "fn_fee_series",
                    "fn_fee_invoice.inv_fn_fee_series_id=fn_fee_series.id"
            ...
            if (!empty($input["pupilsightProgramID"])) {
                $query->where(
                    'fn_fee_invoice_class_assign.pupilsightProgramID = "' .
                        $input["pupilsightProgramID"] .
                        '" '
                );
            }
            ...
            if (!empty($input["pupilsightYearGroupID"])) {
                $query->where(
                    "fn_fee_invoice_class_assign.pupilsightYearGroupID IN (" .
                        implode(",", $input["pupilsightYearGroupID"]) .
                        ") "
                );
            }
            if (!empty($input["pupilsightRollGroupID"])) {
                $query->where(
                    "pupilsightStudentEnrolment.pupilsightRollGroupID IN (" .
                        implode(",", $input["pupilsightRollGroupID"]) .
                        ") "
                );
            }
            ...
            if (!empty($input["student_name"])) {
                $query->where(
                    'pupilsightPerson.officialName LIKE "%' .
                        $input["student_name"] .
                        '%" '
                );
            }
            if (!empty($input["due_date"])) {
                ...
                $sdate = date(
                    "Y-m-d",
                    strtotime(implode("-", array_reverse($sd)))
                );
            ...
            if (!empty($input["invoice_title"])) {
                $query->where(
                    'fn_fee_invoice.title = "' . $input["invoice_title"] . '" '
                );
            }
            if (!empty($input["invoice_no"])) {
                $query->where(
                    'fn_fee_invoice_student_assign.invoice_no LIKE "%' .
                        $input["invoice_no"] .
                        '%" '
                );
            }
            ...
            if (!empty($input["invoice_status"])) {
                $query->where(
                    'fn_fee_invoice_student_assign.invoice_status = "' .
            ...
            foreach ($data as $k => $d) {
                ...
                $query2 = $this->newQuery()
                    ...
                    ->cols([
                        ...
                        "GROUP_CONCAT(fn_fee_invoice_item.id) AS itemid",
                    ])
                    ->where(
                        'fn_fee_invoice_item.fn_fee_invoice_id = "' .
                            $invid .
                            '" '
                    );
                ...
                if (!empty($newdata->data[0]["tot_amount"])) {
                    ...
                    if (!empty($newdata->data[0]["itemid"])) {
                        $query3 = $this->newQuery()
                            ->from("fn_fee_item_level_discount")
                            ->cols([
                                "SUM(fn_fee_item_level_discount.discount) as tot_discount",
                            ])
                            ->where(
                                "fn_fee_item_level_discount.item_id IN (" .
                                    $newdata->data[0]["itemid"] .
                                    ") "
                            )
                            ->where(
                                'fn_fee_item_level_discount.pupilsightPersonID = "' .
                                    $d["std_id"] .
                                    '" '
                            );
                        ...
                        if (!empty($newdata2->data[0]["tot_discount"])) {
                            $amt =
                                $newdata->data[0]["tot_amount"] -
                                $newdata2->data[0]["tot_discount"];
    ...
    public function getInvoice(
        ...
        if (
            ...
            $query = $this->newQuery()
                ...
                ->leftJoin(
                    "pupilsightRollGroup",
                    " pupilsightStudentEnrolment.pupilsightRollGroupID=pupilsightRollGroup.pupilsightRollGroupID"
                )
                ->leftJoin(
                    "fn_fee_series",
                    "fn_fee_invoice.inv_fn_fee_series_id=fn_fee_series.id"
        ...
        if (!empty($data)) {
            foreach ($data as $k => $d) {
                ...
                $query2 = $this->newQuery()
                    ...
                    ->where(
                        'fn_fees_student_collection.fn_fees_invoice_id = "' .
                            $invid .
                            '" '
                    )
                    ->where(
                        'fn_fees_student_collection.pupilsightPersonID = "' .
                            $d["std_id"] .
                            '" '
                    );
                ...
                if (!empty($newdata->data[0]["transaction_id"])) {
                    $query7 = $this->newQuery()
                        ->from("fn_fees_collection")
                        ->cols(["invoice_status"])
                        ->where(
                            'fn_fees_collection.transaction_id = "' .
                                $newdata->data[0]["transaction_id"] .
                                '" '
                        )
                    ...
                    if (!empty($newdata7->data[0]["invoice_status"])) {
                        if (
                            $newdata7->data[0]["invoice_status"] == "Fully Paid"
                        ) {
    ...
    public function getInvoiceAssignItem(QueryCriteria $criteria)
    {
        ...
        $query = $this->newQuery()
            ...
            ->cols([
                "fn_fee_invoice_class_assign.*",
                "fn_fee_invoice.title AS invoice_name",
                "fn_fee_invoice.id AS invoice_id",
                "pupilsightProgram.name AS program_name",
                "GROUP_CONCAT(DISTINCT pupilsightYearGroup.name SEPARATOR ', ') as class",
            ])
            ->leftJoin(
                "fn_fee_invoice",
                "fn_fee_invoice_class_assign.fn_fee_invoice_id=fn_fee_invoice.id"
            )
            ->leftJoin(
                "pupilsightProgram",
                "fn_fee_invoice_class_assign.pupilsightProgramID=pupilsightProgram.pupilsightProgramID"
            ...
            ->where(
                "fn_fee_invoice_class_assign.fn_fee_invoice_id = :fn_fee_invoice_id"
            )
    ...
    public function getCollectionInvoice(QueryCriteria $criteria, $stuId)
    {
        ...
        $query = $this->newQuery()
            ...
            ->cols([
                "fn_fee_invoice.*",
                "fn_fee_invoice.id as invoiceid",
                "SUM(fn_fee_invoice_item.amount) as totalamount",
                "SUM(fn_fee_invoice_item.amount) as pendingamount",
                "fn_fee_invoice_student_assign.invoice_no as stu_invoice_no",
            ])
            ->leftJoin(
                "pupilsightStudentEnrolment",
                "fn_fee_invoice.pupilsightSchoolYearID=pupilsightStudentEnrolment.pupilsightSchoolYearID"
            )
            ->leftJoin(
                "pupilsightPerson",
                "pupilsightStudentEnrolment.pupilsightPersonID=pupilsightPerson.pupilsightPersonID"
            )
            ->leftJoin(
                "fn_fee_invoice_item",
                "fn_fee_invoice.id=fn_fee_invoice_item.fn_fee_invoice_id"
            )
            ->leftJoin(
                "fn_fee_invoice_student_assign",
                "pupilsightPerson.pupilsightPersonID=fn_fee_invoice_student_assign.pupilsightPersonID"
            );
        if (!empty($stuId)) {
            $query->where(
                'pupilsightPerson.pupilsightPersonID = "' . $stuId . '" '
            );
        }
        ...
        foreach ($data as $k => $d) {
            ...
            $sqla =
                'SELECT GROUP_CONCAT(fn_fee_invoice_item_id) AS invitemid FROM fn_fees_student_collection WHERE invoice_no = "' .
                $invno .
                '"';
            ...
            $sqlp =
                'SELECT SUM(amount) as paidtotalamount FROM fn_fee_invoice_item WHERE fn_fee_invoice_id IN "(' .
                $itemids .
                ')"';
    ...
    public function getInvoiceFeeItems(QueryCriteria $criteria)
    {
        $query = $this->newQuery()
            ->from("fn_fee_invoice_item")
            ->cols(["fn_fee_invoice_item.*"]);
        //->where('fn_fee_invoice_item.id = "0" ');

        return $this->runQuery($query, $criteria);
    }

    public function getFeesTransactiontotal(
        QueryCriteria $criteria,
        $input,
        $pupilsightSchoolYearID
    ) {
        ...
        if (!empty($input)) {
            $query = $this->newQuery()
                ...
                ->cols([
                    ...
                    "pupilsightPerson.pupilsightPersonID as stu_id",
                    "fn_fees_collection.id as collection_id",
                    "GROUP_CONCAT(DISTINCT fn_fees_student_collection.invoice_no) as invoice_no",
                    "fn_masters.name as paymentmode",
                    "bnk.name as bankname",
                    "pupilsightStudentEnrolment.pupilsightYearGroupID as classid",
                    "pupilsightStudentEnrolment.pupilsightRollGroupID as sectionid",
                ])
                ->leftJoin(
                    "pupilsightPerson",
                    "fn_fees_collection.pupilsightPersonID=pupilsightPerson.pupilsightPersonID"
                )
                ->leftJoin(
                    "pupilsightStudentEnrolment",
                    "fn_fees_collection.pupilsightPersonID=pupilsightStudentEnrolment.pupilsightPersonID"
                )
                ->leftJoin(
                    "fn_fees_student_collection",
                    "fn_fees_collection.transaction_id=fn_fees_student_collection.transaction_id"
                )
                //->leftJoin('fn_fee_invoice_student_assign', 'fn_fees_student_collection.invoice_no=fn_fee_invoice_student_assign.invoice_no')
                ->leftJoin(
                    "fn_masters",
                    "fn_fees_collection.payment_mode_id=fn_masters.id"
                )
                ->leftJoin(
                    "fn_masters as bnk",
                    "fn_fees_collection.bank_id=bnk.id"
                );
            ...
            if (!empty($input["instrument_no"])) {
                $query->where(
                    'fn_fees_collection.instrument_no = "' .
                        $input["instrument_no"] .
                        '" '
                );
            }
            if (!empty($input["bank_id"])) {
                $query->where(
                    'fn_fees_collection.bank_id = "' . $input["bank_id"] . '" '
                );
            }
            if (!empty($input["payment_mode_id"])) {
                $query->where(
                    'fn_fees_collection.payment_mode_id = "' .
            ...
            if (!empty($input["payment_status"])) {
                if ($input["payment_status"] == "Transaction Successful") {
                    ...
                    $query->where(
                        'fn_fees_collection.payment_status = "' .
            ...
            if (!empty($input["pupilsightYearGroupID"])) {
                $query->where(
                    "pupilsightStudentEnrolment.pupilsightYearGroupID IN (" .
                        implode(",", $input["pupilsightYearGroupID"]) .
                        ") "
                );
            }
            ...
            if (!empty($input["startdate"]) && !empty($input["enddate"])) {
                ...
                $edate = date(
                    "Y-m-d",
                    strtotime(implode("-", array_reverse($ed)))
                );

                $query
                    ->where(
                        'fn_fees_collection.payment_date >= "' . $sdate . '" '
                    )
                    ->where(
                        'fn_fees_collection.payment_date <= "' . $edate . '" '
                    );
            }
            $query
                ->where('fn_fees_collection.transaction_status = "1" ')
                ->where(
                    'fn_fees_collection.pupilsightSchoolYearID = "' .
                        $pupilsightSchoolYearID .
                        '" '
                )
    ...
    public function getFeesTransaction(
        QueryCriteria $criteria,
        $input,
        $pupilsightSchoolYearID
    ) {
    ...
    public function getFeesCancelTransaction(
        QueryCriteria $criteria,
        $input,
        $pupilsightSchoolYearID
    ) {
        ...
        if (!empty($input)) {
            $query = $this->newQuery()
                ...
                ->cols([
                    ...
                    "pupilsightPerson.admission_no",
                    "fn_fees_collection.id as collection_id",
                    "GROUP_CONCAT(DISTINCT fn_fees_student_cancel_collection.invoice_no) as invoice_no",
                    "fn_masters.name as paymentmode",
                    "bnk.name as bankname",
                    "fn_fees_cancel_collection.remarks AS cancelreason",
                    "a.officialName as stfName",
                    "fn_fees_cancel_collection.cdt",
                ])
                ->leftJoin(
                    "pupilsightPerson",
                    "fn_fees_collection.pupilsightPersonID=pupilsightPerson.pupilsightPersonID"
                ...
                ->leftJoin(
                    "fn_fees_student_cancel_collection",
                    "fn_fees_collection.transaction_id=fn_fees_student_cancel_collection.transaction_id"
                )
                ->leftJoin(
                    "fn_fee_invoice_student_assign",
                    "fn_fees_student_cancel_collection.invoice_no=fn_fee_invoice_student_assign.invoice_no"
                )
                ->leftJoin(
                    "fn_masters",
                    "fn_fees_collection.payment_mode_id=fn_masters.id"
                ...
                ->leftJoin(
                    ...
                    "fn_fees_collection.id=fn_fees_cancel_collection.fn_fees_collection_id"
                )
                ->leftJoin(
                    "pupilsightPerson AS a",
                    "fn_fees_cancel_collection.canceled_by=a.pupilsightPersonID"
                );
            if (!empty($input["transaction_id"])) {
                $query->where(
                    'fn_fees_collection.transaction_id = "' .
    ...
    public function getFeesCancelTransactionTotal(
        QueryCriteria $criteria,
        $input,
        $pupilsightSchoolYearID
    ) {
    ...
    public function getFeesRefundTransactionTotal(
        QueryCriteria $criteria,
        $input,
        $pupilsightSchoolYearID
    ) {
        ...
        if (!empty($input)) {
            $query = $this->newQuery()
                ...
                ->cols([
                    ...
                    "pupilsightPerson.pupilsightPersonID as stu_id",
                    "fn_fees_collection.id as collection_id",
                    "fn_fees_collection_refund.*",
                    "fn_masters.name as paymentmode",
                    "bnk.name as bankname",
                    "refby.officialName as refundby",
                    "GROUP_CONCAT(DISTINCT fn_fees_student_collection.invoice_no) as invoice_no",
                ])
                ->leftJoin(
                    "fn_fees_collection_refund",
                    "fn_fees_collection.id=fn_fees_collection_refund.fn_fees_collection_id"
                )
                ->leftJoin(
                    "pupilsightPerson",
                    "fn_fees_collection_refund.pupilsightPersonID=pupilsightPerson.pupilsightPersonID"
                )
                ->leftJoin(
                    "pupilsightStudentEnrolment",
                    "fn_fees_collection.pupilsightPersonID=pupilsightStudentEnrolment.pupilsightPersonID"
                ...
                ->leftJoin(
                    "fn_masters",
                    "fn_fees_collection_refund.payment_mode_id=fn_masters.id"
                )
                ->leftJoin(
                    "fn_masters as bnk",
                    "fn_fees_collection_refund.bank_id=bnk.id"
                )
                ->leftJoin(
                    "pupilsightPerson as refby",
                    "fn_fees_collection_refund.refund_by=refby.pupilsightPersonID"
                )
                ->leftJoin(
                    "fn_fees_student_collection",
                    "fn_fees_collection.transaction_id=fn_fees_student_collection.transaction_id"
                ...
                ->leftJoin(
                    "fn_fee_invoice_student_assign",
                    "fn_fees_student_collection.invoice_no=fn_fee_invoice_student_assign.invoice_no"
                );
            if (!empty($input["transaction_id"])) {
                $query->where(
                    'fn_fees_collection.transaction_id = "' .
            ...
            if (!empty($input["receipt_number"])) {
                $query->where(
                    'fn_fees_collection_refund.receipt_number = "' .
                        $input["receipt_number"] .
                        '" '
                );
            }
            if (!empty($input["instrument_no"])) {
                $query->where(
                    'fn_fees_collection_refund.instrument_no = "' .
                        $input["instrument_no"] .
                        '" '
                );
            }
            if (!empty($input["bank_id"])) {
                $query->where(
                    'fn_fees_collection_refund.bank_id = "' .
                        $input["bank_id"] .
                        '" '
                );
            }
            ...
            if (!empty($input["startdate"]) && !empty($input["enddate"])) {
                ...
                $query
                    ->where(
                        'fn_fees_collection_refund.refund_date >= "' .
                            $sdate .
                            '" '
                    )
                    ->where(
    ...
    public function getFeesRefundTransaction(
        QueryCriteria $criteria,
        $input,
        $pupilsightSchoolYearID
    ) {
        ...
        if (!empty($input)) {
            $query = $this->newQuery()
                ...
                ->cols([
                    ...
                    "pupilsightPerson.admission_no",
                    "fn_fees_collection.id as collection_id",
                    "fn_fees_collection.amount_paying as transaction_amount",
                    "fn_fees_collection.transcation_amount as transaction_pending_amount",
                    "fn_fees_collection_refund.*",
                    "fn_masters.name as paymentmode",
                    "bnk.name as bankname",
                    "refby.officialName as refundby",
    ...
    public function getCollectionInvoiceRefund(
        QueryCriteria $criteria,
        $transids
    ) {
        //echo $transids;
        $query = $this->newQuery()
            ...
            ->cols([
                "fn_fee_invoice.*",
                "fn_fee_invoice.id as invoiceid",
                "SUM(fn_fee_invoice_item.total_amount) as totalamount",
                "SUM(fn_fee_invoice_item.total_amount) as invoiceamount",
                "SUM(fn_fee_invoice_item.amount) as pendingamount",
                "fn_fees_student_collection.invoice_no",
                "fn_fees_collection.fine",
                "fn_fees_collection.discount",
                "fn_fees_collection.payment_status",
            ])
            ->leftJoin(
                "fn_fees_student_collection",
                "fn_fee_invoice.id=fn_fees_student_collection.fn_fees_invoice_id"
            )
            ->leftJoin(
                "fn_fee_invoice_item",
                "fn_fees_student_collection.fn_fee_invoice_item_id=fn_fee_invoice_item.id"
            )
            ->leftJoin(
                "fn_fees_collection",
                "fn_fees_student_collection.transaction_id=fn_fees_collection.transaction_id"
            )
    ...
    public function getReceiptTemplate(QueryCriteria $criteria)
    {
        $query = $this->newQuery()
            ...
            ->cols(["id", "name", "path", "filename", "type"])
            ->orderBy(["id DESC"]);
    ...
    public function getInvoiceforDiscount(
        QueryCriteria $criteria,
        $input,
        $pupilsightSchoolYearID
    ) {
        ...
        if (
            !empty($input["fn_fee_invoice_id"]) ||
            !empty($input["pupilsightProgramID"]) ||
            !empty($input["pupilsightRollGroupID"]) ||
            !empty($input["pupilsightYearGroupID"])
        ) {
            $query = $this->newQuery()
                ->from("fn_fee_invoice")
                ->cols([
            ...
            $query
                ->where(
                    'fn_fee_invoice_student_assign.invoice_status = "Not Paid" '
                )
                ->where("pupilsightPerson.pupilsightRoleIDPrimary = 003")
                //->where('fn_fee_invoice_student_assign.status = 1 ')
                ->groupby(["fn_fee_invoice_student_assign.id"])
    ...
    public function getDepositAccountDetails(
        $criteria,
        $id,
        $pupilsightPersonID
    ) {
        $query = $this->newQuery()
            ...
            ->cols([
                "fn_fees_deposit_account.id",
                "fn_fees_deposit_account.ac_name",
                "fn_fees_collection_deposit.pupilsightPersonID",
                "fn_fees_collection_deposit.amount",
                "fn_fees_collection_deposit.transaction_id",
                "fn_fees_collection_deposit.status",
                "fn_fees_collection_deposit.cdt",
                "fn_fees_collection.receipt_number",
                "fn_fees_collection.payment_mode_id",
                "fn_fees_student_collection.invoice_no",
                "pupilsightPerson.officialName",
            ...
            ->leftJoin(
                "fn_fees_collection",
                "fn_fees_collection_deposit.transaction_id=fn_fees_collection.transaction_id"
            )
            ->leftJoin(
                "fn_fees_student_collection",
                "fn_fees_collection_deposit.transaction_id=fn_fees_student_collection.transaction_id"
            )
            ->leftJoin(
                "fn_masters",
                "fn_fees_collection.payment_mode_id=fn_masters.id"
            ...
            ->leftJoin(
                "pupilsightPerson",
                "fn_fees_collection_deposit.pupilsightPersonID=pupilsightPerson.pupilsightPersonID"
            )
            ->where(
                'fn_fees_collection_deposit.pupilsightPersonID = "' .
                    $pupilsightPersonID .
    ...
    public function getDepositAccountForStudent(
        QueryCriteria $criteria,
        $pupilsightPersonID
    ) {
        $query = $this->newQuery()
    ...
    public function getWaiveOffForStudent($criteria, $pupilsightPersonID)
    {
        $query = $this->newQuery()
            ...
            ->cols(["fn_fee_waive_off.*", "pupilsightPerson.officialName"])
            ->leftJoin(
                "pupilsightPerson",
                "fn_fee_waive_off.assigned_by=pupilsightPerson.pupilsightPersonID"
            )
            ->where(
                'fn_fee_waive_off.pupilsightPersonID = "' .
                    $pupilsightPersonID .
                    '" '
            );
    ...
    public function getAllPaymentDetails(
        ...
        $query = $this->newQuery()
            ...
            ->leftJoin(
                "pupilsightPerson",
                "fn_fee_payment_details.pupilsightPersonID=pupilsightPerson.pupilsightPersonID"
            )
            ->leftJoin(
                "pupilsightStudentEnrolment",
                "pupilsightPerson.pupilsightPersonID=pupilsightStudentEnrolment.pupilsightPersonID"
