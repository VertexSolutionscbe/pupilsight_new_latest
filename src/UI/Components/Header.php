...
namespace Pupilsight\UI\Components;
        ...
        if (isset($_SESSION[$guid]['username']) != false) {
            ...
            if (isActionAccessible($guid, $connection2, '/modules/Messenger/messageWall_view.php')) {
                    ...
                    if ($resultPosts->rowCount() > 0) {
                        ...
                        while ($rowPosts = $resultPosts->fetch()) {
                            if ($last == $rowPosts['pupilsightMessengerID']) {
                                ...
                                $output[$_SESSION[$guid]['messageWallCount']]['pupilsightMessengerID'] = $rowPosts['pupilsightMessengerID'];
                ...
                if (isset($_SESSION[$guid]['messageWallCount']) == false) {
                    $return .= "<a class='inactive inline-block relative mr-4' title='".__('Message Wall')."' href='$URL'><img class='minorLinkIcon' style='margin-left: 4px; opacity: 0.2; vertical-align: -75%' src='".$_SESSION[$guid]['absoluteURL'].'/themes/'.$_SESSION[$guid]['pupilsightThemeName']."/img/messageWall.png'></a>";
                } else {
                    if ($_SESSION[$guid]['messageWallCount'] < 1) {
                        $return .= "<a class='inactive inline-block relative mr-4' title='".__('Message Wall')."' href='$URL'><img class='minorLinkIcon' style='margin-left: 4px; opacity: 0.2; vertical-align: -75%' src='".$_SESSION[$guid]['absoluteURL'].'/themes/'.$_SESSION[$guid]['pupilsightThemeName']."/img/messageWall.png'></a>";
                    } else {
                        $return .= "<a class='inline-block relative mr-4' title='".__('Message Wall')."' href='$URL'><span class='badge -mr-2 right-0'>".$_SESSION[$guid]['messageWallCount']."</span><img class='minorLinkIcon' style='margin-left: 4px; vertical-align: -75%' src='".$_SESSION[$guid]['absoluteURL'].'/themes/'.$_SESSION[$guid]['pupilsightThemeName']."/img/messageWall.png'></a>";
                        ...
                        if (empty($_SESSION[$guid]['pageLoads']) and ($_SESSION[$guid]['messengerLastBubble'] == null or $_SESSION[$guid]['messengerLastBubble'] < date('Y-m-d'))) {
                            ...
                            try {
                                $data = array('messengerLastBubble' => date('Y-m-d'), 'pupilsightPersonID' => $_SESSION[$guid]['pupilsightPersonID']);
                                $sql = 'UPDATE pupilsightPerson SET messengerLastBubble=:messengerLastBubble WHERE pupilsightPersonID=:pupilsightPersonID';
            ...
            if ($resultNotifications->rowCount() > 0) {
                $return .= "<a class='inline-block relative mr-4' title='".__('Notifications')."' href='".$_SESSION[$guid]['absoluteURL']."/index.php?q=notifications.php'><span class='badge -mr-2 right-0'>".$resultNotifications->rowCount()."</span><img class='minorLinkIcon' style='margin-left: 2px; vertical-align: -75%' src='".$_SESSION[$guid]['absoluteURL'].'/themes/'.$_SESSION[$guid]['pupilsightThemeName']."/img/notifications.png'></a>";
            } else {
                $return .= "<a class='inactive inline-block relative mr-4' title='".__('Notifications')."' href='".$_SESSION[$guid]['absoluteURL']."/index.php?q=notifications.php'><img class='minorLinkIcon' style='margin-left: 2px; opacity: 0.2; vertical-align: -75%' src='".$_SESSION[$guid]['absoluteURL'].'/themes/'.$_SESSION[$guid]['pupilsightThemeName']."/img/notifications.png'></a>";
            }
            $return .= '</div>';
        }
    ...
    public function getMinorLinks($cacheLoad)
    {
        ...
        if (isset($_SESSION[$guid]['i18n']['default']['code']) && isset($_SESSION[$guid]['i18n']['code'])) {
            if ($_SESSION[$guid]['i18n']['code'] != $_SESSION[$guid]['i18n']['default']['code']) {
                ...
                $languageLink = "<a class='link-black' href='".$_SESSION[$guid]['absoluteURL']."?i18n=".$_SESSION[$guid]['i18n']['default']['code']."'>".$systemDefaultShortName.'</a>';
            }
        }
        ...
        if (isset($_SESSION[$guid]['username']) == false) {
            ...
            if ($_SESSION[$guid]['webLink'] != '') {
                ...
                $return .= __('Return to')." <a class='link-black' style='margin-right: 12px' target='_blank' href='".$_SESSION[$guid]['webLink']."'>".$_SESSION[$guid]['organisationNameShort'].' '.__('Website').'</a>';
            }
        } else {
            $name = $_SESSION[$guid]['preferredName'].' '.$_SESSION[$guid]['surname'];
            if (isset($_SESSION[$guid]['pupilsightRoleIDCurrentCategory'])) {
                if ($_SESSION[$guid]['pupilsightRoleIDCurrentCategory'] == 'Student') {
                    $highestAction = getHighestGroupedAction($guid, '/modules/Students/student_view_details.php', $connection2);
                    if ($highestAction == 'View Student Profile_brief') {
                        $name = "<a class='link-black' href='".$_SESSION[$guid]['absoluteURL'].'/index.php?q=/modules/Students/student_view_details.php&pupilsightPersonID='.$_SESSION[$guid]['pupilsightPersonID']."'>".$name.'</a>';
                    }
                }
            }

            $return .= $name.' . ';
            $return .= "<a class='link-black' href='./logout.php'>".__('Logout')."</a> . <a class='link-black' href='./index.php?q=preferences.php'>".__('Preferences').'</a>';
            if ($_SESSION[$guid]['emailLink'] != '') {
                $return .= "<span class='hidden sm:inline'> . <a class='link-black' target='_blank' href='".$_SESSION[$guid]['emailLink']."'>".__('Email').'</a></span>';
            }
            if ($_SESSION[$guid]['webLink'] != '') {
                $return .= "<span class='hidden sm:inline'>  . <a class='link-black' target='_blank' href='".$_SESSION[$guid]['webLink']."'>".$_SESSION[$guid]['organisationNameShort'].' '.__('Website').'</a></span>';
            }
            if ($_SESSION[$guid]['website'] != '') {
                $return .= "<span class='hidden sm:inline'>  . <a class='link-black' target='_blank' href='".$_SESSION[$guid]['website']."'>".__('My Website').'</a></span>';
            }

            $return .= !empty($languageLink) ? ' . '.$languageLink : '';

            //Check for house logo (needed to get bubble, below, in right spot)
            if (isset($_SESSION[$guid]['pupilsightHouseIDLogo']) and isset($_SESSION[$guid]['pupilsightHouseIDName'])) {
                if ($_SESSION[$guid]['pupilsightHouseIDLogo'] != '') {
                    $return .= " . <img class='ml-1 w-10 h-10 sm:w-12 sm:h-12 lg:w-16 lg:h-16' title='".$_SESSION[$guid]['pupilsightHouseIDName']."' style='vertical-align: -75%;' src='".$_SESSION[$guid]['absoluteURL'].'/'.$_SESSION[$guid]['pupilsightHouseIDLogo']."'/>";
                }
            }
        }
