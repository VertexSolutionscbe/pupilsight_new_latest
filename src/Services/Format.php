...
class Format
{
    ...
    public static function setupFromSession(Session $session)
    {
        ...
        $settings['pupilsightThemeName'] = $session->get('pupilsightThemeName');
    ...
    {
        $class .= ' inline-block shadow bg-white border border-gray ';
        ...
        if (empty($path) or file_exists(static::$settings['absolutePath'].'/'.$path) == false) {
            $path = '/themes/'.static::$settings['pupilsightThemeName'].'/img/anonymous_'.$imageSize.'.jpg';
        }
    ...
    public static function userBirthdayIcon($dob, $preferredName)
    {
        ...
        return sprintf('<img class="absolute bottom-0 -ml-4" title="%1$s" src="%2$s">', $title, static::$settings['absoluteURL'].'/themes/'.static::$settings['pupilsightThemeName'].'/img/'.$icon);
    }
