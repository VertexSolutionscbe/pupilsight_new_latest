...
class Html extends BaseReader
{
     ...
     */
    protected function processDomElement(
        DOMNode $element,
        Worksheet $sheet,
        &$row,
        &$column,
        ...
        foreach ($element->childNodes as $child) {
            if ($child instanceof DOMText) {
                ...
                switch ($child->nodeName) {
                    ...
                    case "b":
                        if (
                            isset($attributeArray["class"]) &&
                            $attributeArray["class"] === "comment"
                        ) {
                            $sheet
                                ->getComment($column . $row)
                                ->getText()
                    ...
                    case "a":
                        foreach (
                            ...
                            switch ($attributeName) {
                                ...
                                case "class":
                                    if (
                                        $attributeValue === "comment-indicator"
                                    ) {
                    ...
                    case "tr":
                        ...
                        if (isset($attributeArray["height"])) {
                            $sheet
                                ->getRowDimension($row)
                                ->setRowHeight($attributeArray["height"]);
                        }
                    ...
                    case "td":
                        ...
                        if (isset($attributeArray["align"])) {
                            $sheet
                                ->getStyle($column . $row)
                                ->getAlignment()
                                ->setHorizontal($attributeArray["align"]);
                        }
                        ...
                        if (isset($attributeArray["valign"])) {
                            $sheet
                                ->getStyle($column . $row)
                                ->getAlignment()
                                ->setVertical($attributeArray["valign"]);
                        }
                        ...
                        if (isset($attributeArray["data-format"])) {
                            $sheet
                                ->getStyle($column . $row)
                                ->getNumberFormat()
                                ->setFormatCode($attributeArray["data-format"]);
                        }
    ...
    public function loadIntoExisting($pFilename, Spreadsheet $spreadsheet)
    {
        ...
        $loaded = $dom->loadHTML(
            mb_convert_encoding(
                $this->securityScanner->scanFile($pFilename),
                "HTML-ENTITIES",
                "UTF-8"
            )
        );
        if ($loaded === false) {
            throw new Exception(
                "Failed to load " . $pFilename . " as a DOM Document"
            );
        }
        ...
        foreach ($styles as $st) {
            ...
            switch ($styleName) {
                ...
                case "font-family":
                    $cellStyle
                        ->getFont()
                        ->setName(str_replace('\'', "", $styleValue));
                ...
                case "width":
                    $sheet
                        ->getColumnDimension($column)
                        ->setWidth(str_replace("px", "", $styleValue));
                ...
                case "height":
                    $sheet
                        ->getRowDimension($row)
                        ->setRowHeight(str_replace("px", "", $styleValue));
                ...
                case "word-wrap":
                    $cellStyle
                        ->getAlignment()
                        ->setWrapText($styleValue === "break-word");
                ...
                case "text-indent":
                    $cellStyle
                        ->getAlignment()
                        ->setIndent((int) str_replace(["px"], "", $styleValue));
    ...
    private function insertImage(
        ...
        $sheet
            ->getRowDimension($row)
            ->setRowHeight($drawing->getHeight() * 0.9);
    }
