...
function __($text, $params=[], $options=[])
{
    ...
    return $pupilsight->locale->translate($text, $params, $options);
}
...
{
    global $pupilsight;
    return $pupilsight->locale->translateN($singular, $plural, $n, $params, $options);
}
...
function __m(string $text, array $params = [], array $options = [])
{
    ...
    if ($pupilsight->session->has('module')) {
        $options['domain'] = $pupilsight->session->get('module');
    }
...
function renderGradeScaleSelect($connection2, $guid, $pupilsightScaleID, $fieldName, $valueMode, $honourDefault = true, $width = 50, $selectedMode = 'value', $selectedValue = null)
{
    $return = false;
    ...
    while ($rowSelect = $resultSelect->fetch()) {
        ...
        if ($honourDefault and is_null($selectedValue)) { //Select entry based on scale default
            ...
            if ($rowSelect['pupilsightScaleGradeID'] == $selectedValue) {
                $selected = 'selected';
            }
        }
        if ($valueMode == 'value') {
...
//Archives one or more notifications, based on partial match of actionLink and total match of pupilsightPersonID
function archiveNotification($connection2, $guid, $pupilsightPersonID, $actionLink)
{
    $return = true;

    try {
        $data = array('pupilsightPersonID' => $pupilsightPersonID, 'actionLink' => "%$actionLink%");
        $sql = "UPDATE pupilsightNotification SET status='Archived' WHERE pupilsightPersonID=:pupilsightPersonID AND actionLink LIKE :actionLink AND status='New'";
 ...
 */
function setNotification($connection2, $guid, $pupilsightPersonID, $text, $moduleName, $actionLink)
{
    global $pdo, $pupilsight;

    $notificationGateway = new \Pupilsight\Domain\System\NotificationGateway($pdo);
    $notificationSender = new \Pupilsight\Comms\NotificationSender($notificationGateway, $pupilsight->session);

    $notificationSender->addNotification($pupilsightPersonID, $text, $moduleName, $actionLink);
...
function getAlert($guid, $connection2, $pupilsightAlertLevelID)
{
    $output = false;

    try {
        $dataAlert = array('pupilsightAlertLevelID' => $pupilsightAlertLevelID);
        $sqlAlert = 'SELECT * FROM pupilsightAlertLevel WHERE pupilsightAlertLevelID=:pupilsightAlertLevelID';
...
function getUnit($connection2, $pupilsightUnitID, $pupilsightCourseClassID)
{
    $output = array();
    $unitType = false;
    if ($pupilsightUnitID != '') {
        try {
            $dataUnit = array('pupilsightUnitID' => $pupilsightUnitID);
...
function getWeekNumber($date, $connection2, $guid)
{
    $week = 0;
    try {
        $dataWeek = array('pupilsightSchoolYearID' => $_SESSION[$guid]['pupilsightSchoolYearID']);
        $sqlWeek = 'SELECT * FROM pupilsightSchoolYearTerm WHERE pupilsightSchoolYearID=:pupilsightSchoolYearID ORDER BY sequenceNumber';
...
function getYearGroupsFromIDList($guid, $connection2, $ids, $vertical = false, $translated = true)
{
    ...
    try {
        $sqlYears = 'SELECT DISTINCT nameShort, sequenceNumber FROM pupilsightYearGroup ORDER BY sequenceNumber';
        ...
        if (count($years) > 0 and $years[0] != '') {
            if (count($years) == $resultYears->rowCount()) {
                ...
                try {
                    ...
                    $sqlYears = "SELECT DISTINCT nameShort, sequenceNumber FROM pupilsightYearGroup $sqlYearsOr ORDER BY sequenceNumber";
...
function getTerms($connection2, $pupilsightSchoolYearID, $short = false)
{
    $output = false;
    //Scan through year groups
    try {
...
function getHighestMedicalRisk($guid, $pupilsightPersonID, $connection2)
{
    $output = false;

    try {
        $dataAlert = array('pupilsightPersonID' => $pupilsightPersonID);
        $sqlAlert = 'SELECT * FROM pupilsightPersonMedical JOIN pupilsightPersonMedicalCondition ON (pupilsightPersonMedical.pupilsightPersonMedicalID=pupilsightPersonMedicalCondition.pupilsightPersonMedicalID) JOIN pupilsightAlertLevel ON (pupilsightPersonMedicalCondition.pupilsightAlertLevelID=pupilsightAlertLevel.pupilsightAlertLevelID) WHERE pupilsightPersonID=:pupilsightPersonID ORDER BY pupilsightAlertLevel.sequenceNumber DESC';
        $resultAlert = $connection2->prepare($sqlAlert);
        $resultAlert->execute($dataAlert);
    } catch (PDOException $e) {
    }
    ...
    if ($resultAlert->rowCount() > 0) {
        $rowAlert = $resultAlert->fetch();
        $output = array();
        $output[0] = $rowAlert['pupilsightAlertLevelID'];
...
function isSchoolOpen($guid, $date, $connection2, $allYears = '')
{
    ...
    if ($isInTerm == true) {
        try {
            ...
            $sql = "SELECT * FROM pupilsightDaysOfWeek WHERE nameShort=:nameShort AND schoolDay='Y'";
            $result = $connection2->prepare($sql);
            $result->execute($data);
        } catch (PDOException $e) {
        }
    ...
    if ($isInTerm == true and $isSchoolDay == true) {
        try {
            ...
            $sql = "SELECT * FROM pupilsightSchoolYearSpecialDay WHERE type='School Closure' AND date=:date";
            $result = $connection2->prepare($sql);
            $result->execute($data);
        } catch (PDOException $e) {
        }
...
function getAlertBar($guid, $connection2, $pupilsightPersonID, $privacy = '', $divExtras = '', $div = true, $large = false)
{
    $output = '';
    $alerts = [];
    ...
    if ($highestAction == 'View Student Profile_full' or $highestAction == 'View Student Profile_fullNoNotes') {
        ...
        // Academic
        $pupilsightAlertLevelID = '';
        $alertThresholdText = '';
        try {
            $dataAlert = array('pupilsightPersonIDStudent' => $pupilsightPersonID, 'pupilsightSchoolYearID' => $_SESSION[$guid]['pupilsightSchoolYearID'], 'today' => date('Y-m-d'), 'date' => date('Y-m-d', (time() - (24 * 60 * 60 * 60))));
            $sqlAlert = "SELECT *
            ...
            WHERE pupilsightPersonIDStudent=:pupilsightPersonIDStudent
        ...
        $alertThresholdText = '';
        try {
            $dataAlert = array('pupilsightPersonID' => $pupilsightPersonID, 'date' => date('Y-m-d', (time() - (24 * 60 * 60 * 60))));
        ...
        // Medical
        if ($alert = getHighestMedicalRisk($guid, $pupilsightPersonID, $connection2)) {
            $alerts[] = [
...
function getSystemSettings($guid, $connection2)
{
    ...
    try {
        $data = array();
        $sql = "SELECT * FROM pupilsightSetting WHERE scope='System'";
        $result = $connection2->prepare($sql);
        $result->execute($data);
    } catch (PDOException $e) {
    ...
    //DBA
    try {
        $data = array('pupilsightPersonID' => $_SESSION[$guid]['organisationDBA']);
        $sql = 'SELECT surname, preferredName, email FROM pupilsightPerson WHERE pupilsightPersonID=:pupilsightPersonID';
        $result = $connection2->prepare($sql);
        $result->execute($data);
    } catch (PDOException $e) {
    ...
    //Admissions
    try {
        $data = array('pupilsightPersonID' => $_SESSION[$guid]['organisationAdmissions']);
        $sql = 'SELECT surname, preferredName, email FROM pupilsightPerson WHERE pupilsightPersonID=:pupilsightPersonID';
        $result = $connection2->prepare($sql);
        $result->execute($data);
    } catch (PDOException $e) {
    ...
    //HR Administraotr
    try {
        $data = array('pupilsightPersonID' => $_SESSION[$guid]['organisationHR']);
        $sql = 'SELECT surname, preferredName, email FROM pupilsightPerson WHERE pupilsightPersonID=:pupilsightPersonID';
        $result = $connection2->prepare($sql);
        $result->execute($data);
    } catch (PDOException $e) {
...
{
    $_SESSION[$guid]['i18n']['pupilsighti18nID'] = $row['pupilsighti18nID'];
...
function getSettingByScope($connection2, $scope, $name, $returnRow = false )
{
    try {
        ...
        $sql = 'SELECT * FROM pupilsightSetting WHERE scope=:scope AND name=:name';
        $result = $connection2->prepare($sql);
        $result->execute($data);
    } catch (PDOException $e) {
    }
...
function isActionAccessible($guid, $connection2, $address, $sub = '')
{
    ...
    if (isset($_SESSION[$guid]['username'])) {
        ...
        if ($_SESSION[$guid]['pupilsightRoleIDCurrent'] != '') {
            //Check module ready
            $moduleID = checkModuleReady($address, $connection2);
            if ($moduleID != false) {
                ...
                try {
                    ...
                    if ($sub != '') {
                        $data['sub'] = $sub;
                        $sqlWhere = 'AND pupilsightAction.name=:sub';
                    }
                    $sql = "SELECT pupilsightAction.name FROM pupilsightAction, pupilsightPermission, pupilsightRole WHERE (pupilsightAction.URLList LIKE :actionName) AND (pupilsightAction.pupilsightActionID=pupilsightPermission.pupilsightActionID) AND (pupilsightPermission.pupilsightRoleID=pupilsightRole.pupilsightRoleID) AND (pupilsightPermission.pupilsightRoleID=:pupilsightRoleID) AND (pupilsightAction.pupilsightModuleID=$moduleID) $sqlWhere";
                    $result = $connection2->prepare($sql);
                    $result->execute($data);
                    if ($result->rowCount() > 0) {
                        $output = true;
    ...
    if ($_SESSION[$guid]['username'] != '') {
        ...
        if ($_SESSION[$guid]['pupilsightRoleIDCurrent'] != '') {
            ...
            if ($moduleID != false) {
                ...
                try {
                    $data = array('pupilsightRoleID' => $_SESSION[$guid]['pupilsightRoleIDCurrent'], 'moduleID' => $moduleID);
                    $sql = 'SELECT * FROM pupilsightAction, pupilsightPermission, pupilsightRole WHERE (pupilsightAction.pupilsightActionID=pupilsightPermission.pupilsightActionID) AND (pupilsightPermission.pupilsightRoleID=pupilsightRole.pupilsightRoleID) AND (pupilsightPermission.pupilsightRoleID=:pupilsightRoleID) AND (pupilsightAction.pupilsightModuleID=:moduleID)';
                    $result = $connection2->prepare($sql);
                    $result->execute($data);
                    if ($result->rowCount() > 0) {
                        $output = true;
...
function getRoleList($pupilsightRoleIDAll, $connection2)
{
    $output = array();

    //Tokenise list of roles
    $roles = explode(',', $pupilsightRoleIDAll);

    //Check that roles exist
    $count = 0;
    for ($i = 0; $i < count($roles); ++$i) {
        try {
            $data = array('pupilsightRoleID' => $roles[$i]);
            $sql = 'SELECT * FROM pupilsightRole WHERE pupilsightRoleID=:pupilsightRoleID';
            $result = $connection2->prepare($sql);
            $result->execute($data);
        } catch (PDOException $e) {
        ...
        if ($result->rowCount() == 1) {
            $row = $result->fetch();
            $output[$count][0] = $row['pupilsightRoleID'];
...
function checkModuleReady($address, $connection2)
{
    ...
    try {
        ...
        $sql = "SELECT * FROM pupilsightModule WHERE name=:name AND active='Y'";
        $result = $connection2->prepare($sql);
        $result->execute($data);
        if ($result->rowCount() == 1) {
            $row = $result->fetch();
            $output = $row['pupilsightModuleID'];
        }
    } catch (PDOException $e) {
    }
...
function setCurrentSchoolYear($guid,  $connection2)
{
    ...
    //Else get schoolYearID
        ...
        $row = $result->fetch();
        $_SESSION[$guid]['pupilsightSchoolYearID'] = $row['pupilsightSchoolYearID'];
        $_SESSION[$guid]['pupilsightSchoolYearName'] = $row['name'];
        $_SESSION[$guid]['pupilsightSchoolYearSequenceNumber'] = $row['sequenceNumber'];
        $_SESSION[$guid]['pupilsightSchoolYearFirstDay'] = $row['firstDay'];
        $_SESSION[$guid]['pupilsightSchoolYearLastDay'] = $row['lastDay'];
    }
}
...
function getPreviousSchoolYearID($pupilsightSchoolYearID, $connection2)
{
    $output = false;
    ...
    if ($result->rowcount() == 1) {
        ...
        try {
            ...
            $sqlPrevious = 'SELECT * FROM pupilsightSchoolYear WHERE sequenceNumber<:sequenceNumber ORDER BY sequenceNumber DESC';
        ...
        if ($resultPrevious->rowCount() >= 1) {
            ...
            $output = $rowPrevious['pupilsightSchoolYearID'];
        }
    }
...
//Take a school year, and return the previous one, or false if none
function getNextSchoolYearID($pupilsightSchoolYearID, $connection2)
{
    $output = false;
    ...
    if ($result->rowcount() == 1) {
        ...
        try {
            $dataPrevious = array('sequenceNumber' => $row['sequenceNumber']);
            $sqlPrevious = 'SELECT * FROM pupilsightSchoolYear WHERE sequenceNumber>:sequenceNumber ORDER BY sequenceNumber ASC';
            $resultPrevious = $connection2->prepare($sqlPrevious);
            $resultPrevious->execute($dataPrevious);
        } catch (PDOException $e) {
        }
...
function getNextYearGroupID($pupilsightYearGroupID, $connection2)
{
    $output = false;
    try {
    ...
    if ($result->rowCount() == 1) {
        ...
        try {
            $dataPrevious = array('sequenceNumber' => $row['sequenceNumber']);
            $sqlPrevious = 'SELECT * FROM pupilsightYearGroup WHERE sequenceNumber>:sequenceNumber ORDER BY sequenceNumber ASC';
            $resultPrevious = $connection2->prepare($sqlPrevious);
            $resultPrevious->execute($dataPrevious);
        } catch (PDOException $e) {
        }
        if ($resultPrevious->rowCount() >= 1) {
            $rowPrevious = $resultPrevious->fetch();
            $output = $rowPrevious['pupilsightYearGroupID'];
        }
    }
...
function getNextRollGroupID($pupilsightRollGroupID, $connection2)
{
    $output = false;
    try {
    ...
    if ($result->rowCount() == 1) {
        ...
        if (!is_null($row['pupilsightRollGroupIDNext'])) {
            $output = $row['pupilsightRollGroupIDNext'];
        }
    }
...
function getLastYearGroupID($connection2)
{
    ...
    try {
        $data = array();
        $sql = 'SELECT * FROM pupilsightYearGroup ORDER BY sequenceNumber DESC';
        $result = $connection2->prepare($sql);
        $result->execute($data);
    } catch (PDOException $e) {
    }
...
function setLog($connection2, $pupilsightSchoolYearID, $pupilsightModuleID, $pupilsightPersonID, $title, $array = null, $ip = null)
{
    if ((!is_array($array) && $array != null) || $title == null || $pupilsightSchoolYearID == null) {
        return;
    }
    ...
    try {
        $dataLog = array('pupilsightSchoolYearID' => $pupilsightSchoolYearID, 'pupilsightModuleID' => $pupilsightModuleID, 'pupilsightPersonID' => $pupilsightPersonID, 'title' => $title, 'serialisedArray' => $serialisedArray, 'ip' => $ip);
        $sqlLog = 'INSERT INTO pupilsightLog SET pupilsightSchoolYearID=:pupilsightSchoolYearID, pupilsightModuleID=:pupilsightModuleID, pupilsightPersonID=:pupilsightPersonID, title=:title, serialisedArray=:serialisedArray, ip=:ip';
        $resultLog = $connection2->prepare($sqlLog);
        $resultLog->execute($dataLog);
    } catch (PDOException $e) {
        return;
    }
    $pupilsightLogID = $connection2->lastInsertId();

    return $pupilsightLogID;
}

function getLog($connection2, $pupilsightSchoolYearID, $pupilsightModuleID = null, $pupilsightPersonID = null, $title = null, $startDate = null, $endDate = null, $ip = null, $array = null)
{
    if ($pupilsightSchoolYearID == null || $pupilsightSchoolYearID == '') {
        return;
    }
    $dataLog = array('pupilsightSchoolYearID' => $pupilsightSchoolYearID);
    $where = '';
    ...
    if ($pupilsightModuleID != null && $pupilsightModuleID != '') {
        $dataLog['pupilsightModuleID'] = $pupilsightModuleID;
        $where .= ' AND pupilsightModuleID=:pupilsightModuleID';
    }

    if ($pupilsightPersonID != null && $pupilsightPersonID != '') {
        $dataLog['pupilsightPersonID'] = $pupilsightPersonID;
        $where .= ' AND pupilsightPersonID=:pupilsightPersonID';
    }
...
function getLogByID($connection2, $pupilsightLogID)
{
    if ($pupilsightLogID == null) {
        return;
    }
    try {
        $dataLog = array('pupilsightLogID' => $pupilsightLogID);
        $sqlLog = 'SELECT * FROM pupilsightLog WHERE pupilsightLogID=:pupilsightLogID';
        $resultLog = $connection2->prepare($sqlLog);
        $resultLog->execute($dataLog);
        $row = $resultLog->fetch();
    } catch (PDOException $e) {
...
function getModuleIDFromName($connection2, $name)
{
    try {
        ...
        $sqlModuleID = 'SELECT pupilsightModuleID FROM pupilsightModule WHERE name=:name';
...
function getPupilsightMailer($guid) {
    ...
    trigger_error('getPupilsightMailer method is deprecated and replaced by Pupilsight\Comms\Mailer class', E_USER_DEPRECATED);
 ...
 *
 * @param string $guid
 *      The guid of your Pupilsight Install.
 * @param string $return
