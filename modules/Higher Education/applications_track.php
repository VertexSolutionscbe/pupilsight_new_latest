...
if (isActionAccessible($guid, $connection2, '/modules/Higher Education/applications_track.php') == false) {
    ...
    if (studentEnrolment($_SESSION[$guid]['pupilsightPersonID'], $connection2) == false) {
        ...
        if (isset($values['higherEducationApplicationID']) == false) {
            ...
            echo "<a href='".$_SESSION[$guid]['absoluteURL'].'/index.php?q=/modules/'.$_SESSION[$guid]['module']."/applications_track_add.php'><img title='New' src='./themes/".$_SESSION[$guid]['pupilsightThemeName']."/img/page_new.png'/></a>";
            echo '</div>';
            ...
            if ($resultApps->rowCount() < 1) {
                ...
                while ($rowApps = $resultApps->fetch()) {
                    ...
                    echo '</td>';
                    echo '<td>';
                    echo "<a href='".$_SESSION[$guid]['absoluteURL'].'/index.php?q=/modules/'.$_SESSION[$guid]['module'].'/applications_track_edit.php&higherEducationApplicationInstitutionID='.$rowApps['higherEducationApplicationInstitutionID']."'><img title='Edit' src='./themes/".$_SESSION[$guid]['pupilsightThemeName']."/img/config.png'/></a> ";
                    echo "<a class='thickbox' href='".$_SESSION[$guid]['absoluteURL'].'/fullscreen.php?q=/modules/'.$_SESSION[$guid]['module'].'/applications_track_delete.php&higherEducationApplicationInstitutionID='.$rowApps['higherEducationApplicationInstitutionID']."&width=650&height=135'><img title='Delete' src='./themes/".$_SESSION[$guid]['pupilsightThemeName']."/img/garbage.png'/></a> ";
                    echo '</td>';
                    echo '</tr>';
