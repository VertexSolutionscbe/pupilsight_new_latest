...
if (isActionAccessible($guid, $connection2, '/modules/Higher Education/references_request.php') == false) {
    ...
    if (studentEnrolment($_SESSION[$guid]['pupilsightPersonID'], $connection2) == false) {
        ...
        try {
            ...
            $sql = 'SELECT higherEducationReference.* FROM higherEducationReference WHERE higherEducationReference.pupilsightPersonID=:pupilsightPersonID ORDER BY timestamp';
        ...
        echo "<a href='".$_SESSION[$guid]['absoluteURL'].'/index.php?q=/modules/'.$_SESSION[$guid]['module']."/references_request_add.php'><img title='New' src='./themes/".$_SESSION[$guid]['pupilsightThemeName']."/img/page_new.png'/></a>";
        echo '</div>';
        ...
        if ($result->rowCount() < 1) {
            ...
            while ($row = $result->fetch()) {
                ...
                if ($row['status'] == 'Cancelled') {
                    ...
                    echo "<img style='padding-bottom: 3px; margin-right: 3px; float: left' title='In Progress' src='./themes/".$_SESSION[$guid]['pupilsightThemeName']."/img/iconTick_light.png'/> ";
                }
                echo '</td>';
                echo '<td>';
                echo '<b>'.$row['status'].'</b>';
                ...
                try {
                    ...
                    $sqlReferee = 'SELECT DISTINCT pupilsightPerson.title, surname, preferredName FROM higherEducationReferenceComponent JOIN pupilsightPerson ON (higherEducationReferenceComponent.pupilsightPersonID=pupilsightPerson.pupilsightPersonID) WHERE higherEducationReferenceID=:higherEducationReferenceID ORDER BY surname, preferredName';
