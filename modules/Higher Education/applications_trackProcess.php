...
if (isActionAccessible($guid, $connection2, '/modules/Higher Education/applications_track.php') == false) {
    ...
    if ($higherEducationApplicationID == '') {
        //Insert new record
        try {
            $data = array('pupilsightPersonID' => $_SESSION[$guid]['pupilsightPersonID'], 'applying' => $applying, 'careerInterests' => $careerInterests, 'coursesMajors' => $coursesMajors, 'otherScores' => $otherScores, 'personalStatement' => $personalStatement, 'meetingNotes' => $meetingNotes);
            $sql = 'INSERT INTO higherEducationApplication SET pupilsightPersonID=:pupilsightPersonID, applying=:applying, careerInterests=:careerInterests, coursesMajors=:coursesMajors, otherScores=:otherScores, personalStatement=:personalStatement, meetingNotes=:meetingNotes';
