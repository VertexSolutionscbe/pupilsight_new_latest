...
if (isActionAccessible($guid, $connection2, '/modules/Higher Education/references_myNotes.php') == false) {
    ...
    if (studentEnrolment($_SESSION[$guid]['pupilsightPersonID'], $connection2) == false) {
        ...
        //Write to database
        try {
            $data = array('referenceNotes' => $referenceNotes, 'pupilsightPersonID' => $_SESSION[$guid]['pupilsightPersonID']);
            $sql = 'UPDATE higherEducationStudent SET referenceNotes=:referenceNotes WHERE pupilsightPersonID=:pupilsightPersonID';
