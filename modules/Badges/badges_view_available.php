...
if (isActionAccessible($guid, $connection2, '/modules/Badges/badges_view_available.php') == false) {
    ...
    $form = Form::create('grantbadges',$pupilsight->session->get('absoluteURL').'/index.php','GET');
    $form->setFactory(DatabaseFormFactory::create($pdo));
    $form->addClass('noIntBorder');
    $form->addHiddenValue('q','/modules/' . $pupilsight->session->get('module') . '/badges_view_available.php');
    ...
    $row = $form->addRow()->addSearchSubmit($pupilsight->session);
    echo $form->getOutput();
    ...
    if ($result->rowCount() < 1) {
        ...
        while ($row = $result->fetch()) {
            ...
            if ($row['logo'] != '') {
                echo "<img style='margin-bottom: 20px; max-width: 150px' src='".$pupilsight->session->get('absoluteURL','').'/'.$row['logo']."'/><br/>";
            } else {
                echo "<img style='margin-bottom: 20px; max-width: 150px' src='".$pupilsight->session->get('absoluteURL','').'/themes/'.$pupilsight->session->get('pupilsightThemeName')."/img/anonymous_240_square.jpg'/><br/>";
            }
