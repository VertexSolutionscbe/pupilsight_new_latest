...
if (isActionAccessible($guid, $connection2, '/modules/Badges/badges_manage_delete.php') == false) {
    ...
    if ($badgesBadgeID == '') {
        ...
        if ($result->rowCount() != 1) {
            ...
            $form = DeleteForm::createForm($pupilsight->session->get('absoluteURL','').'/modules/'.$pupilsight->session->get('module')."/badges_manage_deleteProcess.php?badgesBadgeID=$badgesBadgeID&search=".$_GET['search']."&&category=" . $_GET['category']);
            echo $form->getOutput();
        }
    }
}
