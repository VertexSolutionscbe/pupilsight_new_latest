    ...
    if ($badgesBadgeID == '') { echo "<div class='error'>";
        ...
        if ($result->rowCount() != 1) {
            ...
            if ($_GET['search'] != '' || $_GET['category'] != '') {
                ...
                echo "<a href='".$pupilsight->session->get('absoluteURL','').'/index.php?q=/modules/Badges/badges_manage.php&search='.$_GET['search'].'&category='.$_GET['category']."'>Back to Search Results</a>";
                echo '</div>';
            }

            $form = Form::create('badges', $pupilsight->session->get('absoluteURL','').'/modules/'.$pupilsight->session->get('module').'/badges_manage_editProcess.php?badgesBadgeID='.$badgesBadgeID.'&search='.$_GET['search'].'&category='.$_GET['category']);
            ...
            $row = $form->addRow();
                ...
                $row->addFileUpload('file')
                    ...
                    ->setAttachment('logo', $pupilsight->session->get('absoluteURL',''), $values['logo']);
