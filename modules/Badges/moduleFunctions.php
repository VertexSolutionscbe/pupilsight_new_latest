...
function getBadges($connection2, $guid, $pupilsightPersonID)
{
    $output = '';
    ...
    try {
        ...
        $sql = 'SELECT badgesBadgeStudent.*, badgesBadge.name AS award, badgesBadge.logo AS logo, badgesBadge.category AS category, pupilsightSchoolYear.name AS year FROM badgesBadgeStudent JOIN badgesBadge ON (badgesBadgeStudent.badgesBadgeID=badgesBadge.badgesBadgeID) JOIN pupilsightSchoolYear ON (badgesBadgeStudent.pupilsightSchoolYearID=pupilsightSchoolYear.pupilsightSchoolYearID) WHERE pupilsightPersonID=:pupilsightPersonID AND license=\'Y\' ORDER BY pupilsightSchoolYear.sequenceNumber DESC, date DESC';
    ...
    if ($result->rowCount() > 0) {
        ...
        while ($row = $result->fetch()) {
            ...
            if ($row['logo'] != '') {
                ...
                $output .= "<img style='margin-bottom: 20px; max-width: 150px' src='".$_SESSION[$guid]['absoluteURL'].'/themes/'.$_SESSION[$guid]['pupilsightThemeName']."/img/anonymous_240_square.jpg'/><br/>";
            }
    ...
    try {
        $data = array('pupilsightPersonID' => $pupilsightPersonID);
        $sql = 'SELECT badgesBadgeStudent.*, badgesBadge.name AS award, badgesBadge.logo AS logo, badgesBadge.category AS category, pupilsightSchoolYear.name AS year FROM badgesBadgeStudent JOIN badgesBadge ON (badgesBadgeStudent.badgesBadgeID=badgesBadge.badgesBadgeID) JOIN pupilsightSchoolYear ON (badgesBadgeStudent.pupilsightSchoolYearID=pupilsightSchoolYear.pupilsightSchoolYearID) WHERE pupilsightPersonID=:pupilsightPersonID ORDER BY pupilsightSchoolYear.sequenceNumber DESC, date DESC';
        $result = $connection2->prepare($sql);
        $result->execute($data);
    } catch (PDOException $e) {
        echo "<div class='error'>".$e->getMessage().'</div>';
