...
if (isActionAccessible($guid, $connection2, '/modules/Campaign/view_selected_campaign_form.php') != false) {
    ...
    if ($id == '') {
        ...
        if ($result->rowCount() != 1) {
            ...
            echo '</h2>';
            $values = $result->fetch();
            $viewurl = 'wp/wp-admin/admin.php?page=fluent_forms';
...
<script>
    ...
    $('#application_view').load(function(){
        ...
        iframe.find("#saveFormData").click(function () {
            ...
            var cid = $("#cid").val();
            $.ajax({
                url: 'modules/Campaign/changeOfflineCampaignForm.php',
                type: 'post',
                data: { formshortcode: fformid, cid:cid },
                async: true,
                success: function (response) {
                    alert('Application Form Changed Successfully!');
                    location.reload();
                }
            });
