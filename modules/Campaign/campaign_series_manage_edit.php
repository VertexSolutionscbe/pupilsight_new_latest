...
if (isActionAccessible($guid, $connection2, '/modules/Campaign/campaign_series_manage_edit.php') == false) {
    //Acess denied
    ...
    if ($id == '') {
        ...
        if ($result->rowCount() != 1) {
            ...
            $form = Form::create('program', $_SESSION[$guid]['absoluteURL'].'/modules/'.$_SESSION[$guid]['module'].'/campaign_series_manage_editProcess.php?id='.$id);
            ...
            $row = $form->addRow();
                    ...
                    $row->addSelect('type')->fromArray($seriestype)->selected($values['type'])->required();
