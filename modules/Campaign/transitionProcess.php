...
if (isActionAccessible($guid, $connection2, '/modules/Campaign/transitionProcess.php') == false) {
    $URL .= '&return=error0';
    header("Location: {$URL}");
} else {
    //Proceed!
                ...
                if(!empty($table_name)){
                    foreach($table_name as $k=> $d){
                       ...
                       if(!empty($table_name) && !empty($column_name)){
                            $data = array('campaign_id' => $campaign,'table_name' => $table_name, 'column_name' => $column, 'fluent_form_column_name' => $fluent,  'transition_id' => $tansition_id, 'cuid' => $cuid);
                            ...
                            // if(empty($values)){
                                $sql = "INSERT INTO campaign_transitions_form_map SET campaign_id=:campaign_id,table_name=:table_name, column_name=:column_name,fluent_form_column_name=:fluent_form_column_name,transition_id=:transition_id, cuid=:cuid";
