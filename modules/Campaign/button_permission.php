...
if (isActionAccessible($guid, $connection2, '/modules/Campaign/index.php') == false) {
    ...
    $sql = 'SELECT * FROM pupilsightModuleButton WHERE pupilsightModuleID = 178 ';
                        ...
                        <select name="staff[<?php echo $bl['pupilsightModuleButtonID']?>][]" class="form-control staffList" multiple>
                            ...
                            <?php if(!empty($getstaff)) { 
                                ...
                                foreach($getstaff as $st){ 
                                    $sqlchk = 'SELECT pupilsightPersonID FROM pupilsightModuleButtonPermission WHERE pupilsightModuleID = 178 AND pupilsightPersonID = '.$st['stu_id'].' AND pupilsightModuleButtonID = '.$bl['pupilsightModuleButtonID'].' ';
