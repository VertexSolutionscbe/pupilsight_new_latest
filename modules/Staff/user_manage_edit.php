...
if (isActionAccessible($guid, $connection2, '/modules/User Admin/user_manage_edit.php') == false) {
	...
	if ($pupilsightPersonID == '') {
		...
		if ($result->rowCount() != 1) {
			...
			$form = Form::create('addUser', $_SESSION[$guid]['absoluteURL'] . '/modules/User Admin' . '/user_manage_editProcess.php?pupilsightPersonID=' . $pupilsightPersonID . '&search=' . $search);
			...
			if (empty($roleRestriction) || ($roleRestriction == 'Admin Only' && !in_array('001', $currentUserRoles)) || ($roleRestriction == 'Same Role' && !in_array($values['pupilsightRoleIDPrimary'], $currentUserRoles) && !in_array('001', $currentUserRoles))) {
				$row = $form->addRow("row_pupilsightRoleIDPrimaryName");
			...
			$row = $form->addRow("row_pupilsightRoleIDAll");
			...
			if (!empty($restrictedRoles)) {
				...
				$row = $form->addRow("row_pupilsightRoleIDRestricted");
			...
			if ($values['address1'] != '') {
				...
				if ($resultAddress->rowCount() > 0) {
					...
					$row = $form->addRow("row_matchAddress")->addClass('address  matchHighlight');
			...
			if ($student || $staff) {
				...
				$row = $form->addRow("row_nextSchool");
				...
				$row = $form->addRow("row_departureReason");
