...
if ($proceed == false) {
    ...
    if ($result->rowCount() < 1) {
        ...
        echo "<a href='" . $_SESSION[$guid]['absoluteURL'] . '/index.php?q=/modules/' . $_SESSION[$guid]['module'] . "/applicationForm_jobOpenings_view.php'>" . __('View Current Job Openings') . "<img style='margin-left: 5px' title='" . __('View Current Job Openings') . "' src='./themes/" . $_SESSION[$guid]['pupilsightThemeName'] . "/img/plus.png'/></a>";
        echo '</div>';
        ...
        foreach ($jobOpenings as $jobOpening) {
            $jobOpeningsProcessed[$jobOpening['pupilsightStaffJobOpeningID']] = $jobOpening['jobTitle'];
        }
        $row = $form->addRow();
        $row->addLabel('pupilsightStaffJobOpeningID[]', __('Job Openings'))->description(__('Please select one or more jobs to apply for.'));
        $row->addCheckbox('pupilsightStaffJobOpeningID[]')->fromArray($jobOpeningsProcessed)->required();
        ...
        /*  if ($pupilsightPersonID = null) { //Logged in
