...
$URL = $_SESSION[$guid]['absoluteURL'].'/index.php?q=/modules/'.getModuleName($_POST['address'])."/staff_attendance_configSettings.php";

if (isActionAccessible($guid, $connection2, '/modules/Attendance/staff_attendance_configSettings_manage_edit.php') == false) {
    $URL .= '&return=error0';
    header("Location: {$URL}");
} else {
    // echo '<pre>';
    ...
    //Proceed!
    //Validate Inputs
    //$classes = (isset($_POST['classes']))? $_POST['classes'] : "0";
   ...
   // print_r($pupilsightYearGroupID)  ;die();
    ...
    if ($pupilsightProgramID == '' or $attn_type == '' or $id == '' or $pupilsightSchoolYearID =='' ) {
        ...
        try {
            $datau = array('pupilsightSchoolYearID'=>$pupilsightSchoolYearID,'pupilsightProgramID' => $pupilsightProgramID, 'attn_type' => $attn_type, 'id' => $id);
            $sql = 'SELECT id FROM attn_settings_staff WHERE (pupilsightSchoolYearID=:pupilsightSchoolYearID AND pupilsightProgramID=:pupilsightProgramID  AND attn_type=:attn_type) AND NOT id=:id';
           // print_r( $datau);die();
        ...
        if ($result->rowCount() > 0) {
            ...
            //Write to database
            try {
                $data = array('pupilsightProgramID' => $pupilsightProgramID, 'pupilsightSchoolYearID'=>$pupilsightSchoolYearID,'pupilsightYearGroupID'=>$pupilsightYearGroupID,'attn_type' => $attn_type, 'enable_sms_absent' => $enable_sms_absent, 'display_field_1' => $display_field_1, 'enable_sort_display_field_1' => $enable_sort_display_field_1, 'display_field_2' => $display_field_2, 'enable_sort_display_field_2' => $enable_sort_display_field_2, 'default_display' => $default_display, 'no_of_session' => $no_of_session,'lock_attendance_marking'=>$lock_attendance_marking,'fromDate'=>$fromDate,'toDate'=>$toDate,'auto_lock_attendance'=>$auto_lock_attendance,'select_sub_mandatory'=>$select_sub_mandatory,'sms_template_id'=>$sms_template_id,'sms_recipients'=>$sms_recipients,'id' => $id);
               
               $sql = 'UPDATE attn_settings_staff SET pupilsightSchoolYearID=:pupilsightSchoolYearID,pupilsightProgramID=:pupilsightProgramID,pupilsightYearGroupID=:pupilsightYearGroupID, attn_type=:attn_type, enable_sms_absent=:enable_sms_absent, display_field_1=:display_field_1, enable_sort_display_field_1=:enable_sort_display_field_1, display_field_2=:display_field_2, enable_sort_display_field_2=:enable_sort_display_field_2, default_display=:default_display, no_of_session=:no_of_session,lock_attendance_marking=:lock_attendance_marking,fromDate=:fromDate,toDate=:toDate,auto_lock_attendance=:auto_lock_attendance,select_sub_mandatory=:select_sub_mandatory,sms_template_id=:sms_template_id,sms_recipients=:sms_recipients WHERE id=:id';
                $result = $connection2->prepare($sql);
                $result->execute($data);
                ...
                if(!empty($session_name)){
                    ...
                    foreach($session_name as $k=> $d){
                        ...
                        if(!empty($sname) && !empty($sno)){
                            $data1 = array('attn_settings_id' => $id, 'session_name' => $sname, 'session_no' => $sno);
                            $sql1 = "INSERT INTO attn_session_settings_staff SET attn_settings_id=:attn_settings_id, session_name=:session_name, session_no=:session_no";
