...
if (isActionAccessible($guid, $connection2, '/modules/Attendance/staff_attendance_configSettings_manage_edit.php') == false) {
    ...
    //Proceed!
    $page->breadcrumbs
        ->add(__('Attendance Configuration'), 'staff_attendance_configSettings.php')
    ...
    $sqldata = 'SELECT * FROM attn_settings_staff WHERE id = "'.$id.'" ';
    $resultdata = $connection2->query($sqldata);
    $values = $resultdata->fetch();
   /* echo "<pre>";
    print_r($values);*/
    if(!empty($values['no_of_session'])){
        $sqlsessdata = 'SELECT * FROM attn_session_settings_staff WHERE attn_settings_id = "'.$id.'" ';
...
//student_id,preferredName,pupilsightPersonID,Class-yearGroup,rollGroup->Section,dob
   ...
   $recipients= array(
    ...
    $form = Form::create('attendanceConfigSetting', $_SESSION[$guid]['absoluteURL'].'/modules/'.$_SESSION[$guid]['module'].'/staff_attendance_configSettings_manage_editProcess.php?id='.$id);
