...
if (isActionAccessible($guid, $connection2, '/modules/Attendance/attendance_configSettings_manage_edit.php') == false) {
   ...
   // print_r($pupilsightYearGroupID)  ;die();
    ...
    if ($pupilsightProgramID == '' or $attn_type == '' or $id == '' or $pupilsightSchoolYearID =='' or $fromDate>$toDate) {
        ...
        try {
            $datau = array('pupilsightSchoolYearID'=>$pupilsightSchoolYearID,'pupilsightProgramID' => $pupilsightProgramID,'pupilsightYearGroupID'=>$pupilsightYearGroupID, 'attn_type' => $attn_type, 'id' => $id);
            $sql = 'SELECT id FROM attn_settings WHERE (pupilsightSchoolYearID=:pupilsightSchoolYearID AND pupilsightProgramID=:pupilsightProgramID AND pupilsightYearGroupID=:pupilsightYearGroupID AND attn_type=:attn_type) AND NOT id=:id';
           // print_r( $datau);die();
        ...
        if ($result->rowCount() > 0) {
            ...
            try {
               ...
               $sql = 'UPDATE attn_settings SET pupilsightSchoolYearID=:pupilsightSchoolYearID,pupilsightProgramID=:pupilsightProgramID,pupilsightYearGroupID=:pupilsightYearGroupID, attn_type=:attn_type, enable_sms_absent=:enable_sms_absent, display_field_1=:display_field_1, enable_sort_display_field_1=:enable_sort_display_field_1, display_field_2=:display_field_2, enable_sort_display_field_2=:enable_sort_display_field_2, default_display=:default_display, no_of_session=:no_of_session,lock_attendance_marking=:lock_attendance_marking,fromDate=:fromDate,toDate=:toDate,auto_lock_attendance=:auto_lock_attendance,select_sub_mandatory=:select_sub_mandatory,sms_template_id=:sms_template_id,sms_recipients=:sms_recipients WHERE id=:id';
                $result = $connection2->prepare($sql);
                $result->execute($data);
