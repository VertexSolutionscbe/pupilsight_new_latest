...
if (isActionAccessible($guid, $connection2, '/modules/Attendance/staff_attendance_configSettings_manage_add.php') == false) {
    $URL .= '&return=error0';
    header("Location: {$URL}");
} else {
    ...
    if ($pupilsightProgramID == '' or $attn_type == '') {
        ...
        try {
            ...
            $data = array('pupilsightProgramID' => $pupilsightProgramID,'attn_type' => $attn_type,'pupilsightSchoolYearID'=>$pupilsightSchoolYearID);
            $sql = 'SELECT id FROM attn_settings_staff WHERE pupilsightProgramID=:pupilsightProgramID             
        ...
        if ($result->rowCount() > 0) {
            ...
            try {
               ...
               $sql = 'INSERT INTO attn_settings_staff SET pupilsightSchoolYearID=:pupilsightSchoolYearID,pupilsightProgramID=:pupilsightProgramID, pupilsightYearGroupID=:pupilsightYearGroupID,attn_type=:attn_type, enable_sms_absent=:enable_sms_absent, select_sub_mandatory=:select_sub_mandatory,display_field_1=:display_field_1, enable_sort_display_field_1=:enable_sort_display_field_1, display_field_2=:display_field_2, enable_sort_display_field_2=:enable_sort_display_field_2, default_display=:default_display, no_of_session=:no_of_session,lock_attendance_marking=:lock_attendance_marking,fromDate=:fromDate,toDate=:toDate,auto_lock_attendance=:auto_lock_attendance,sms_template_id=:sms_template_id,sms_recipients=:sms_recipients';
                $result = $connection2->prepare($sql);
                $result->execute($data);
