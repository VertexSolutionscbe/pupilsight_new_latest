...
if (isActionAccessible($guid, $connection2, '/modules/Attendance/report_studentsNotOnsite_byDate.php') == false) {
    ...
    if ($currentDate != '') {
        ...
        if ($result->rowCount() < 1) {
            ...
            echo "<a target='_blank' href='".$_SESSION[$guid]['absoluteURL'].'/report.php?q=/modules/'.$_SESSION[$guid]['module'].'/report_studentsNotOnsite_byDate_print.php&currentDate='.dateConvertBack($guid, $currentDate)."&allStudents=" . $allStudents . "&sort=" . $sort . "&pupilsightYearGroupIDList=";
            if (is_array($pupilsightYearGroupIDList)) {
