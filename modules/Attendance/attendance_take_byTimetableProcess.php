			...
			if ($currentDate>$today) {
				...
				if (isSchoolOpen($guid, $currentDate, $connection2)==FALSE) {
					...
					if ($resultLog->rowCount()<1) {
						$data=array("pupilsightPersonIDTaker"=>$_SESSION[$guid]["pupilsightPersonID"], "pupilsightCourseClassID"=>$pupilsightCourseClassID, 'periodID' =>$periodID, "date"=>$currentDate, "timestampTaken"=>date("Y-m-d H:i:s"));
						$sql="INSERT INTO pupilsightAttendanceLogCourseClass SET pupilsightPersonIDTaker=:pupilsightPersonIDTaker, pupilsightCourseClassID=:pupilsightCourseClassID, periodID=:periodID, date=:date, timestampTaken=:timestampTaken" ;

					} else {
						$resultUpdate=$resultLog->fetch() ;
						$data=array("pupilsightAttendanceLogCourseClassID" => $resultUpdate['pupilsightAttendanceLogCourseClassID'], 'periodID' => $periodID, "pupilsightPersonIDTaker"=>$_SESSION[$guid]["pupilsightPersonID"], "pupilsightCourseClassID"=>$pupilsightCourseClassID, "date"=>$currentDate, "timestampTaken"=>date("Y-m-d H:i:s"));
						$sql="UPDATE pupilsightAttendanceLogCourseClass SET pupilsightPersonIDTaker=:pupilsightPersonIDTaker, pupilsightCourseClassID=:pupilsightCourseClassID, periodID=:periodID, date=:date, timestampTaken=:timestampTaken WHERE pupilsightAttendanceLogCourseClassID=:pupilsightAttendanceLogCourseClassID" ;
					}
