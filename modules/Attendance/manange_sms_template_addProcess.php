...
if (isActionAccessible($guid, $connection2, '/modules/Attendance/manange_sms_template_add.php') == false) {
    $URL .= '&return=error0';
    header("Location: {$URL}");
} else {
    //Proceed!
    ...
    if ($name == '' or $entities == '' or $description == '') {
        ...
        try {
            ...
            $sql = 'SELECT * FROM pupilsightTemplateForAttendance WHERE name=:name AND type=:type';
        ...
        if ($result->rowCount() > 0) {
            ...
            //Check for other currents
                ...
                try {
                    ...
                    $sql = "INSERT INTO pupilsightTemplateForAttendance SET name=:name, type=:type, status=:status, description=:description, entities=:entities, created_by=:created_by";
                    $result = $connection2->prepare($sql);
                    $result->execute($data);
                } catch (PDOException $e) {
                    $URL .= '&return=error9';
