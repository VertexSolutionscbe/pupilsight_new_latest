...
?>
	<form method="post" action="<?php print $_SESSION[$guid]["absoluteURL"] . "/modules/" . $_SESSION[$guid]["module"] . "/issues_createProccess.php" ?>">
		<table class='smallIntBorder' cellspacing='0' style="width: 100%">
			...
			if (getPermissionValue($connection2, $_SESSION[$guid]["pupilsightPersonID"], "createIssueForOther")) {
			?>
				<tr>
					<td>
					...
					<td class="right">
						<select name='createFor' id='createFor' style='width:302px'>
							...
							foreach ($allPeople as $row) {
								if (intval($row["pupilsightPersonID"]) != $_SESSION[$guid]["pupilsightPersonID"]) {
									print "<option value='" . $row["pupilsightPersonID"] . "'>" . $row['surname'] . ", " . $row['preferredName'] . "</option>";
								}
							}
							?>
						</select>
			...
			<tr>
				...
				<td class="right">
					...
					<input type="submit" class='btn btn-primary' value="<?php print __($guid, "Submit"); ?>">
				</td>
			</tr>
		</table>
	</form>
