...
if (isActionAccessible($guid, $connection2, "/modules/Help Desk/helpDesk_manageTechnicians.php") == FALSE) {
    ...
    try {
        ...
        $sql = "SELECT pupilsightPersonID FROM helpDeskTechnicians WHERE helpDeskTechnicians.technicianID=:technicianID" ;
    ...
    $row = $result->fetch();
    setLog($connection2, $_SESSION[$guid]["pupilsightSchoolYearID"], $pupilsightModuleID, $_SESSION[$guid]["pupilsightPersonID"], "Technician Removed", array("pupilsightPersonID" => $row['pupilsightPersonID']), null);
