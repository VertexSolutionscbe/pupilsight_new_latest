...
if (isActionAccessible($guid, $connection2, "/modules/Help Desk/helpDesk_settings.php") == FALSE) {
	...
	try {
		...
		$sql = "UPDATE pupilsightSetting SET value=:value WHERE scope='Help Desk' AND name='issuePriority'" ;
	...
	try {
		...
		$sql = "UPDATE pupilsightSetting SET value=:value WHERE scope='Help Desk' AND name='issueCategory'" ;
		$result = $connection2->prepare($sql);
		$result->execute($data);
	} catch (PDOException $e) { 
		$fails++;
	...
	try {
		...
		$sql = "UPDATE pupilsightSetting SET value=:value WHERE scope='Help Desk' AND name='issuePriorityName'" ;
		$result = $connection2->prepare($sql);
		$result->execute($data);
	} catch (PDOException $e) { 
		$fails++;
	...
	try {
		...
		$sql = "UPDATE pupilsightSetting SET value=:value WHERE scope='Help Desk' AND name='resolvedIssuePrivacy'" ;
		$result = $connection2->prepare($sql);
		$result->execute($data);
	} catch (PDOException $e) { 
		$fails++;
	...
	if ($fails == 4) {
		...
		setLog($connection2, $_SESSION[$guid]["pupilsightSchoolYearID"], $pupilsightModuleID, $_SESSION[$guid]["pupilsightPersonID"], "Help Desk Settings Edited", null);
