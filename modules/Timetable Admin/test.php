...
if (isActionAccessible($guid, $connection2, '/modules/Timetable Admin/tt.php') == false) {
    ...
    //Proceed!
    $page->breadcrumbs->add(__('My Test Page'));
    ...
    if ($pupilsightSchoolYearID != '') {
        ...
        $timetableGatewaytest = $container->get(TimetableGateway::class);
        $timetablestest = $timetableGatewaytest->testSelect();
        $testtable = DataTable::create('testtables');
        ...
        $testtable->addColumn('name', __('Name'));
        $testtable->addColumn('nameShort', __('Short Name'));
        $testtable->addColumn('active', __('Active'))->format(Format::using('yesNo', 'active'));
        
        // ACTIONS
        $testtable->addActionColumn()
        ...
        echo $testtable->render($timetablestest->toDataSet());

       
        
        //echo '<pre>';print_r($timetablestest->toDataSet());
