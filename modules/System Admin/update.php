...
if (isActionAccessible($guid, $connection2, '/modules/System Admin/update.php') == false) {
    ...
    echo '<p>';
    echo __('This page allows you to semi-automatically update your Pupilsight installation to a new version. You need to take care of the file updates, and based on the new files, Pupilsight will do the database upgrades.');
    echo '</p>';
    ...
    if ($cuttingEdgeCode != 'Y') {
        ...
        if ($return == 'success0') {
            ...
            echo '</h3>';
            echo '<ol>';
            echo '<li>'.sprintf(__('You are currently using Pupilsight v%1$s.'), $versionCode).'</i></li>';
            echo '<li>'.sprintf(__('Check %1$s for a newer version of Pupilsight.'), "<a target='_blank' href='http://pupilsight.in/download'>the Pupilsight download page</a>").'</li>';
            ...
            echo '</h3>';
            echo '<p>';
            echo sprintf(__('It seems that you have updated your Pupilsight code to a new version, and are ready to update your database from v%1$s to v%2$s. <b>Click "Submit" below to continue. This operation cannot be undone: backup your entire database prior to running the update!'), $versionDB, $versionCode).'</b>';
            echo '</p>';
        ...
        if ($return == 'success0') {
            ...
            echo '</h3>';
            echo '<ol>';
            echo '<li>'.sprintf(__('You are currently using Cutting Edge Pupilsight v%1$s'), $versionCode).'</i></li>';
            echo '<li>'.sprintf(__('Check %1$s to get the latest commits.'), "<a target='_blank' href='https://github.com/PupilsightEdu/core'>our GitHub repo</a>").'</li>';
            ...
            echo '</h3>';
            echo '<p>';
            echo sprintf(__('It seems that you have updated your Pupilsight code to a new version, and are ready to update your database from v%1$s line %2$s to v%3$s line %4$s. <b>Click "Submit" below to continue. This operation cannot be undone: backup your entire database prior to running the update!'), $versionDB, $cuttingEdgeCodeLine, $versionCode, $versionMaxLinesMax).'</b>';
            echo '</p>';
    ...
    if (version_compare($version, '16.0.00', '>=')) {
        ...
        echo '</h3>';
        echo '<p>';
        echo __('Starting from v16, Pupilsight is offering installations the option to migrate from MySQL\'s MyISAM engine to InnoDB, as a way to achieve greater reliability and performance.');
        echo '</p>';
