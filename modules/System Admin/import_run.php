...
if (isActionAccessible($guid, $connection2, "/modules/System Admin/import_run.php")==false) {
    ...
    if ($step==1) {
        ...
        $sql = "SELECT pupilsightLog.pupilsightLogID
                FROM pupilsightLog WHERE pupilsightLog.title = CONCAT('Import - ', :type) 
                ORDER BY pupilsightLog.timestamp DESC LIMIT 1" ;
        ...
        if ($usesDates) {
            $notes .= '<li>'.__("Dates are converted based on the separator used: American mm/dd/yy or dd/mm/yyyy, European dd.mm.yy, dd.mm.yyyy or dd-mm-yyyy. To avoid potential ambiguity, it's best to use ISO YYYY-MM-DD.");
    ...
    elseif ($step==2) {
        ...
        if ($importer->isValidMimeType($_FILES['file']['type']) == false) {
            ...
            if ($columnOrder == 'last') {
                $data = array('type' => $type);
                $sql = "SELECT * FROM pupilsightLog WHERE pupilsightLog.title = CONCAT('Import - ', :type) 
                        ORDER BY pupilsightLog.timestamp DESC LIMIT 1" ;
    ...
    elseif ($step==3 || $step==4) {
        ...
        if (empty($csvData) || empty($columnOrder)) {
            ...
            if ($step==4) {
                ...
                $importer->createImportLog($_SESSION[$guid]['pupilsightPersonID'], $type, $results, $columnOrder);
            }
        }
    }
}
