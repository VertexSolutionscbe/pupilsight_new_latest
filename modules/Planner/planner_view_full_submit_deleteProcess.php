...
if (isActionAccessible($guid, $connection2, '/modules/Planner/planner_view_full.php') == false) {
    ...
    if ($pupilsightPlannerEntryID == '' or $pupilsightPlannerEntryHomeworkID == '') {
        ...
        if ($result->rowCount() != 1) {
            ...
            //INSERT
            try {
                $data = array('pupilsightPlannerEntryHomeworkID' => $pupilsightPlannerEntryHomeworkID);
                $sql = 'DELETE FROM pupilsightPlannerEntryHomework WHERE pupilsightPlannerEntryHomeworkID=:pupilsightPlannerEntryHomeworkID';
                $result = $connection2->prepare($sql);
                $result->execute($data);
            } catch (PDOException $e) {
                $URL .= '&return=error2';
