                ...
                if ($result->rowCount() != 1) {
                    ...
                    if (($row['role'] == 'Student' and $row['viewableStudents'] == 'N') and ($highestAction == 'Lesson Planner_viewMyChildrensClasses' and $row['viewableParents'] == 'N')) {
                        ...
                        if ($row['role'] == 'Teacher') {
                            ...
                            echo "<a href='".$_SESSION[$guid]['absoluteURL']."/index.php?q=/modules/Planner/planner_edit.php&viewBy=$viewBy&pupilsightCourseClassID=$pupilsightCourseClassID&pupilsightPlannerEntryID=$pupilsightPlannerEntryID&date=".$row['date']."&subView=$subView'>".__('Edit')."<img style='margin: 0 0 -4px 3px' title='".__('Edit')."' src='./themes/".$_SESSION[$guid]['pupilsightThemeName']."/img/config.png'/></a> | ";
                            try {
                        ...
                        echo "<a href='".$_SESSION[$guid]['absoluteURL'].'/index.php?q=/modules/'.getModuleName($_GET['q'])."/planner_view_full.php$paramsVar#chat'>".__('Refresh')."<img style='margin-left: 5px' title='".__('Refresh')."' src='./themes/".$_SESSION[$guid]['pupilsightThemeName']."/img/refresh.png'/></a> <a href='".$_SESSION[$guid]['absoluteURL']."/index.php?q=/modules/Planner/planner_view_full_post.php&pupilsightPlannerEntryID=$pupilsightPlannerEntryID&viewBy=$viewBy&subView=$subView&pupilsightCourseClassID=$pupilsightCourseClassID&date=$date&search=".$pupilsightPersonID."'>".__('Add')."<img style='margin-left: 5px' title='".__('Add')."' src='./themes/".$_SESSION[$guid]['pupilsightThemeName']."/img/page_new.png'/></a> ";
                        echo '</div>';
