...
if (isActionAccessible($guid, $connection2, '/modules/Academics/assign_second_lang_class_add.php.php') != false) {
    //Acess denied
    ...
    $page->breadcrumbs
        ->add(__('Assign Second Language to class'), 'assign_second_lang_class_add.php', ['pupilsightSchoolYearID' => $pupilsightSchoolYearID])
        ->add(__('Assign Second Language to class'));
    ...
    if ($pupilsightSchoolYearID == '') {
        ...
        if ($search != '') {
            ...
            echo "<a href='".$_SESSION[$guid]['absoluteURL']."/index.php?q=/modules/Academics/assign_second_lang_class_add.php&pupilsightSchoolYearID=$pupilsightSchoolYearID&search=$search'>".__('Back to Search Results').'</a>';
            echo '</div>';
        }
...
//select subjects from department
     ...
     */
        ...
        $row = $form->addRow();
            $row->addFooter();
            $row->addSubmit()->addClass('submit_align submt assign_sec_sub');
        ...
        $departments = $departmentGateway->get_secondlang_assigned_toclass($criteria);
