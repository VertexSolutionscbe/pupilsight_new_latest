...
if (isActionAccessible($guid, $connection2, '/modules/Academics/sketch_manage_attribute.php') == false) {
    ...
    $sketchTempData = $result->fetch();

    $sketch_id = $sketchTempData['sketch_id'];

    $page->breadcrumbs
        ->add(__('Manage Sketch Template'), 'sketch_report_template_manage.php&id=' . $sketch_id . '')
        ->add(__('Edit Sketch Template'));
    ...
    if (isset($_GET['return'])) {
        ...
        $form->addHiddenValue('pupilsightSchoolYearID', $sketchTempData['pupilsightSchoolYearID']);
        $form->addHiddenValue('pupilsightProgramID', $sketchTempData['pupilsightProgramID']);

        $row = $form->addRow();
        $row->addLabel('name', __('Name'));
        $row->addTextField('name')->setValue($sketchTempData['name'])->required();

        $row = $form->addRow();
        $row->addLabel('pupilsightYearGroupID', __('Class'));
        $row->addSelect('pupilsightYearGroupID')->setId('pupilsightYearGroupIDbyPP')->fromArray($class)->required()->placeholder('Select Class')->selected($sketchTempData['pupilsightYearGroupID'])->readonly();
        ...
        $row = $form->addRow();
        $row->addLabel('', __(''));
        $row->addContent('<a href="public/sketch_template/'.$sketchTempData['template_filename'].'" download>'.$sketchTempData['template_filename'].'</a>');
