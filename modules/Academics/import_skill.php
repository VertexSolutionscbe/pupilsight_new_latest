...
$URL = $_SESSION[$guid]['absoluteURL'] . '/index.php?q=/modules/Academics/ac_manage_skill.php';
...
if (isActionAccessible($guid, $connection2, "/modules/Academics/ac_manage_skill.php") == false) {
    ...
    $page->breadcrumbs->add(__('Manage Skills'), 'ac_manage_skill.php')
    ->add('Skill Import', '');
    $form = Form::create('importStep1', $_SESSION[$guid]['absoluteURL'] . '/index.php?q=/modules/' . $_SESSION[$guid]['module'] . '/import_skill.php');
    ...
    if ($_POST) {
        ...
        foreach ($headers as $key => $hd) {
            ...
            if ($hd == 'Skill Name') {
                $headers[$key] = '##_name';
            } else if ($hd == 'Skill Code') {
                $headers[$key] = '##_code';
            } else if ($hd == 'Skill Description') {
                $headers[$key] = '##_description';
            } 
        }
        ...
        if (!empty($all_rows)) {
            ...
            try {
                ...
                foreach ($all_rows as  $alrow) {
                    try {
                        // Staff Entry
                        $sql = "INSERT INTO ac_manage_skill (";
