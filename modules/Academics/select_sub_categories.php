...
if (isActionAccessible($guid, $connection2, '/modules/Academics/select_sub_categories.php') != false) {
    ...
    foreach($subjectType as $k=>$sbt){
        $sql1 = "SELECT b.subject_type,b.subject_type_id,b.assesment_method,b.assesment_option,b.max_marks,b.min_marks,b.gradeSystemId FROM examinationTestSubjectCategory AS b WHERE b.test_id = ".$_GET['tid']." AND b.subject_type_id = ".$sbt['id']." ";
    ...
    // $sqlt = 'SELECT test_master_id FROM examinationTest WHERE id = '.$_GET['tid'].' ';
    // $resultt = $connection2->query($sqlt);
    // $tm = $resultt->fetch();
    // $tmId = $tm['test_master_id'];
    $tmId = $_GET['tid'];
