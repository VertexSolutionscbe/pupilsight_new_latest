...
if (isActionAccessible($guid, $connection2, '/modules/User Admin/import_userPhotos.php') == false) {
    ...
    if ($step == 1) {
        ...
        $form->addHiddenValue('pupilsightSchoolYearID', $_SESSION[$guid]['pupilsightSchoolYearID']);
    ...
    <?php
        ...
        if ($_FILES['file']['type'] != 'application/zip' and $_FILES['file']['type'] != 'application/x-zip-compressed') {
            ...
            $lockFail = false;
            try {
                $sql = 'LOCK TABLES pupilsightPerson WRITE';
                        ...
                        if (substr($zip->getNameIndex($i), 0, 8) != '__MACOSX') {
                            ...
                            if ($fileTypeFail == false) {
                                ...
                                try {
                                    ...
                                    $sql = 'SELECT username FROM pupilsightPerson WHERE username=:username';
                                ...
                                if ($userCheckFail) {
                                    ...
                                    if ($userCheckFail == false) {
                                        ...
                                        if ($fileUploadFail == false) {
                                            ...
                                            if ($imageFail == false) {
                                                //Update pupilsightPerson
                                                $updateFail = false;
                                                try {
                                                    $data = array('image_240' => $filePath, 'username' => $username);
                                                    $sql = 'UPDATE pupilsightPerson SET image_240=:image_240 WHERE username=:username';
                                                    $result = $connection2->prepare($sql);
                                                    $result->execute($data);
                                                } catch (PDOException $e) {
                                                    echo "<div class='alert alert-danger'>";
