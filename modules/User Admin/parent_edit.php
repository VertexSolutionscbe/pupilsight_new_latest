...
if (isActionAccessible($guid, $connection2, '/modules/User Admin/parent_edit.php') == false) {
    ...
    if ($pupilsightPersonID == '') {
        ...
        try {
			...
			$data = array('pupilsightPersonID' => $pupilsightPersonID);
            $sqlp = 'SELECT * FROM pupilsightFamilyRelationship WHERE pupilsightPersonID1=:pupilsightPersonID';
        ...
        if ($result->rowCount() != 1) {
			...
			foreach($students as $stu){
				echo "<div style='height:50px;'><div class='float-left mb-2'><a href='".$_SESSION[$guid]['absoluteURL']."/index.php?q=/modules/User Admin/student_edit.php&pupilsightPersonID=".$stu['pupilsightPersonID2']."&search=' class='btn btn-primary '>Student</a>";  
			}
			echo "&nbsp;&nbsp;<a href='".$_SESSION[$guid]['absoluteURL']."/index.php?q=/modules/User Admin/parent_edit.php&pupilsightPersonID=".$pupilsightPersonID."&search=' class='btn btn-primary active'>Parent</a></div><div class='float-none'></div></div>"; 
			

			$form = Form::create('addUser', $_SESSION[$guid]['absoluteURL'].'/modules/'.$_SESSION[$guid]['module'].'/parent_editProcess.php?pupilsightPersonID='.$pupilsightPersonID.'&search='.$search);
