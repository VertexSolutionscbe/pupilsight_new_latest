 ...
 ?>
        ...
        <td class="w-full  px-2 border-b-0 sm:border-b border-t-0 newdes">
			<div class="input-group stylish-input-group">
                <div class=" mb-1" style="width:100%"><label for="invoice_title" class="inline-block sm:my-1 sm:max-w-xs font-bold text-sm sm:text-xs">Fee Structure </label></div>
                <?php foreach($rowdatacls as $k => $cl){ 
                    if(!empty($cl['fsid'])){
                        ...
                        $fid = $chkstrid['fid'];
                        if(!empty($fid)){
                            $sqlf = 'SELECT id, name FROM fn_fee_structure WHERE id IN ('.$cl['fsid'].') AND id NOT IN ('.$fid.') ';
                        } else {
                            $sqlf = 'SELECT id, name FROM fn_fee_structure WHERE id IN ('.$cl['fsid'].') ';
                        }
                        //echo $sqlf;
