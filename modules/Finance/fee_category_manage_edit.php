...
if (isActionAccessible($guid, $connection2, '/modules/Finance/fee_category_manage_edit.php') == false) {
    ...
    //Proceed!
    $page->breadcrumbs
        ->add(__('Manage Category'), 'fee_category_manage.php')
    ...
    if ($id == '') {
        ...
        if ($result->rowCount() != 1) {
            ...
            echo '<h2>';
            echo __('Edit Fee Category');
            echo '</h2>';

            $form = Form::create('program', $_SESSION[$guid]['absoluteURL'].'/modules/'.$_SESSION[$guid]['module'].'/fee_category_manage_editProcess.php?id='.$id);
            ...
            //     $row->addSequenceNumber('sequenceNumber', 'fee_category', $values['sequenceNumber'])
            //         ->required()
            //         ->maxLength(3)
