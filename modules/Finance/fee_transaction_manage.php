...
if (
    ...
    $paymentmode = [];
    $paymentmode1 = ["" => "Select Payment Mode"];
    $paymentmode2 = [];
    ...
    if (
        $pupilsightSchoolYearID == "" or
        $pupilsightSchoolYearID == $_SESSION[$guid]["pupilsightSchoolYearID"]
    ) {
        $pupilsightSchoolYearID = $_SESSION[$guid]["pupilsightSchoolYearID"];
        $pupilsightSchoolYearName =
            $_SESSION[$guid]["pupilsightSchoolYearName"];
    }
    ...
    if ($_POST) {
        ...
        if (!empty($pupilsightProgramID)) {
            $classes = $HelperGateway->getClassByProgram(
                $connection2,
                $pupilsightProgramID,
                $pupilsightSchoolYearID
            );
            if (!empty($pupilsightYearGroupID)) {
                $sections = $HelperGateway->getMultipleSectionByProgram(
                    $connection2,
    ...
    if ($_POST) {
        $feeTransaction = $FeesGateway->getFeesTransaction(
            $criteria,
            $input,
            $pupilsightSchoolYearID
        );

        $c_query = $FeesGateway->getFeesTransactiontotal(
            $criteria,
            $input,
            $pupilsightSchoolYearID
        );
    ...
    $col->addSelect("pupilsightYearGroupID")
        ->setId("showMultiClassByProg")
        ->fromArray($classes)
        ->selected($pupilsightYearGroupID)
        ->selectMultiple();

    $col = $row->addColumn()->setClass("newdes");
    $col->addLabel("[pupilsightRollGroupID", __("Section"))->addClass("dte");
    $col->addSelect("pupilsightRollGroupID")
        ->setId("showMultiSecByProgCls")
        ->fromArray($sections)
        ->selected($pupilsightRollGroupID)
        ->selectMultiple();

    $col = $row->addColumn()->setClass("newdes");
    $col->addLabel("admission_no", __("Admission No"));
    $col->addTextField("admission_no")
        ->setValue($admission_no)
        ->addClass("txtfield");

    $col = $row->addColumn()->setClass("newdes");
    $col->addLabel("student_name", __("Student Name"));
    $col->addTextField("student_name")
        ->setValue($student_name)
        ->addClass("txtfield");

    $col = $row->addColumn()->setClass("newdes");
    $col->addLabel("receipt_number", __("Receipt No"));
    $col->addTextField("receipt_number")
    ...
    $col->addTextField("transaction_id")
    ...
    $col->addSelect("bank_id")
        ->selected($bank_id)
        ->fromArray($bank);

    $col = $row->addColumn()->setClass("newdes");
    $col->addLabel("payment_mode_id", __("Payment Mode"));
    $col->addSelect("payment_mode_id")
        ->selected($payment_mode_id)
        ->fromArray($paymentmode);

    $col = $row->addColumn()->setClass("newdes");
    $col->addLabel("payment_status", __("Transaction Status"));
    $col->addSelect("payment_status")
        ->selected($payment_status)
        ->fromArray($tran_status);
    ...
    $col->addLabel(" ", __(" "));
    $col->addContent(
        '<button id="searchTransaction" class=" btn btn-primary">Search</button>'
    );
    ...
    $col->addLabel(
        "",
        __("Total Transaction Amount: " . number_format($t_amount, 2))
    );
    ...
    $table
        ->addColumn("payment_date", __("Payment Date"))
    ...
    $table
        ->addColumn("print", __("Print Receipt"))
        ->format(function ($feeTransaction) {
            if (!empty($feeTransaction["filename"])) {
                return '<a href="public/receipts/' .
                    $feeTransaction["filename"] .
                    '.pdf"  download><i class="mdi mdi-receipt mdi-24px"></i></a>';
            } elseif (!empty($feeTransaction["transaction_id"])) {
                return '<a href="public/receipts/' .
                    $feeTransaction["transaction_id"] .
                    '.pdf"  download><i class="mdi mdi-receipt mdi-24px"></i></a>';
            } else {
                return "Stoped";
            }
