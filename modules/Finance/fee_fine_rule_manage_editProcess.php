        ...
        if ($result->rowCount() != 1) {
            ...
            if ($name == ''  or $code == '' ) {
                ...
                try {
                    ...
                    $sql = 'SELECT * FROM fn_fees_fine_rule WHERE (name=:name OR code=:code) AND NOT id=:id';
                    $result = $connection2->prepare($sql);
                    $result->execute($data);
                } catch (PDOException $e) {
                    $URL .= '&return=error2';
                ...
                if ($result->rowCount() > 0) {
                    ...
                    //Write to database
                    try {
                        $data = array('name' => $name, 'code' => $code, 'description' => $description, 'fine_type' => $fine_type, 'rule_type' => $fixed_fine_type, 'is_fine_editable' => $is_fine_editable, 'udt' => $udt, 'id' => $id);
                        $sql = 'UPDATE fn_fees_fine_rule SET name=:name, code=:code, description=:description, fine_type=:fine_type,rule_type=:rule_type, is_fine_editable=:is_fine_editable, udt=:udt WHERE id=:id';
                        $result = $connection2->prepare($sql);
                        $result->execute($data);

                        $datad = array('fn_fees_fine_rule_id' => $id);
                        $sqld = 'DELETE FROM fn_fees_rule_type WHERE fn_fees_fine_rule_id=:fn_fees_fine_rule_id';
