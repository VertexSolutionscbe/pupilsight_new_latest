...
if (isActionAccessible($guid, $connection2, '/modules/Finance/expenses_manage_add.php', 'Manage Expenses_all') == false) {
    ...
    if ($allowExpenseAdd != 'Y') {
        ...
        if (isset($_GET['editID'])) {
            $editLink = $_SESSION[$guid]['absoluteURL'].'/index.php?q=/modules/Finance/expenses_manage_edit.php&pupilsightFinanceExpenseID='.$_GET['editID'].'&pupilsightFinanceBudgetCycleID='.$_GET['pupilsightFinanceBudgetCycleID'].'&status2='.$_GET['status2'].'&pupilsightFinanceBudgetID2='.$_GET['pupilsightFinanceBudgetID2'];
        }
        ...
        if ($pupilsightFinanceBudgetCycleID == '') {
			...
			$row = $form->addRow();
				...
				$row->addSelect('pupilsightFinanceBudgetID')->fromQuery($pdo, $sql)->required()->placeholder();
