...
if (isActionAccessible($guid, $connection2, '/modules/User Admin/parent_add.php') == false) {
    ...
    if (isset($_GET['editID'])) {
        $editLink = $_SESSION[$guid]['absoluteURL'].'/index.php?q=/modules/User Admin/parent_edit.php&pupilsightPersonID='.$_GET['editID'].'&search='.$_GET['search'];
    }
    ...
    // echo "<div style='height:50px;'><div class='float-left mb-2'><a href='".$_SESSION[$guid]['absoluteURL']."/index.php?q=/modules/User Admin/student_add.php' class='btn btn-primary'>Student</a>";  
    // echo "&nbsp;&nbsp;<a href='".$_SESSION[$guid]['absoluteURL']."/index.php?q=/modules/User Admin/parent_add.php' class='btn btn-primary active'>Parent</a></div><div class='float-none'></div></div>"; 

    $form = Form::create('addUser', $_SESSION[$guid]['absoluteURL'].'/modules/'.$_SESSION[$guid]['module'].'/parent2_addProcess.php?search='.$search);
    $form->setFactory(DatabaseFormFactory::create($pdo));

    $form->addHiddenValue('address', $_SESSION[$guid]['address']);
    $form->addHiddenValue('student_id', $studentId);
    ...
    $data = array();
    $sql = "SELECT * FROM pupilsightRole WHERE pupilsightRoleID = '004' ORDER BY name";
