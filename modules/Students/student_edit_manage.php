...
if (isActionAccessible($guid, $connection2, '/modules/User Admin/student_edit.php') == false) {
	...
	if ($pupilsightPersonID == '') {
		...
		if ($result->rowCount() != 1) {
			...
			$row = $form->addRow("primary_role_div");
			...
			$row = $form->addRow("surname_div");
			$row->addLabel('surname', __('Surname'))->description(__('Family name as shown in ID documents.'));
			$row->addTextField('surname')->maxLength(60);

			$row = $form->addRow("firstName_div");
			...
			$row = $form->addRow("officialName_div");
			...
			$row = $form->addRow("gender_div");
			$row->addLabel('gender', __('Gender'));
			$row->addSelectGender('gender');

			$row = $form->addRow("dob_div");
			$row->addLabel('dob', __('Date of Birth'));
			$row->addDate('dob');

			$row = $form->addRow("user_photo_div");
			...
			if (!empty($restrictedRoles)) {
				...
				$row = $form->addRow("restricted_roles_div");
			...
			$row = $form->addRow("canLogin_div");
			$row->addLabel('canLogin', __('Can Login?'));
			$row->addYesNo('canLogin')->required();

			$row = $form->addRow("passwordForceReset_div");
			...
			$row = $form->addRow("showAddresses_div");
			...
			$row = $form->addRow('address1Country_div')->addClass('address');
			...
			if ($values['address1'] != '') {
				...
				if ($resultAddress->rowCount() > 0) {
					...
					$row = $form->addRow('matchAddress_div')->addClass('address  matchHighlight');
			...
			$row = $form->addRow('address2Country_div')->addClass('address');
			...
			if ($student || $staff) {
				...
				$row = $form->addRow("nextSchool_div");
				...
				$row = $form->addRow("departureReason_div");
			...
			$row = $form->addRow("citizenship1_div");
			...
			$row = $form->addRow("citizenship2_div");
			...
			$row = $form->addRow("citizenship2Passport_div");
			...
			$row = $form->addRow("residencyStatus_div");
			...
			$row = $form->addRow("visaExpiryDate_div");
			...
			if ($student) {
				...
				if (!empty($studentAgreementOptions)) {
					...
					$row = $form->addRow("studentAgreements_div");
