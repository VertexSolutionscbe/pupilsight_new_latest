...
if (isActionAccessible($guid, $connection2, '/modules/Students/report_emergencySMS_byYearGroup.php') == false) {
    ...
    $row = $form->addRow();
        ...
        $row->addSelectYearGroup('pupilsightYearGroupID', true)->required()->selected($pupilsightYearGroupID);
    ...
    if ($pupilsightYearGroupID != '') {
        ...
        try {
            if ($pupilsightYearGroupID == '*') {
                ...
                $data = array('pupilsightSchoolYearID' => $_SESSION[$guid]['pupilsightSchoolYearID'], 'pupilsightYearGroupID' => $pupilsightYearGroupID);
                $sql = "SELECT surname, preferredName, pupilsightPerson.pupilsightPersonID, emergency1Number1, emergency2Number1 FROM pupilsightPerson JOIN pupilsightStudentEnrolment ON (pupilsightPerson.pupilsightPersonID=pupilsightStudentEnrolment.pupilsightPersonID) WHERE status='Full' AND (dateStart IS NULL OR dateStart<='".date('Y-m-d')."') AND (dateEnd IS NULL  OR dateEnd>='".date('Y-m-d')."') AND pupilsightStudentEnrolment.pupilsightYearGroupID=:pupilsightYearGroupID AND pupilsightStudentEnrolment.pupilsightSchoolYearID=:pupilsightSchoolYearID ORDER BY surname, preferredName";
            }
