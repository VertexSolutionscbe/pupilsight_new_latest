...
if (isActionAccessible($guid, $connection2, '/modules/Students/medicalForm_manage_condition_add.php') == false) {
    ...
    if (isset($_GET['editID'])) {
        $editLink = $_SESSION[$guid]['absoluteURL'].'/index.php?q=/modules/Students/medicalForm_manage_condition_edit.php&'.
            http_build_query([
                'pupilsightPersonMedicalConditionID' => $editID,
                'search' => $search,
                'pupilsightPersonMedicalID' => $pupilsightPersonMedicalID,
            ]);
    }
    ...
    if ($pupilsightPersonMedicalID == '') {
        ...
        if (empty($values)) {
            ...
            $form = Form::create('action', $_SESSION[$guid]['absoluteURL'].'/modules/'.$_SESSION[$guid]['module']."/medicalForm_manage_condition_addProcess.php?pupilsightPersonMedicalID=$pupilsightPersonMedicalID&search=$search");
