...
if (isActionAccessible($guid, $connection2, '/modules/Students/report_student_emergencySummary.php') == false) {
    ...
    if (count($choices) > 0) {
        ...
        echo "<a target='_blank' href='".$_SESSION[$guid]['absoluteURL'].'/report.php?q=/modules/'.$_SESSION[$guid]['module']."/report_student_emergencySummary_print.php'>".__('Print')."<img style='margin-left: 5px' title='".__('Print')."' src='./themes/".$_SESSION[$guid]['pupilsightThemeName']."/img/print.png'/></a>";
        echo '</div>';
        ...
        while ($row = $result->fetch()) {
            ...
            while ($rowFamily = $resultFamily->fetch()) {
                try {
                    ...
                    $sqlFamily2 = 'SELECT * FROM pupilsightPerson JOIN pupilsightFamilyAdult ON (pupilsightPerson.pupilsightPersonID=pupilsightFamilyAdult.pupilsightPersonID) WHERE pupilsightFamilyID=:pupilsightFamilyID ORDER BY contactPriority, surname, preferredName';
