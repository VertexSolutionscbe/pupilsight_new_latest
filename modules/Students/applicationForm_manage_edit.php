...
if (isActionAccessible($guid, $connection2, '/modules/Students/applicationForm_manage_edit.php') == false) {
    ...
    try {
        ...
        $sql = "SELECT *, pupilsightApplicationForm.status AS 'applicationStatus', pupilsightPayment.status AS 'paymentStatus' FROM pupilsightApplicationForm LEFT JOIN pupilsightPayment ON (pupilsightApplicationForm.pupilsightPaymentID=pupilsightPayment.pupilsightPaymentID AND foreignTable='pupilsightApplicationForm') WHERE pupilsightApplicationFormID=:pupilsightApplicationFormID";
    ...
    echo "<a target='_blank' href='".$_SESSION[$guid]['absoluteURL'].'/report.php?q=/modules/'.$_SESSION[$guid]['module']."/applicationForm_manage_edit_print.php&pupilsightApplicationFormID=$pupilsightApplicationFormID'>".__('Print')."<img style='margin-left: 5px' title='".__('Print')."' src='./themes/".$_SESSION[$guid]['pupilsightThemeName']."/img/print.png'/></a>";
    echo '</div>';
    ...
    $form->addHiddenValue('pupilsightApplicationFormID', $application['pupilsightApplicationFormID']);
    ...
    $row = $form->addRow();
        $row->addLabel('pupilsightApplicationFormID', __('Application ID'));
        $row->addTextField('pupilsightApplicationFormID')->readOnly();
    ...
    $row = $form->addRow();
        $row->addLabel('pupilsightSchoolYearIDEntry', __('Year of Entry'))->description(__('When will the student join?'));
        $row->addSelectSchoolYear('pupilsightSchoolYearIDEntry', 'Active')->required();

    // YEAR GROUP
    $row = $form->addRow();
        $row->addLabel('pupilsightYearGroupIDEntry', __('Year Group at Entry'))->description(__('Which year level will student enter.'));
        $row->addSelectYearGroup('pupilsightYearGroupIDEntry')->required();

    // ROLL GROUP
    $sqlSelect = "SELECT pupilsightRollGroupID as value, name, pupilsightSchoolYearID FROM pupilsightRollGroup ORDER BY pupilsightSchoolYearID, name";
    $resultSelect = $pdo->executeQuery(array(), $sqlSelect);

    $rollGroups = ($resultSelect->rowCount() > 0)? $resultSelect->fetchAll() : array();
    $rollGroupsChained = array_combine(array_column($rollGroups, 'value'), array_column($rollGroups, 'pupilsightSchoolYearID'));
    $rollGroupsOptions = array_combine(array_column($rollGroups, 'value'), array_column($rollGroups, 'name'));

    $row = $form->addRow();
        $row->addLabel('pupilsightRollGroupID', __('Roll Group at Entry'))->description(__('If set, the student will automatically be enroled on Accept.'));
        $row->addSelect('pupilsightRollGroupID')
            ->fromArray($rollGroupsOptions)
            ->chainedTo('pupilsightSchoolYearIDEntry', $rollGroupsChained)
            ->placeholder();
    ...
    $data = array( 'pupilsightApplicationFormID' => $application['pupilsightApplicationFormID'] );
    $sql = "SELECT DISTINCT pupilsightApplicationFormID, preferredName, surname, status FROM pupilsightApplicationForm
            ...
            WHERE pupilsightApplicationFormID <> :pupilsightApplicationFormID
    ...
    if ($resultLinked && $resultLinked->rowCount() > 0) {
        ...
        $row->addContent();
        $row->addContent();
        $row->addContent("<a href='#' onclick='if (confirm(\"".$messageDelete."\")) window.location = \"".$_SESSION[$guid]['absoluteURL'].'/modules/'.$_SESSION[$guid]['module'].'/applicationForm_manage_deleteLinkProcess.php?pupilsightApplicationFormID='.$pupilsightApplicationFormID."&pupilsightSchoolYearID=".$pupilsightSchoolYearID."\"; else return false;'><img style='margin-left: 4px' title='".__('Remove').' '.__('Sibling Applications')."' src='./themes/".$_SESSION[$guid]['pupilsightThemeName']."/img/garbage.png'/></a>")->addClass('right');

    } else {
        // Or add a new link (mutually exclusive, to prevent linking multiple families)
        $data = array();
        $sql = "SELECT pupilsightApplicationFormID, surname, preferredName, pupilsightApplicationForm.status, pupilsightSchoolYearID, pupilsightSchoolYear.name as schoolYearName FROM pupilsightApplicationForm JOIN pupilsightSchoolYear ON (pupilsightApplicationForm.pupilsightSchoolYearIDEntry=pupilsightSchoolYear.pupilsightSchoolYearID) LEFT JOIN pupilsightYearGroup ON (pupilsightApplicationForm.pupilsightYearGroupIDEntry=pupilsightYearGroup.pupilsightYearGroupID) WHERE pupilsightApplicationForm.pupilsightSchoolYearIDEntry >= (SELECT pupilsightSchoolYearID from pupilsightSchoolYear WHERE status='Current') ORDER BY pupilsightSchoolYearID, surname, preferredName";
        ...
        if (isset($resultApplications) && $resultApplications->rowCount() > 0) {
            ...
            $linkedApplications = array_reduce($resultApplications->fetchAll(), function($applications, $item) {
                ...
                $value = $item['pupilsightApplicationFormID'];
            ...
            $row = $form->addRow();
                ...
                $row->addSelect('linkedApplicationFormID')
                    ...
                    ->append("<input type='submit' style='float:right;margin-left: 183px;' value='".__('Go')."' onclick='if(confirm(\"".$messageConfirm."\")) document.forms[0].submit(); else return false;'>");
        }
    }
    ...
    if (empty($application['pupilsightFamilyID'])) {
        ...
        if (!empty($application['parent1pupilsightPersonID']) ) {
            ...
            $email = $form->addHiddenValue('parent1pupilsightPersonID', $application['parent1pupilsightPersonID']);
        ...
        for ($i = $start; $i < 3; ++$i) {
            ...
            if ($i == 2) {
                $checked = (!empty($application['parent2pupilsightPersonID']) || !empty($application['parent2surname']))? 'Yes' : 'No';
        ...
        $form->addHiddenValue('pupilsightFamilyID', $application['pupilsightFamilyID']);

        $row = $form->addRow();
            $row->addHeading(__('Family'))->append(sprintf(__('The applying family is already a member of %1$s.'), $_SESSION[$guid]['organisationName']));

        $dataFamily = array('pupilsightFamilyID' => $application['pupilsightFamilyID']);
        ...
        if ($resultFamily->rowCount() != 1) {
            $proceed = false;
            $form->addRow()->addTextField('pupilsightFamilyError')->readOnly()->setValue(__('There is an error with this form!'));
        } else {
            ...
            try {
                $dataRelationships = array('pupilsightApplicationFormID' => $pupilsightApplicationFormID);
                $sqlRelationships = 'SELECT surname, preferredName, title, gender, pupilsightApplicationFormRelationship.pupilsightPersonID, relationship FROM pupilsightApplicationFormRelationship JOIN pupilsightPerson ON (pupilsightApplicationFormRelationship.pupilsightPersonID=pupilsightPerson.pupilsightPersonID) WHERE pupilsightApplicationFormRelationship.pupilsightApplicationFormID=:pupilsightApplicationFormID';
    ...
    if (!empty($requiredDocuments)) {
        ...
        for ($i = 0; $i < count($requiredDocumentsList); $i++) {
            ...
            $dataFile = array('pupilsightApplicationFormID' => $pupilsightApplicationFormID, 'name' => $requiredDocumentsList[$i]);
            $sqlFile = "SELECT CONCAT('attachment[', pupilsightApplicationFormFileID, ']') as id, path FROM pupilsightApplicationFormFile WHERE pupilsightApplicationFormID=:pupilsightApplicationFormID AND name=:name ORDER BY pupilsightApplicationFormFileID DESC";
