...
if (isActionAccessible($guid, $connection2, '/modules/Transport/transport_route_edit.php') == false) {
    ...
    $bus_id = $bus_name1 + $bus_name2;
            ...
            echo '<h2>';
            echo __('Edit Bus Route');
            echo '</h2>';

           
            $form = Form::create('program', $_SESSION[$guid]['absoluteURL'].'/modules/'.$_SESSION[$guid]['module'].'/transport_route_editProcess.php?id='.$id);
            ...
            $col->addSelect('bus_id')->fromArray($bus_id)->selected($values['bus_id']);

            $col = $row->addColumn()->setClass('newdes');
            $col->addLabel('type', __('Type'));
            $col->addSelect('type')->fromArray($type)->required()->selected($values['type']);     
            ...
            $row = $form->addRow();
                    ...
                    $col->addTextField('end_point')->addClass('txtfield')->required()->setValue($values['end_point']);
                 ...
                 if(!empty($childvalues)){
                    $i = 1;
                       foreach($childvalues as $cv){
                        ...
                        $col->addTextField('stop_no['.$cv['id'].']')->addClass('txtfield kountseat numfield')->required()->setValue($cv['stop_no']);
            
                        $col = $row->addColumn()->setClass('newdes nobrdbtm');
                        if($i == '1'){
                        $col->addLabel('stop_name', __('Stop Name'))->addClass('dte');
                        }
                        $col->addTextField('stop_name['.$cv['id'].']')->addClass('txtfield kountseat')->required()->setValue($cv['stop_name']);
                
                        $col = $row->addColumn()->setClass('newdes nobrdbtm');
                        if($i == '1'){
                        $col->addLabel('pickup_time', __('Pick Up Time'))->addClass('dte');
                        }
                        $col->addTextField('pickup_time['.$cv['id'].']')->addClass('txtfield')->required()->setValue($cv['pickup_time']);   
