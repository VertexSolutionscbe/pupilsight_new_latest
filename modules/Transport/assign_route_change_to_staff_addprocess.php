...
$URL = $_SESSION[$guid]['absoluteURL'].'/index.php?q=/modules/'.getModuleName($_POST['address']).'/assign_staff_route_manage.php';

if (isActionAccessible($guid, $connection2, '/modules/Transport/assign_route_change_to_staff_add.php') == false) {
    $URL .= '&return=error0';
    header("Location: {$URL}");
} else {
    //Proceed!
            ...
            if($type == 'both'){
                foreach($stu_id as $stu){
                    ...
                    if($result->rowCount() == 0 && $_POST['onward_rt_tway'] != '' && $_POST['onward_sp_tway'] != ''){
                        $data1 = array('status' =>$status, 'pupilsightPersonID' => $stu,'pupilsightSchoolYearID' =>$academic_year,'route_id'=>$_POST['onward_rt_tway'],'route_stop_id' =>$_POST['onward_sp_tway'],'type' => 'onward');
                    
                        $sql1 = 'INSERT INTO trans_route_assign SET status=:status,pupilsightPersonID=:pupilsightPersonID , pupilsightSchoolYearID=:pupilsightSchoolYearID,route_id=:route_id,route_stop_id=:route_stop_id,type=:type';
                    ...
                    if($result2->rowCount() == 0 && $_POST['return_rt_tway'] != '' && $_POST['return_sp_tway'] != ''){
                        $data3 = array('status' =>$status, 'pupilsightPersonID' => $stu,'pupilsightSchoolYearID' =>$academic_year,'route_id'=>$_POST['return_rt_tway'],'route_stop_id' =>$_POST['return_sp_tway'],'type' => 'return');
                    
                        $sql3 = 'INSERT INTO trans_route_assign SET status=:status,pupilsightPersonID=:pupilsightPersonID , pupilsightSchoolYearID=:pupilsightSchoolYearID,route_id=:route_id,route_stop_id=:route_stop_id,type=:type';
                ...
                foreach($stu_id as $stu){
                    ...
                    if($result->rowCount() == 0){
                        $data1 = array('status' =>$status, 'pupilsightPersonID' => $stu,'pupilsightSchoolYearID' =>$academic_year,'route_id'=>$routeId,'route_stop_id' =>$stopId,'type' => $type);
