...
$URL = $_SESSION[$guid]['absoluteURL'].'/index.php?q=/modules/Transport/view_members_in_route.php';

if (isActionAccessible($guid, $connection2, '/modules/Transport/send_route_email_msg.php') == false) {
    $URL .= '&return=error0';
    header("Location: {$URL}");
} else {
    //Proceed!
        ...
        foreach($studentId as $st){
           ...
           // echo $sub;
            ...
            if(!empty($body) && !empty($to)){ 
                $url = $_SESSION[$guid]['absoluteURL'].'/index.php?q=/modules/Transport/mailsend.php';
                $url .="&to=".$to;
