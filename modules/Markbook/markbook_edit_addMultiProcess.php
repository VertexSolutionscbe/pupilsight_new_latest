...
$URL = $_SESSION[$guid]['absoluteURL'].'/index.php?q=/modules/'.getModuleName($_GET['address'])."/markbook_edit_addMulti.php&pupilsightCourseClassID=$pupilsightCourseClassID";
...
if (isActionAccessible($guid, $connection2, '/modules/Markbook/markbook_edit_addMulti.php') == false) {
    ...
    if (empty($_POST)) {
        ...
        //Get next groupingID
        try {
            $sqlGrouping = 'SELECT DISTINCT groupingID FROM pupilsightMarkbookColumn WHERE NOT groupingID IS NULL ORDER BY groupingID DESC';
        ...
        if (count($pupilsightCourseClassIDMulti) < 1 or is_numeric($groupingID) == false or $groupingID < 1 or $name == '' or $description == '' or $type == '' or $date == '' or $viewableStudents == '' or $viewableParents == '') {
            $URL .= '&return=error1';
            header("Location: {$URL}");
        } else {
            $partialFail = false;
            ...
            foreach ($pupilsightCourseClassIDMulti as $pupilsightCourseClassIDSingle) {
                ...
                try {
                    $dataSequence = array('pupilsightCourseClassID' => $pupilsightCourseClassIDSingle);
