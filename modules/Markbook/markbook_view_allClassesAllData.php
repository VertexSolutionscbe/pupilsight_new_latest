...
use Pupilsight\Domain\Planner\PlannerEntryGateway;
use Pupilsight\Domain\Markbook\MarkbookColumnGateway;
...
use Pupilsight\Services\Format;
    ...
    if ($pupilsightCourseClassID == '') {
        ...
        if ($multiAdd) {
            ...
            echo "<a class = 'fw-btn-fill btn btn-primary addbtncss' style='    font-size: 18px;width: 85px !important;width:auto !important' href='".$_SESSION[$guid]['absoluteURL'].'/index.php?q=/modules/'.$_SESSION[$guid]['module']."/markbook_edit_addMulti.php&pupilsightCourseClassID=$pupilsightCourseClassID'>".__('Add')."</a>";
            echo '</div>';
        }
        //Get class chooser
    ...
    $_SESSION[$guid]['markbookClass'] = $pupilsightCourseClassID;
    ...
    $pupilsightYearGroupIDList = $class['pupilsightYearGroupIDList'];
    ...
    if ($multiAdd) {
        echo "<div class='linkTop'>";
        echo "<a href='".$_SESSION[$guid]['absoluteURL'].'/index.php?q=/modules/'.$_SESSION[$guid]['module']."/markbook_edit_addMulti.php&pupilsightCourseClassID=$pupilsightCourseClassID'>".__('Add Multiple Columns')."<img title='".__('Add Multiple Columns')."' src='./themes/".$_SESSION[$guid]['pupilsightThemeName']."/img/page_new_multi.png'/></a>";
        echo '</div>';
    }
	...
	$canEditThisClass = (isset($teacherList[ $_SESSION[$guid]['pupilsightPersonID'] ]) || $highestAction2 == 'Edit Markbook_everything');

    // Get criteria filter values, including session defaults
    $search = $_GET['search'] ?? '';
    $pupilsightSchoolYearTermID = $_GET['pupilsightSchoolYearTermID'] ?? $_SESSION[$guid]['markbookTerm'] ?? '';
	...
	if(isset($_GET['reset']) && $_GET['reset']==1){
		...
		$sql = 'SET @count:=0;UPDATE pupilsightMarkbookColumn SET `sequenceNumber`=@count:=@count+1 WHERE `pupilsightCourseClassID` = :pupilsightCourseClassID order by pupilsightMarkbookColumnID ASC';
    ...
    $criteria = $markbookGateway->newQueryCriteria()
        ...
        ->sortBy(['pupilsightMarkbookColumn.sequenceNumber', 'pupilsightMarkbookColumn.date', 'pupilsightMarkbookColumn.complete', 'pupilsightMarkbookColumn.completeDate'])
    ...
    $columns = $markbookGateway->queryMarkbookColumnsByClass($criteria, $pupilsightCourseClassID);
    $columns->transform(function (&$column) use ($plannerGateway) {
        if (isset($column['pupilsightPlannerEntryID'])) {
            $column['pupilsightPlannerEntry'] = $plannerGateway->getPlannerEntryByID($column['pupilsightPlannerEntryID']);
        }
    });
    ...
    if ($markbook == NULL || $markbook->getColumnCountTotal() < 1) {
        ...
        if (isActionAccessible($guid, $connection2, '/modules/Markbook/markbook_edit.php') and $canEditThisClass) {
            echo "<a href='".$_SESSION[$guid]['absoluteURL'].'/index.php?q=/modules/'.$_SESSION[$guid]['module']."/markbook_edit_add.php&pupilsightCourseClassID=$pupilsightCourseClassID'>".__('Add')."<img title='".__('Add')."' src='./themes/".$_SESSION[$guid]['pupilsightThemeName']."/img/page_new.png'/></a>";
			if ($markbook->getSetting('enableColumnWeighting') == 'Y') {
	            if (isActionAccessible($guid, $connection2, '/modules/Markbook/weighting_manage.php') == true) {
	                echo " | <a href='".$_SESSION[$guid]['absoluteURL'].'/index.php?q=/modules/'.$_SESSION[$guid]['module']."/weighting_manage.php&pupilsightCourseClassID=$pupilsightCourseClassID'>".__('Weightings')."<img title='".__('Weightings')."' src='./themes/".$_SESSION[$guid]['pupilsightThemeName']."/img/run.png'/></a>";
	            }
	        }
		}
        echo '</div>';
        ...
        $markbook->cachePersonalizedTargets( $pupilsightCourseClassID );
        ...
        if (isActionAccessible($guid, $connection2, '/modules/Formal Assessment/externalAssessment_details.php')) {
            $markbook->cacheExternalAssessments( $courseName, $pupilsightYearGroupIDList );
        }
        	...
        	echo "<div style='padding-top: 16px; margin-right: 10px; text-align: left; width: 300px; float: left;'>";
	        	...
	        	if ($markbook->getColumnCountTotal() > $markbook->getColumnCountThisPage()) {
		            ...
		            if ($pageNum <= 0) {
		                ...
		                echo "<a href='".$_SESSION[$guid]['absoluteURL'].'/index.php?q=/modules/'.$_SESSION[$guid]['module']."/markbook_view.php&pupilsightCourseClassID=$pupilsightCourseClassID&page=".($pageNum - 1)."'>".__('Older').'</a>';
		            }
		            echo ' | ';
		            if ((($pageNum + 1) * $markbook->getColumnsPerPage() ) >= $markbook->getColumnCountTotal() ) {
		                echo __('Newer');
		            } else {
		                echo "<a href='".$_SESSION[$guid]['absoluteURL'].'/index.php?q=/modules/'.$_SESSION[$guid]['module']."/markbook_view.php&pupilsightCourseClassID=$pupilsightCourseClassID&page=".($pageNum + 1)."'>".__('Newer').'</a>';
		            }
		        }
	        echo '</div>';
        }
        ...
        if (isActionAccessible($guid, $connection2, '/modules/Markbook/markbook_edit.php') and $canEditThisClass) {
			...
			echo "<a href='#' onclick='resetOrder()'>".__('Reset Order')."<img title='".__('Reset Order')."' src='./themes/".$_SESSION[$guid]['pupilsightThemeName']."/img/reincarnate.png'/></a> | ";
            echo "<a href='".$_SESSION[$guid]['absoluteURL'].'/index.php?q=/modules/'.$_SESSION[$guid]['module']."/markbook_edit_targets.php&pupilsightCourseClassID=$pupilsightCourseClassID'>".__('Targets')."<img title='".__('Set Personalised Attainment Targets')."' src='./themes/".$_SESSION[$guid]['pupilsightThemeName']."/img/target.png'/></a> | ";
            if ($markbook->getSetting('enableColumnWeighting') == 'Y') {
                if (isActionAccessible($guid, $connection2, '/modules/Markbook/weighting_manage.php') == true) {
                    echo "<a href='".$_SESSION[$guid]['absoluteURL'].'/index.php?q=/modules/'.$_SESSION[$guid]['module']."/weighting_manage.php&pupilsightCourseClassID=$pupilsightCourseClassID'>".__('Weightings')."<img title='".__('Weightings')."' src='./themes/".$_SESSION[$guid]['pupilsightThemeName']."/img/run.png'/></a> | ";
                }
            }
            echo "<a href='".$_SESSION[$guid]['absoluteURL']."/modules/Markbook/markbook_viewExportAll.php?pupilsightCourseClassID=$pupilsightCourseClassID&return=markbook_view.php'>".__('Export to Excel')."<img title='".__('Export to Excel')."' src='./themes/".$_SESSION[$guid]['pupilsightThemeName']."/img/download.png'/></a>";
        ...
        for ($i = 0; $i < $markbook->getColumnCountThisPage(); ++$i) {
            ...
            $unit = getUnit($connection2, $column->getData('pupilsightUnitID'), '', $column->getData('pupilsightCourseClassID') );
            ...
            echo "<th class='marksColumn notdraggable' data-header='".$column->pupilsightMarkbookColumnID."' style='padding: 0px 0px 30px 0px !important; text-align: center;vertical-align: top;'>";
            ...
            if (isActionAccessible($guid, $connection2, '/modules/Markbook/markbook_edit.php') and $canEditThisClass) {
                ...
                echo "<a href='".$_SESSION[$guid]['absoluteURL'].'/index.php?q=/modules/'.$_SESSION[$guid]['module']."/markbook_edit_edit.php&pupilsightCourseClassID=$pupilsightCourseClassID&pupilsightMarkbookColumnID=".$column->pupilsightMarkbookColumnID."'><img title='".__('Edit')."' src='./themes/".$_SESSION[$guid]['pupilsightThemeName']."/img/config.png'/></a> ";

                echo "<a class='miniIcon' href='".$_SESSION[$guid]['absoluteURL'].'/index.php?q=/modules/'.$_SESSION[$guid]['module']."/markbook_edit_data.php&pupilsightCourseClassID=$pupilsightCourseClassID&pupilsightMarkbookColumnID=".$column->pupilsightMarkbookColumnID."'><img title='".__('Enter Data')."' src='./themes/".$_SESSION[$guid]['pupilsightThemeName']."/img/markbook.png'/> ";
                    ...
                    if ( $includeMarks ) {
                        ...
                        echo "<img title='$weightInfo' src='./themes/".$_SESSION[$guid]['pupilsightThemeName']."/img/iconTick_double.png'/>";
                    } else {
                        if ($markbook->getSetting('enableColumnWeighting') == 'Y' ) {
                            $weightInfo = '<strong>'.__('Excluded from averages').':</strong><br/>'. $weightInfo;
                        }
                        echo "<img title='$weightInfo' src='./themes/".$_SESSION[$guid]['pupilsightThemeName']."/img/iconCross.png'/>";
                    }

                echo "</a> ";
                echo "<a class='thickbox' href='".$_SESSION[$guid]['absoluteURL'].'/fullscreen.php?q=/modules/'.$_SESSION[$guid]['module']."/markbook_edit_delete.php&pupilsightCourseClassID=$pupilsightCourseClassID&pupilsightMarkbookColumnID=".$column->pupilsightMarkbookColumnID."&width=650&height=135'><img title='".__('Delete')."' src='./themes/".$_SESSION[$guid]['pupilsightThemeName']."/img/garbage.png'/></a> ";
                echo "<a href='".$_SESSION[$guid]['absoluteURL'].'/modules/Markbook/markbook_viewExport.php?pupilsightMarkbookColumnID='.$column->pupilsightMarkbookColumnID."&pupilsightCourseClassID=$pupilsightCourseClassID&return=markbook_view.php'><img title='".__('Export to Excel')."' src='./themes/".$_SESSION[$guid]['pupilsightThemeName']."/img/download.png'/></a>";
                echo '</div>';
            }

            echo '<table class="columnLabels blank rounded-t-none" cellspacing=0><tr>';

            if ($column->pupilsightMarkbookColumnID == false ) { //or $contents == false
            	echo '<th>';
            	echo '</th>';
            } else {
                ...
                if ($column->displayAttainment() ) {
                    ...
                    if (empty($scale)) {
                        try {
                            $dataScale = array('pupilsightScaleID' => $column->getData('pupilsightScaleIDAttainment'));
                ...
                if ($column->displayEffort() ) {
                    echo "<th class='columnLabel medColumn'>";
                    try {
                        $dataScale = array('pupilsightScaleID' => $column->getData('pupilsightScaleIDEffort'));
                        $sqlScale = 'SELECT * FROM pupilsightScale WHERE pupilsightScaleID=:pupilsightScaleID';
                        $resultScale = $connection2->prepare($sqlScale);
                        $resultScale->execute($dataScale);
                    } catch (PDOException $e) {
        ...
        if ($markbook->getSetting('enableColumnWeighting') == 'Y' && $columnFilter != 'unmarked') {
            ...
            if ($columnFilter == 'averages') {
                ...
                if ($markbook->getSetting('enableTypeWeighting') == 'Y' ) {
                    if ( ($markbook->getSetting('enableGroupByTerm') == 'Y' && $pupilsightSchoolYearTermID > 0) ||
                         ($markbook->getSetting('enableGroupByTerm') == 'N' && $pupilsightSchoolYearTermID <= 0) ) {
                ...
                } else if (count($markbook->getGroupedMarkbookTypes('year')) > 0 && $pupilsightSchoolYearTermID > 0) {
            ...
            if ($markbook->getSetting('enableTypeWeighting') == 'Y' && count($markbook->getGroupedMarkbookTypes('year')) > 0 && $pupilsightSchoolYearTermID <= 0) {

                if ($columnFilter == 'averages' && $pupilsightSchoolYearTermID <= 0) {
                    foreach ($markbook->getGroupedMarkbookTypes('year') as $type) {
                        echo "<th class='dataColumn notdraggable dragtable-drag-boundary' data-header='$type'>";
                        echo '<div class="verticalText">' . $markbook->getTypeDescription($type) . '</div>';
                        echo '</th>';
        ...
        try {
            if ($studentOrderBy == 'rollOrder') {
                $dataStudents = array('pupilsightCourseClassID' => $pupilsightCourseClassID, 'pupilsightSchoolYearID'=>$_SESSION[$guid]['pupilsightSchoolYearID'] );
                $sqlStudents = "SELECT title, surname, preferredName, pupilsightPerson.pupilsightPersonID, dateStart, rollOrder FROM pupilsightCourseClassPerson INNER JOIN pupilsightPerson ON (pupilsightCourseClassPerson.pupilsightPersonID=pupilsightPerson.pupilsightPersonID) LEFT JOIN pupilsightStudentEnrolment ON (pupilsightStudentEnrolment.pupilsightPersonID=pupilsightCourseClassPerson.pupilsightPersonID) WHERE role='Student' AND pupilsightCourseClassID=:pupilsightCourseClassID AND status='Full' AND (dateStart IS NULL OR dateStart<='".date('Y-m-d')."') AND (dateEnd IS NULL  OR dateEnd>='".date('Y-m-d')."') AND pupilsightSchoolYearID=:pupilsightSchoolYearID ORDER BY ISNULL(rollOrder), rollOrder, surname, preferredName";
            } else if ($studentOrderBy == 'preferredName') {
                $dataStudents = array('pupilsightCourseClassID' => $pupilsightCourseClassID);
                $sqlStudents = "SELECT title, surname, preferredName, pupilsightPerson.pupilsightPersonID, dateStart FROM pupilsightCourseClassPerson JOIN pupilsightPerson ON (pupilsightCourseClassPerson.pupilsightPersonID=pupilsightPerson.pupilsightPersonID) WHERE role='Student' AND pupilsightCourseClassID=:pupilsightCourseClassID AND status='Full' AND (dateStart IS NULL OR dateStart<='".date('Y-m-d')."') AND (dateEnd IS NULL  OR dateEnd>='".date('Y-m-d')."') ORDER BY preferredName, surname";
            } else {
                $dataStudents = array('pupilsightCourseClassID' => $pupilsightCourseClassID);
        ...
        if ($resultStudents->rowCount() < 1) {
            ...
            while ($rowStudents = $resultStudents->fetch()) {
                ...
                echo "<a class='studentName' href='index.php?q=/modules/Students/student_view_details.php&pupilsightPersonID=".$rowStudents['pupilsightPersonID'].'&subpage=Markbook#'.$pupilsightCourseClassID."'>";
                ...
                if ($markbook->hasPersonalizedTargets()) {
                    echo '<td class="dataColumn studentTarget">';
                        echo $markbook->getTargetForStudent( $rowStudents['pupilsightPersonID'] );
                    echo '</td>';
                }
                ...
                for ($i = 0; $i < $markbook->getColumnCountThisPage(); ++$i) {
                    ...
                    try {
                        $dataEntry = array('pupilsightMarkbookColumnID' => $column->pupilsightMarkbookColumnID, 'pupilsightPersonIDStudent' => $rowStudents['pupilsightPersonID']);
                        $sqlEntry = 'SELECT * FROM pupilsightMarkbookEntry WHERE pupilsightMarkbookColumnID=:pupilsightMarkbookColumnID AND pupilsightPersonIDStudent=:pupilsightPersonIDStudent LIMIT 1';
                        $resultEntry = $connection2->prepare($sqlEntry);
                        $resultEntry->execute($dataEntry);
                    } catch (PDOException $e) {
                        echo "<div class='alert alert-danger'>".$e->getMessage().'</div>';
                    ...
                    if ($resultEntry->rowCount() == 1) {
                        ...
                        if ($column->displayAttainment()) {
                            ...
                            if ($column->hasAttainmentRubric()) {
                                echo "<a class='thickbox rubricIcon' href='".$_SESSION[$guid]['absoluteURL'].'/fullscreen.php?q=/modules/'.$_SESSION[$guid]['module'].'/markbook_view_rubric.php&pupilsightRubricID='.$column->getData('pupilsightRubricIDAttainment')."&pupilsightCourseClassID=$pupilsightCourseClassID&pupilsightMarkbookColumnID=".$column->pupilsightMarkbookColumnID.'&pupilsightPersonID='.$rowStudents['pupilsightPersonID']."&mark=FALSE&type=attainment&width=1100&height=550'><img title='".__('View Rubric')."' src='./themes/".$_SESSION[$guid]['pupilsightThemeName']."/img/rubric.png'/></a>";
                            }
                            ...
                            if ($column->hasAttainmentGrade()) {
                                ...
                                if (empty($attainment) && $column->hasAttainmentRubric() == false) {
                                    if (isActionAccessible($guid, $connection2, "/modules/Markbook/markbook_edit.php") && $canEditThisClass) {
                                        print "<a class='markbookQuickEdit' href='" . $_SESSION[$guid]["absoluteURL"] . "/index.php?q=/modules/" . $_SESSION[$guid]["module"] . "/markbook_edit_data.php&pupilsightCourseClassID=$pupilsightCourseClassID&pupilsightMarkbookColumnID=" . $column->pupilsightMarkbookColumnID . "#".$rowStudents["pupilsightPersonID"]."'><img style='margin-top: 3px' title='" . __("Edit") . "' src='./themes/" . $_SESSION[$guid]["pupilsightThemeName"] . "/img/config.png' width='14' height='14'/></a> " ;
                                    }
                                }
                        ...
                        if ($column->displayEffort()) {
                            ...
                            if ($column->hasEffortRubric()) {
                                echo "<a class='thickbox rubricIcon' href='".$_SESSION[$guid]['absoluteURL'].'/fullscreen.php?q=/modules/'.$_SESSION[$guid]['module'].'/markbook_view_rubric.php&pupilsightRubricID='.$column->getData('pupilsightRubricIDEffort')."&pupilsightCourseClassID=$pupilsightCourseClassID&pupilsightMarkbookColumnID=".$column->pupilsightMarkbookColumnID.'&pupilsightPersonID='.$rowStudents['pupilsightPersonID']."&mark=FALSE&type=effort&width=1100&height=550'><img title='".__('View Rubric')."' src='./themes/".$_SESSION[$guid]['pupilsightThemeName']."/img/rubric.png'/></a>";
                            }
                            if ($column->hasEffortGrade()) {
                        ...
                        $editLink = '';
                        if (isActionAccessible($guid, $connection2, "/modules/Markbook/markbook_edit.php") && $canEditThisClass) {
                            $editLink = "<a class='markbookQuickEdit' href='" . $_SESSION[$guid]["absoluteURL"] . "/index.php?q=/modules/" . $_SESSION[$guid]["module"] . "/markbook_edit_data.php&pupilsightCourseClassID=$pupilsightCourseClassID&pupilsightMarkbookColumnID=" . $column->pupilsightMarkbookColumnID . "#".$rowStudents["pupilsightPersonID"]."'><img style='margin-top: 3px' title='" . __("Add") . "' src='./themes/" . $_SESSION[$guid]["pupilsightThemeName"] . "/img/page_new_mini.png'/></a> " ;
                        }
                    ...
                    if ($column->displaySubmission()) {
                        ...
                        echo "<td class='smallColumn'>";
                        try {
                            $dataWork = array('pupilsightPlannerEntryID' => $column->getData('pupilsightPlannerEntryID'), 'pupilsightPersonID' => $rowStudents['pupilsightPersonID']);
                ...
                if ($markbook->getSetting('enableColumnWeighting') == 'Y' && $columnFilter != 'unmarked') {
                    ...
                    if ($columnFilter == 'averages') {
                        ...
                        if ($markbook->getSetting('enableTypeWeighting') == 'Y' ) {
                            if ( ($markbook->getSetting('enableGroupByTerm') == 'Y' && $pupilsightSchoolYearTermID > 0) ||
                                ...
                                foreach ($markbook->getGroupedMarkbookTypes('term') as $type) {
                                    echo '<td class="dataColumn">';
                                        echo $markbook->getFormattedAverage( $markbook->getTypeAverage($rowStudents['pupilsightPersonID'], $pupilsightSchoolYearTermID, $type) );
                                    echo '</td>';
                                    @$totals['typeAverage'][$type] += $markbook->getTypeAverage($rowStudents['pupilsightPersonID'], $pupilsightSchoolYearTermID, $type);
                                }
                            }
                        } else if (count($markbook->getGroupedMarkbookTypes('year')) > 0 && $pupilsightSchoolYearTermID > 0) {
                            foreach ($markbook->getGroupedMarkbookTypes('year') as $type) {
                        ...
                        if ( ($markbook->getSetting('enableGroupByTerm') == 'Y' && $pupilsightSchoolYearTermID <= 0) ) {
                            foreach ($markbook->getCurrentTerms() as $term) {
                                echo '<td class="dataColumn">';
                                    echo $markbook->getFormattedAverage( $markbook->getTermAverage($rowStudents['pupilsightPersonID'], $term['pupilsightSchoolYearTermID']) );
                                echo '</td>';
                                @$totals['termAverage'][$term['pupilsightSchoolYearTermID']] += $markbook->getTermAverage($rowStudents['pupilsightPersonID'], $term['pupilsightSchoolYearTermID']);
                            }
                        }
                    }
                    ...
                    if ($markbook->getSetting('enableGroupByTerm') == 'Y' && $pupilsightSchoolYearTermID > 0) {
                        ...
                        echo $markbook->getFormattedAverage( $markbook->getTermAverage($rowStudents['pupilsightPersonID'], $pupilsightSchoolYearTermID) );
                        echo '</td>';
                        @$totals['termAverage'][$pupilsightSchoolYearTermID] += $markbook->getTermAverage($rowStudents['pupilsightPersonID'], $pupilsightSchoolYearTermID);
                    }

                    echo '<td class="dataColumn dataDivider">';
                    echo $markbook->getFormattedAverage( $markbook->getCumulativeAverage($rowStudents['pupilsightPersonID']) );
                    echo '</td>';
                    if ($markbook->getCumulativeAverage($rowStudents['pupilsightPersonID']) != '') {
                        @$totals['cumulativeAverage'] += $markbook->getCumulativeAverage($rowStudents['pupilsightPersonID']);
                        @$totals['count'] += 1;
                    }
                    ...
                    if ($markbook->getSetting('enableTypeWeighting') == 'Y' && count($markbook->getGroupedMarkbookTypes('year')) > 0 && $pupilsightSchoolYearTermID <= 0) {
                        ...
                        if ($columnFilter == 'averages') {
                            foreach ($markbook->getGroupedMarkbookTypes('year') as $type) {
                                echo '<td class="dataColumn">';
                                    echo $markbook->getFormattedAverage( $markbook->getTypeAverage($rowStudents['pupilsightPersonID'], 'final', $type) );
                                echo '</td>';
                                @$totals[$type] += $markbook->getTypeAverage($rowStudents['pupilsightPersonID'], 'final', $type);
                            }
                        }

                        echo '<td class="dataColumn">';
                        echo $markbook->getFormattedAverage($markbook->getFinalGradeAverage($rowStudents['pupilsightPersonID']));
                        echo '</td>';
                        @$totals['finalGrade'] += $markbook->getFinalGradeAverage($rowStudents['pupilsightPersonID']);
                    }
                }
