...
if (isActionAccessible($guid, $connection2, '/modules/Data Updater/data_updates.php') == false) {
    ...
    if ($requiredUpdates == 'Y') {
        $updatesRequiredCount = $dataUpdaterGateway->countAllRequiredUpdatesByPerson($pupilsightPersonID);
    ...
    $updatablePeople = $dataUpdaterGateway->selectUpdatableUsersByPerson($pupilsightPersonID)->toDataSet();
    $updatablePeople->transform(function (&$person) use ($dataUpdaterGateway, $pupilsightPersonID, &$requiredUpdatesByType, &$cutoffDate) {
        $person['updates'] = $dataUpdaterGateway->selectDataUpdatesByPerson($person['pupilsightPersonID'], $pupilsightPersonID)->fetchGrouped();
