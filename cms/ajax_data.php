...
if ($type == "insertcampaigndetails") {
    ...
    if (!empty($pupilsightYearGroupID) && !empty($submissionId)) {
        ...
        $sqlchfee =
            "SELECT is_fee_generate FROM campaign WHERE id = " . $campid . " ";
        $resultchkfee = database::doSelectOne($sqlchfee);
        $chkfeeSettNew = $resultchkfee["is_fee_generate"];

        // if($chkfeeSett == '2'){
        if ($chkfeeSettNew == "2") {
            $insert = $adminlib->updateApplicantData(
                $submissionId,
                $pupilsightProgramID,
            ...
            $sql =
                "SELECT b.id, b.formatval FROM campaign AS a LEFT JOIN fn_fee_series AS b ON a.application_series_id = b.id WHERE a.id = " .
                $campid .
                " ";
            ...
            if (!empty($result["formatval"])) {
                ...
                foreach ($invformat as $inv) {
                    if ($inv == "{AB}") {
                        $sqlfort =
                            "SELECT id, no_of_digit, last_no FROM fn_fee_series_number_format WHERE fn_fee_series_id=" .
                            $seriesId .
                            ' AND type= "numberwise"';
                        ...
                        $sql1 =
                            "UPDATE fn_fee_series_number_format SET last_no= " .
                            $lastno .
                            " WHERE fn_fee_series_id= " .
                            $seriesId .
                            " AND type= 'numberwise'  ";
            ...
            $insert = $adminlib->updateApplicantData2(
                $submissionId,
                $pupilsightProgramID,
                $pupilsightYearGroupID,
                $application_id
            ...
            $sqlfs =
                "SELECT academic_id, pupilsightProgramID, fn_fee_structure_id, fn_fees_receipt_template_id  FROM campaign WHERE id = " .
                $campid .
                " ";
            ...
            if (!empty($campData["fn_fee_structure_id"])) {
                ...
                $datas = ["id" => $id];
                $sqls =
                    "SELECT a.*, b.formatval FROM fn_fee_structure AS a LEFT JOIN fn_fee_series AS b ON a.inv_fee_series_id = b.id WHERE a.id=:id";
                $results = $connection2->prepare($sqls);
                $results->execute($datas);
                $values = $results->fetch();

                $datac = ["fn_fee_structure_id" => $id];
                $sqlc =
                    "SELECT * FROM fn_fee_structure_item WHERE fn_fee_structure_id=:fn_fee_structure_id";
                $resultc = $connection2->prepare($sqlc);
                $resultc->execute($datac);
                $childvalues = $resultc->fetchAll();

                $data = [
                    "title" => $values["invoice_title"],
                    "fn_fee_structure_id" => $id,
                    "pupilsightSchoolYearID" =>
                        $values["pupilsightSchoolYearID"],
                    "pupilsightSchoolFinanceYearID" =>
                        $values["pupilsightSchoolFinanceYearID"],
                    "inv_fn_fee_series_id" => $values["inv_fee_series_id"],
                    "rec_fn_fee_series_id" => $values["recp_fee_series_id"],
                    "fn_fees_head_id" => $values["fn_fees_head_id"],
                    "fn_fees_fine_rule_id" => $values["fn_fees_fine_rule_id"],
                    "fn_fees_discount_id" => $values["fn_fees_discount_id"],
                    "due_date" => $values["due_date"],
                    "amount_editable" => $values["amount_editable"],
                    "display_fee_item" => $values["display_fee_item"],
                    "cdt" => $cdt,
                ];

                $sql =
                    "INSERT INTO fn_fee_invoice SET title=:title, fn_fee_structure_id=:fn_fee_structure_id, pupilsightSchoolYearID=:pupilsightSchoolYearID, pupilsightSchoolFinanceYearID=:pupilsightSchoolFinanceYearID, inv_fn_fee_series_id=:inv_fn_fee_series_id, rec_fn_fee_series_id=:rec_fn_fee_series_id, fn_fees_head_id=:fn_fees_head_id, fn_fees_fine_rule_id=:fn_fees_fine_rule_id, fn_fees_discount_id=:fn_fees_discount_id, due_date=:due_date, amount_editable=:amount_editable, display_fee_item=:display_fee_item, cdt=:cdt";
                ...
                if (!empty($childvalues)) {
                    foreach ($childvalues as $cv) {
                        ...
                        if (!empty($feeitem) && !empty($amt)) {
                            $data1 = [
                                "fn_fee_invoice_id" => $invId,
                                "fn_fee_item_id" => $feeitem,
                                "description" => $desc,
                                "amount" => $amt,
                                "tax" => $taxdata,
                                "discount" => $disc,
                            ...
                            $sql1 =
                                "INSERT INTO fn_fee_invoice_item SET fn_fee_invoice_id=:fn_fee_invoice_id, fn_fee_item_id=:fn_fee_item_id, description=:description, amount=:amount,  tax=:tax, discount=:discount, total_amount=:total_amount";
                ...
                $sqlav =
                    "SELECT * FROM fn_fee_invoice_class_assign WHERE fn_fee_invoice_id=:fn_fee_invoice_id AND pupilsightProgramID=:pupilsightProgramID AND pupilsightYearGroupID=:pupilsightYearGroupID";
                $resultav = $connection2->prepare($sqlav);
                $resultav->execute($dataav);
                if ($resultav->rowCount() == 0) {
                    $sql1av =
                        "INSERT INTO fn_fee_invoice_class_assign SET fn_fee_invoice_id=:fn_fee_invoice_id,pupilsightProgramID=:pupilsightProgramID, pupilsightYearGroupID=:pupilsightYearGroupID";
                ...
                $sqlstu =
                    "SELECT * FROM fn_fee_invoice_applicant_assign WHERE fn_fee_invoice_id=:fn_fee_invoice_id AND submission_id=:submission_id";
                ...
                if ($resultstu->rowCount() == 0) {
                    ...
                    $sqlstu1 =
                        "INSERT INTO fn_fee_invoice_applicant_assign SET fn_fee_invoice_id=:fn_fee_invoice_id,invoice_no=:invoice_no, submission_id=:submission_id";
            ...
            $sqlet =
                "SELECT b.* FROM campaign AS a LEFT JOIN pupilsightTemplate AS b ON a.email_template_id = b.pupilsightTemplateID WHERE a.id = " .
                $campid .
                " ";
            ...
            $sqlst =
                "SELECT b.* FROM campaign AS a LEFT JOIN pupilsightTemplate AS b ON a.sms_template_id = b.pupilsightTemplateID WHERE a.id = " .
                $campid .
                " ";
            ...
            $sqle =
                "SELECT response, application_id FROM wp_fluentform_submissions WHERE id = " .
                $subid .
                " ";
            ...
            if (!empty($application_no)) {
                $emailquote = str_replace(
                    "@application_no",
                    $application_no,
                    $emailquote
                );
                $smsquote = str_replace(
                    "@application_no",
                    $application_no,
                    $smsquote
                );
            ...
            $sqlm =
                ...
                $subid .
                " And field_name = 'numeric-field_1' ";
...
if ($type == "saveApplicantForm") {
    ...
    // print_r($data);
    file_put_contents(
        "../public/applicationpdf/" . $submissionId . "-application.pdf",
        $data
    );
}
...
if ($type == "chkPreviousSubmission") {
    ...
    $form_id = $_POST["fid"];
    $sql =
        "SELECT id FROM wp_fluentform_entry_details WHERE form_id=" .
        $form_id .
        ' AND field_value = "' .
        $val .
        '" ';
...
if ($type == "getCampClass") {
    ...
    $cid = $_POST["cid"];
    $sql =
        "SELECT a.id, b.pupilsightYearGroupID, b.name FROM campaign_prog_class AS a LEFT JOIN pupilsightYearGroup AS b ON a.pupilsightYearGroupID = b.pupilsightYearGroupID WHERE a.pupilsightProgramId=" .
        $val .
        ' AND a.campaign_id = "' .
        $cid .
        '" ';
...
function sendSMS($number, $msg, $subid, $cuid, $connection2, $container)
{
    ...
    $sq =
        "INSERT INTO campaign_email_sms_sent_details SET  submission_id = " .
        $subid .
        ", phone=" .
        $number .
        ", description='" .
