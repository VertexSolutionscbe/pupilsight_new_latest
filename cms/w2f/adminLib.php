...
class adminlib {
	...
	public function getApplist($val){
		...
		$sql = "SELECT  cm.id, cm.form_id,cm.name, we.submission_id, ws.created_at FROM wp_fluentform_entry_details as we LEFT JOIN campaign AS cm ON we.form_id=cm.form_id LEFT JOIN wp_fluentform_submissions AS ws ON we.submission_id=ws.id WHERE we.field_value = '".$val."' AND we.field_name = 'email' ";
		$result = database::doSelect($sql);	
