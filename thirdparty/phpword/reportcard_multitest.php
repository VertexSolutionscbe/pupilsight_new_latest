        ...
        foreach($testdata as $td){
            ...
            foreach($sketchData as $sd){
                ...
                if($sd['report_column_word'] == 'marks_obtained'){
                    ...
                    $subsql = 'SELECT a.pupilsightDepartmentID, AVG(a.max_marks) AS max_marks, a.skill_configure, a.gradeSystemId, b.name AS subject FROM examinationSubjectToTest AS a LEFT JOIN pupilsightDepartment AS b ON a.pupilsightDepartmentID = b.pupilsightDepartmentID WHERE a.test_id IN ('.$testIds.')  GROUP BY a.pupilsightDepartmentID ';
                    ...
                    foreach($testdatasub as $k => $tsub){
                        ...
                        //     $sqlmarks = 'SELECT AVG(marks_obtained) AS getmarks FROM examinationMarksEntrybySubject WHERE pupilsightDepartmentID = '.$tsub['pupilsightDepartmentID'].' AND pupilsightPersonID = '.$td['pupilsightPersonID'].' AND test_id IN ('.$testIds.') ';
                        // } else if($tsub['skill_configure'] == 'Sum'){
                        //     $sqlmarks = 'SELECT SUM(marks_obtained) AS getmarks FROM examinationMarksEntrybySubject WHERE pupilsightDepartmentID = '.$tsub['pupilsightDepartmentID'].' AND pupilsightPersonID = '.$td['pupilsightPersonID'].' AND test_id IN ('.$testIds.') ';
                        // } else {
                            $sqlmarks = 'SELECT AVG(marks_obtained) AS getmarks FROM examinationMarksEntrybySubject WHERE pupilsightDepartmentID = '.$tsub['pupilsightDepartmentID'].' AND pupilsightPersonID = '.$td['pupilsightPersonID'].' AND test_id IN ('.$testIds.') ';
                        // }
