...
if(!empty($sketch_id)){
    ...
    if(!empty($filename)){
        ...
        foreach($testdata as $td){
            ...
            foreach($sketchData as $sd){
                ...
                if($sd['report_column_word'] == 'student_id'){
                    $phpword->setValue($sd['report_column_word'], $td['pupilsightPersonID']);
                }

                if($sd['report_column_word'] == 'student_name'){
                    $phpword->setValue($sd['report_column_word'], $td['officialName']);
                }

                if($sd['report_column_word'] == 'student_gender'){
                    $phpword->setValue($sd['report_column_word'], $td['gender']);
                }

                if($sd['report_column_word'] == 'student_dob'){
                    $phpword->setValue($sd['report_column_word'], $td['dob']);
                }

                if($sd['report_column_word'] == 'student_address'){
                    $phpword->setValue($sd['report_column_word'], $td['address1']);
                }
                
                if($sd['report_column_word'] == 'father_name'){
                    $phpword->setValue($sd['report_column_word'], $father_name);
                }

                if($sd['report_column_word'] == 'father_email'){
                    $phpword->setValue($sd['report_column_word'], $father_email);
                }

                if($sd['report_column_word'] == 'father_phone'){
                    $phpword->setValue($sd['report_column_word'], $father_phone);
                }

                if($sd['report_column_word'] == 'mother_name'){
                    $phpword->setValue($sd['report_column_word'], $mother_name);
                }

                if($sd['report_column_word'] == 'mother_email'){
                    $phpword->setValue($sd['report_column_word'], $mother_email);
                }

                if($sd['report_column_word'] == 'mother_phone'){
                    $phpword->setValue($sd['report_column_word'], $mother_phone);
                }

                if($sd['report_column_word'] == 'class'){
                    $phpword->setValue($sd['report_column_word'], $td['classname']);
                }

                if($sd['report_column_word'] == 'section'){
                    $phpword->setValue($sd['report_column_word'], $td['sectionname']);
                }
                ...
                if($sd['report_column_word'] == 'marks_obtained'){
                    ...
                    $subsql = 'SELECT a.pupilsightDepartmentID, a.max_marks, a.skill_configure, a.gradeSystemId, b.name AS subject FROM examinationSubjectToTest AS a LEFT JOIN pupilsightDepartment AS b ON a.pupilsightDepartmentID = b.pupilsightDepartmentID WHERE test_id = '.$test_id.' GROUP BY a.pupilsightDepartmentID ';
                    ...
                    foreach($testdatasub as $k => $tsub){
                        ...
                        $getmarks = str_replace('.00','',$getmarks);
            ...
            header('Content-Length: ' . filesize($savedocsx));
            ob_clean();
            flush();
            readfile($savedocsx);
            unlink($savedocsx);
    ...
    if(!empty($testdataAc['filename'])){
        ...
        $sql = 'SELECT b.officialName, b.pupilsightPersonID, d.name as classname, e.name as sectionname FROM `examinationMarksEntrybySubject` AS a LEFT JOIN pupilsightPerson AS b ON a.pupilsightPersonID = b.pupilsightPersonID LEFT JOIN pupilsightStudentEnrolment AS c ON a.pupilsightPersonID = c.pupilsightPersonID LEFT JOIN pupilsightYearGroup AS d ON c.pupilsightYearGroupID = d.pupilsightYearGroupID LEFT JOIN pupilsightRollGroup AS e ON c.pupilsightRollGroupID = e.pupilsightRollGroupID WHERE a.test_id = '.$test_id.' AND a.pupilsightPersonID = '.$pupilsightPersonID.' GROUP BY a.pupilsightPersonID ';
