...
$sqlpt =
    "SELECT offline_template_path, offline_template_filename FROM campaign WHERE id = '" .
    $cid .
    "' ";
//echo $sqlpt;
...
$file = $valuept["offline_template_path"];

$sqlf =
    "Select a.academic_id, b.form_fields FROM campaign AS a LEFT JOIN wp_fluentform_forms AS b ON a.offline_form_id = b.id WHERE a.id = ' " .
    $cid .
    " ' ";
                ...
                try {
                    if ($value["field_name"] == "student_photo") {
                        $sql =
                            ...
                            $cid .
                            '" AND field_name = "student_photo" AND template_type = "Offline" ';
                        ...
                        $sql =
                            'SELECT * FROM campaign_configure_image_template WHERE campaign_id = "' .
                            $cid .
                            '" AND field_name = "father_photo" AND template_type = "Offline" ';
                        $result = $connection2->query($sql);
                        $imgStuData = $result->fetch();
                        if (!empty($imgStuData)) {
                            $imgData[$k]["pageno"] = $imgStuData["page_no"];
                        ...
                        $sql =
                            'SELECT * FROM campaign_configure_image_template WHERE campaign_id = "' .
                            $cid .
                            '" AND field_name = "mother_photo" AND template_type = "Offline" ';
                        $result = $connection2->query($sql);
                        $imgStuData = $result->fetch();
                        if (!empty($imgStuData)) {
                            $imgData[$k]["pageno"] = $imgStuData["page_no"];
                        ...
                        $sql =
                            'SELECT * FROM campaign_configure_image_template WHERE campaign_id = "' .
                            $cid .
                            '" AND field_name = "guardian_photo" AND template_type = "Offline" ';
                        $result = $connection2->query($sql);
                        $imgStuData = $result->fetch();
                        if (!empty($imgStuData)) {
                            $imgData[$k]["pageno"] = $imgStuData["page_no"];
