	...
	if(!empty($sid) && !empty($cid)){
		...
		if(empty($chkInvoice['id'])){
			...
			if(!empty($invoiceno)){
				...
				$transcation_amount = $response->getTrnAmt();
				$amount_paying = $response->getTrnAmt();
				$total_amount_without_fine_discount = $response->getTrnAmt();
				$deposit = '';
				$fine = '';
				$discount = '';
				$remarks = '';
                ...
                if(!empty($fn_fees_receipt_template_id)){
                    ...
                    if(!empty($transactionId)){
						$URL = $_SESSION[$guid]['absoluteURL']."?return=success";
                    } else {
                        $URL = $_SESSION[$guid]['absoluteURL']."?return=success";
                    }
