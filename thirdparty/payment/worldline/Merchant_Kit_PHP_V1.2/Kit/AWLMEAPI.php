...
class AWLMEAPI
{
	...
	{ //echo $reqMsgDTO; die;
		...
		if ($reqMsgDTO->getAddField10() == '' || is_null($reqMsgDTO->getAddField10())) {
			$reqMsgDTO->setAddField10("NA");
		}

		if ($reqMsgDTO->getAddField9() == '' || is_null($reqMsgDTO->getAddField9())) {
		...
		try {
			...
			if (
				is_null($reqMsgDTO) || $reqMsgDTO == ''
				|| is_null($reqMsgDTO->getMid() || $reqMsgDTO->getMid() == '')
				|| is_null($reqMsgDTO->getOrderId()) || $reqMsgDTO->getOrderId() == ''
				|| is_null($reqMsgDTO->getTrnAmt()) || $reqMsgDTO->getTrnAmt() == ''
				|| is_null($reqMsgDTO->getMeTransReqType()) || $reqMsgDTO->getMeTransReqType() == ''
				|| is_null($reqMsgDTO->getEnckey()) ||  $reqMsgDTO->getEnckey() == ''
				|| is_null($reqMsgDTO->getResponseUrl()) || $reqMsgDTO->getResponseUrl() == ''
				|| is_null($reqMsgDTO->getTrnCurrency()) || $reqMsgDTO->getTrnCurrency() == ''
			) {
			...
			if (($reqMsgDTO->getMeTransReqType() == "R")
				&& (is_null($reqMsgDTO->getRecurrPeriod()) || $reqMsgDTO->getRecurrPeriod() == ''
					|| is_null($reqMsgDTO->getRecurrDay()) ||  $reqMsgDTO->getRecurrDay() == ''
					|| is_null($reqMsgDTO->getNoOfRecurring()) || $reqMsgDTO->getNoOfRecurring() == '')
			) {
			...
			print_r($merchantReqStr);
			print_r($reqMsgDTO->getEnckey());
			die();
			...
			$reqMsgDTO->setStatusDesc("Error Occurred during Creating request message");
	...
	function getTransactionStatus($mid, $orderid, $pgmetrn, $enc_key)
	{
		...
		if (is_null($mid) || $mid == '' || $orderid == '' || is_null($orderid) || is_null($enc_key) || $enc_key == '') {
			$resMsgDTO->setStatusCode("F");
	...
	function cancelTransaction($reqMsgDTO)
	{
		...
		try {
			if (
				is_null($reqMsgDTO->getMid()) || $reqMsgDTO->getMid() == ''
				|| is_null($reqMsgDTO->getOrderId()) || $reqMsgDTO->getOrderId() == ''
				|| is_null($reqMsgDTO->getPgMeTrnRefNo()) || $reqMsgDTO->getPgMeTrnRefNo() == ''
				|| is_null($reqMsgDTO->getEnckey()) || $reqMsgDTO->getEnckey() == ''
			) {
			...
			if (is_null($reqMsgDTO->getAddField10()) || $reqMsgDTO->getAddField10() == '') {
				$reqMsgDTO->setAddField10('NA');
			}
	...
	{   //$reqMsgDTO = new ReqMsgDTO(); 
		...
		try {
			if (
				...
				|| is_null($reqMsgDTO->getPgMeTrnRefNo()) || $reqMsgDTO->getPgMeTrnRefNo() == ''
				|| is_null($reqMsgDTO->getEnckey()) || $reqMsgDTO->getEnckey() == ''
				|| is_null($reqMsgDTO->getRefundAmt()) || $reqMsgDTO->getRefundAmt() == ''
			) {
	...
	function generateTrnReqMsgWithCard($reqMsgDTO)
	{
		...
		try {
			if (
				is_null($reqMsgDTO) || $reqMsgDTO == '' || is_null($reqMsgDTO->getMid() || $reqMsgDTO->getMid() == '')
				|| is_null($reqMsgDTO->getOrderId()) || $reqMsgDTO->getOrderId() == ''
				|| is_null($reqMsgDTO->getTrnAmt())  || $reqMsgDTO->getTrnAmt() == ''
				|| is_null($reqMsgDTO->getMeTransReqType()) || $reqMsgDTO->getMeTransReqType() == ''
				|| is_null($reqMsgDTO->getEnckey()) || $reqMsgDTO->getEnckey() == ''
			...
			if (
				$reqMsgDTO->getPayTypeCode() == "DC" || $reqMsgDTO->getPayTypeCode() == "CC"
				||  $reqMsgDTO->getPayTypeCode() == "CUG"
			) {

				if (
					is_null($reqMsgDTO->getCardNumber()) || $reqMsgDTO->getCardNumber() == ''
					|| is_null($reqMsgDTO->getExpiryDate()) || $reqMsgDTO->getExpiryDate() == ''
					|| is_null($reqMsgDTO->getPayTypeCode()) || $reqMsgDTO->getPayTypeCode() == ''
				) {
				...
				if (is_null($reqMsgDTO->getNetBankCode()) || $reqMsgDTO->getNetBankCode() == '') {
